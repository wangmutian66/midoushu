...
class Index extends Base {
    ...
    public function index(){      
        ...
        // 如果是手机跳转到 手机模块
        if(isMobile()){
            $this->redirect(U('Mobilered/Index/index'));
        }

        // 热销产品 推荐分类 费嘞列表 推荐商品
        $hot_goods = $hot_cate = $cateList = $recommend_goods = array();
        $sql = "select a.goods_name,a.goods_id,a.suppliers_id,a.shop_price,a.market_price,a.cat_id,b.parent_id_path,b.name from ".C('database.prefix')."goods_red as a left join ";
        $sql .= C('database.prefix')."goods_red_category as b on a.cat_id=b.id where a.is_hot=1 and a.is_on_sale=1 and a.is_check=1 order by a.sort";//二级分类下热卖商品       
        $index_hot_goods = S('index_hot_goods_red'); 
    //    dump($index_hot_goods);die;
        if(empty($index_hot_goods))
        {
            $index_hot_goods = Db::query($sql);//首页热卖商品
            S('index_hot_goods_red',$index_hot_goods,TPSHOP_CACHE_TIME);
        }
       
        if($index_hot_goods){
            foreach($index_hot_goods as $k => $val){
                // 米豆换算
        ...
        $sql2 = "select a.goods_name,a.goods_id,a.suppliers_id,a.shop_price,a.market_price,a.cat_id,b.parent_id_path,b.name from ".C('database.prefix')."goods_red as a left join ";
        $sql2 .= C('database.prefix')."goods_red_category as b on a.cat_id=b.id where a.is_recommend=1 and a.is_on_sale=1 and a.is_check=1 order by a.sort";//二级分类下热卖商品
        $index_recommend_goods = S('index_recommend_goods_red'); 
        if(empty($index_recommend_goods))
        {
        	$index_recommend_goods = Db::query($sql2);//首页推荐商品
        	S('index_recommend_goods_red',$index_recommend_goods,TPSHOP_CACHE_TIME);
        }
         
        if($index_recommend_goods){
        	foreach($index_recommend_goods as $k => $va){
                // 米豆换算
                $midouInfo = getMidou($va['goods_id']);
                $va['midou']       = $midouInfo['midou'];
                $va['midou_money'] = $midouInfo['midou_money'];
                $va['midou_index'] = $midouInfo['midou_index'];
        ...
        $hot_category = M('goods_red_category')->where("is_hot=1 and level=3 and is_show=1")->cache(true,TPSHOP_CACHE_TIME)->select();//热门三级分类
        ...
        $newgoodsList = Db::name('goods_red')
            ->where($new_goods_where)
            ->order("new_sort desc")
            ->cache("pc_home_red_new_goods_list")
            ->limit(4)
            ->select();
        ...
        $hotgoodsList = Db::name('goods_red')
            ->where($hot_goods_where)
            ->cache("pc_home_red_hot_goods_list")
            ->limit(4)
            ->order('hot_sort desc')
            ->select();
    ...
    public function ajax_favorite(){
        ...
        $favourite_goods = Db::name('goods_red')->where($where)->order('goods_id DESC')->page($p,$i)->cache(true,TPSHOP_CACHE_TIME)->select();//首页推荐商品
