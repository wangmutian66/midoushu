...
class Order extends Base {
    ...
    public function order_list(){
        ...
        if($search_key)
        {
            $where .= " and (order_sn like :search_key1 or order_id in (select order_id from `".C('database.prefix')."order_red_goods` where goods_name like :search_key2) ) ";
            $bind['search_key1'] = "%$search_key%";
            $bind['search_key2'] = "%$search_key%";
        }
        $where.=' and order_prom_type < 5 ';//虚拟拼团订单不列出来
        $count = M('order_red')->where($where)->bind($bind)->count();
        $Page = new Page($count,10);

        $show = $Page->show_cxpc();
        $order_str = "order_id DESC";
        $order_list = M('order_red')->order($order_str)->where($where)->bind($bind)->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function order_detail(){
        ...
        if($order_info['order_prom_type'] == 4){
            $pre_sell_item =  M('goods_red_activity')->where(array('act_id'=>$order_info['order_prom_id']))->find();
            $pre_sell_item = array_merge($pre_sell_item,unserialize($pre_sell_item['ext_info']));
        ...
        //获取订单进度条
        $sql = "SELECT action_id,log_time,status_desc,order_status FROM ((SELECT * FROM __PREFIX__order_red_action WHERE order_id = :id AND status_desc <>'' ORDER BY action_id) AS a) GROUP BY status_desc ORDER BY action_id";
        $bind['id'] = $id;
    ...
    public function cancel_order_info(){
    	$order_id = I('order_id/d',0);
    	$order = M('order_red')->where(array('order_id'=>$order_id,'order_status'=>3,'pay_status'=>1))->find();
    ...
    public function refund_order()
    {
    	...
    	$order = M('order_red')
                ->field('order_id,consignee,mobile,pay_code,pay_name,user_money,integral_money,coupon_price,order_amount,total_amount,order_midou')
    ...
    public function virtual_order(){
    	...
    	//获取订单操作记录
    	$order_action = M('order_red_action')->where(array('order_id'=>$order_id))->select();
    	$this->assign('order_status',C('ORDER_STATUS'));
    	$this->assign('pay_status',C('PAY_STATUS'));
    	$this->assign('order_info',$order_info);
    	$this->assign('order_action',$order_action);
    	...
    	if($order_info['pay_status'] == 1 && $order_info['order_status']!=3){
    		$vrorder = M('vr_red_order_code')->where(array('order_id'=>$order_id))->select();
    ...
    public function delComment()
    {
        ...
        $comment = Db::name('comment_red')->where('comment_id', $comment_id)->find();
        if ($this->user_id != $comment['user_id']) {
            $this->error('不能删除别人的评论');
        }
        Db::name('reply_red')->where('comment_id', $comment_id)->delete();
        Db::name('comment_red')->where('comment_id', $comment_id)->delete();
        $this->success('删除评论成功');
    }
    ...
    public function ajaxZan()
    {
        ...
        $user_id = $this->user_id;
        $comment_info = M('comment_red')->where(array('comment_id' => $comment_id))->find();
    ...
    public function reply_add()
    {
        ...
        $user_goods_count = Db::name('order_red')
            ->alias('o')
            ->join('__ORDER_RED_GOODS__ og','o.order_id = og.order_id', 'LEFT')
            ->where($where)
            ->count();
        if ($user_goods_count > 0) {
            M('reply_red')->add($reply_data);
            M('comment_red')->where(array('comment_id' => $comment_id))->setInc('reply_num');
            $json['success'] = true;
        } else {
            $json['success'] = false;
    ...
    public function return_goods_list()
    {
        ...
        if(!empty($goods_id_arr))
            $goodsList = M('goods_red')->where("goods_id","in", implode(',',$goods_id_arr))->getField('goods_id,goods_name,shop_price');
    ...
    {
        $id = I('id/d',0);
        $ReturnGoodsModel = new \app\common\model\ReturnRedGoods();
    ...
    */
    ...
    public function return_goods_cancel(){
        ...
        $res=M('return_red_goods')->where(['id'=>$id,'user_id'=>$this->user_id])->save(array('status'=>-2,'canceltime'=>time()));
        if($res){
    ...
    public function comment_list()
    {
        ...
        if (empty($order_id) || empty($rec_id)) {
            ...
            $order_info = M('order_red')->field('order_sn,order_id,add_time,order_prom_type,suppliers_id') ->where($order_comment_where)->find();
            //查找评价商品
            $order_comment_where['rec_id'] = $rec_id;
            $order_goods = M('order_red_goods')
                ->field('goods_id,is_comment,goods_name,goods_num,goods_price,spec_key_name,suppliers_id,midou')
