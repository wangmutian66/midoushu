...
class Payment extends Base {
    ...
    public function getCode(){        
            ...
            if($order_num == 1){
                ...
                M('order_red')->where("order_id",$order_id)->save(array('pay_code'=>$this->pay_code,'pay_name'=>$payment_arr[$this->pay_code]));
            } else {
                $order    = M('order_red')->where('order_sn ="'.$order_sn.'" OR parent_sn ="'.$order_sn.'"')->select();
                M('order_red')->where('order_sn ="'.$order_sn.'" OR parent_sn ="'.$order_sn.'"')->save(array('pay_code'=>$this->pay_code,'pay_name'=>$payment_arr[$this->pay_code]));
            }
            ...
            if($order_num == 1){
                ...
                $order_new['order_amount'] = M('order_red')->where('order_sn ="'.$order_sn.'" OR parent_sn ="'.$order_sn.'"')->sum('order_amount');
            }
    ...
    public function notifyUrl(){            
        $this->payment->response_red();            
        exit();
    }

    // 页面跳转 // http://www.tp-shop.cn/index.php/Home/Payment/returnUrl        
    public function returnUrl(){
        $result = $this->payment->respond2_red(); // $result['order_sn'] = '201512241425288593';
        ...
        if(session('order_id')){
            $order = M('order_red')->where("order_sn", $result['order_sn'])->find();
            //2018-9-25 王牧田修改  订单提交支付后直接确认
            M('order_red')->where("order_id", $order['order_id'])->save(['order_status'=>1]);
        ...
        if(session('order_sn')){
            $order = M('order_red')->where('order_sn ="'.$result['order_sn'].'" OR parent_sn ="'.$result['order_sn'].'"')->find();
            //2018-9-25 王牧田修改  订单提交支付后直接确认
            M('order_red')->where("order_id", $order['order_id'])->save(['order_status'=>1]);
            if(empty($order)) // order_sn 找不到 根据 order_id 去找
            {
                $order_sn = session('order_sn'); // 最近支付的一笔订单 id        
                $order = M('order_red')->where('order_sn ="'.$order_sn.'" OR parent_sn ="'.$order_sn.'"')->find();
            }       
            $this->assign('order', $order);
        } 
