...
// 更改帖子评论数和浏览数 http://www.tpshopb2.com/home/test/updatecomment
  ...
  //      echo THINK_VERSION;
    //    $mod = new Testf;
       ...
       /* $list = [
            ['user_id'=>2595,'user_money'=>array('exp','user_money + 5'),'change_time'=>$tims,'desc'=>'全返','order_sn'=>"{$order_sn}",'order_id'=>'{$order_id}'],
            ['user_id'=>2595,'user_money'=>array('exp','user_money + 5'),'change_time'=>$tims,'desc'=>'全返','order_sn'=>"{$order_sn}",'order_id'=>'{$order_id}'],
            ['user_id'=>2595,'user_money'=>array('exp','user_money + 5'),'change_time'=>$tims,'desc'=>'全返','order_sn'=>"{$order_sn}",'order_id'=>'{$order_id}'],
        ];*/
        $list = array(array('name'=>1,'email'=>'test@qq.com'),array('name'=>2,'email'=>'test@cc.com'),array('name'=>3,'email'=>'test@dd.com'),);
        $r = model('Account_log')->saveAll($list);
        dump($r);
        echo model('Account_log')->getlastsql();
    }
