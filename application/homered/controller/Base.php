...
class Base extends Controller {
    ...
    public function public_assign()
    {
        ...
        $brand_list = M('brand_red')->cache(true)->field('id,name,parent_cat_id,logo,is_hot')->where("parent_cat_id>0")->select();
        ...
        if(ACTION_NAME == 'goodsInfo'){
            $goods_id = I('id/d');
            $suppliers_id = db('goods_red')->where(['goods_id'=>$goods_id])->value('suppliers_id');
    ...
    public function new_goods()
    {
        $newmap['is_on_sale'] = 1;
        $newmap['is_new']     = 1;
        $countcus = M('goods_red')->cache(true)->where($newmap)->count();  //获取总记录数
        $max_num = $countcus-3;
        if($max_num > 0) $max_num; else $max_num=0;
        $startnum = rand(0,$max_num);
        $newgoodsLists = M('goods_red')->cache(true)->where($newmap)->limit($startnum.',3')->select();
        foreach ($newgoodsLists as $k => $val) {
            $midouInfo = getMidou($val['goods_id']);
            $newgoodsLists[$k]['midou']       = $midouInfo['midou'];
            $newgoodsLists[$k]['midou_money'] = $midouInfo['midou_money'];
        }
