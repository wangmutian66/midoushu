...
class Api extends Base {
    ...
    public function check_order_pay_status()
    {
        ...
        $order = M('order_red')->field('pay_status')->where(['order_id'=>$order_id])->find();
        if($order['pay_status'] != 0){
            //2018-9-25 王牧田修改  订单提交支付后直接确认
            M('order_red')->where(['order_id'=>$order_id])->save(['order_status'=>1]);
    ...
    public function searchKey(){
        $searchKey = input('key');
        $searchKeyList = Db::name('search_red_word')
