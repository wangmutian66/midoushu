...
#http://www.hljzhp.com/mobile/Goods/goodsInfo/id/1987/recommend_id/1103
    /*
     * 初始化操作
     */
...
//        Session::start();
        ...
        /*推广员 部分开始*/
        $recommend_id= I('get.recommend_id/d');
        $invite_code = I('get.invite_code/s');

        if($recommend_id || $invite_code){
            if($recommend_id){
                $cache_key = "staff_{$recommend_id}";
                $recommend_where['id']    =   ['eq',$recommend_id];
            }
            if($invite_code){
                $cache_key = "staff_{$invite_code}";
                $recommend_where['invite_code']    =   ['eq',$invite_code];
            }
            $staff_info = M('staff')->where($recommend_where)->cache($cache_key)->find(); 
            if($staff_info){
        ...
        if(strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger')){
            ...
            if (empty($_SESSION['openid'])){
                if(is_array($this->weixin_config) && $this->weixin_config['wait_access'] == 1){
                    ...
                    #扫码付款，充值接口不需要绑定账号
                    $special_methods    =   ['sweepcode','club_card'];
                    if ($is_bind_account && !in_array(strtolower(ACTION_NAME), $special_methods)) {
                        if (strtolower(CONTROLLER_NAME) != 'user' || strtolower(ACTION_NAME) != 'bind_guide') {
                ...
                if (isset($_GET['code'])) {
                    ...
                    setcookie('subscribe', $data2['subscribe']);
                }
            }
        }
        //
        #如果手机号码为空，则需要用户绑定手机号码
        if(session('user') && !session('user.mobile')){
            if(strtolower(MODULE_NAME) == 'mobile' && !\Think\Request::instance()->isAjax()){
                $completion_array = ['sweepcode','dosweepCode','do_apply_promoters','dopays','mypays','dopaid','setmobile','consolidated_data','index','goodsinfo','api','mpublic_log','mypays_pay','pay_status','index_welfare','index_cash','club_card','setmobilestore','storegoodsList'];
                $nologin = array(
                    'login', 'pop_login', 'do_login', 'logout', 'verify', 'set_pwd', 'finished',
                    'verifyHandle', 'reg', 'send_sms_reg_code', 'find_pwd', 'check_validate_code',
                    'forget_pwd', 'check_captcha', 'check_username', 'send_validate_code', 'express' , 'bind_guide', 'bind_account','reg1'
                );
                $completion_array = array_merge($completion_array,$nologin);
                if(!in_array(strtolower(ACTION_NAME),$completion_array)){
                    session('referurl',$this->get_url());
                    $this->redirect(U('/Mobile/User/setMobile',['reason'=>'请设置您的手机号码']));
                }
            }
        }
        $this->public_assign();
    ...
    public function public_assign()
    {
       ...
       if(ACTION_NAME == 'goodsInfo'){
            $goods_id = I('id/d');
            $suppliers_id = db('goods')->where(['goods_id'=>$goods_id])->cache(true)->value('suppliers_id');
            ...
            $r = db('chat_group')->cache(true)->where('is_default = 1')->find();
            $user_list = db('users')->where("chat_group_id = {$r['id']} and is_line = 1")->field('user_id')->select();
            $this->assign('chat_group_id',$r['id']);
            $this->assign('is_default_chat',(empty($user_list)?0:1));
        }
        /*分享锁定*/
