...
class ReturnCart extends MobileBase {
    ...
    #    dump($cartList);die;
        ...
        $hot_goods = M('Goods')->where('is_hot=1 and is_allreturn=1 and is_on_sale=1 and is_check=1')->limit(20)->cache(true,TPSHOP_CACHE_TIME)->select();
    ...
    public function cart2(){
        ...
        if(empty($address)){
            header("Location: ".U('Mobile/User/add_address',array('source'=>'cart2','goods_id'=>$goods_id,'goods_num'=>$goods_num,'item_id'=>$item_id,'action'=>$action,'is_allreturn'=>1)));
            exit;
        }else{
        ...
        if($action == 'buy_now'){
            ...
            if ($cartLogic->getUserCartOrderCount(1) == 0){
                $this->error('你的购物车没有选中商品', 'ReturnCart/index');
            }
        ...
        // 获取 供货商物流
        foreach ($cartList['cartList'] as $k => $val) {
            $cartList['cartList'][$k]['shippinglist'] = M('Plugin')->where("`type` = 'shipping' and is_default = 1 and is_default = 1 and suppliers_id = ".$val['suppliers_id']." and status = 1")->cache(true,TPSHOP_CACHE_TIME)->select();  // 物流公司
        }
