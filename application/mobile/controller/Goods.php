...
class Goods extends MobileBase {
    ...
    public function goodsInfo(){
        ...
        if($goods['is_allreturn'] == 1){
            $this->redirect(U('ReturnGoods/goodsInfo',['id' => $goods['goods_id']]));
        }
        // 可返米豆
   ...
   //     dump($commentStatistics);die;
    ...
    {
        $getnum = 9;   //取出数量
        $goods=M('goods')->where(array('is_recommend'=>1,'is_on_sale'=>1,'is_check'=>1))->page(1,$getnum)->cache(true,TPSHOP_CACHE_TIME)->select(); //推荐商品
