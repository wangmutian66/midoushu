...
use think\Validate;
use think\Db;
class GoodsRed extends Validate
{

    // 验证规则
    protected $rule = [
        'goods_id'              =>'checkGoodsId',
        'goods_name'            =>'require|min:3|max:300|unique:goods_red',
        'cat_id'                =>'number|gt:0',
        'goods_sn'              =>'unique:goods_red|max:20',
        'shop_price'            =>['require','regex'=>'([0-9]\d*(\.\d*[0-9])?)|(0\.\d*[0-9])'],
        'market_price'          =>'require|regex:\d{1,10}(\.\d{1,2})?$|checkMarketPrice',
        'cost_price'            =>['require','regex'=>'([1-9]\d*(\.\d*[1-9])?)|(0\.\d*[1-9])'],
        'cost_operating'        =>['regex'=>'([0-9]\d*(\.\d*[0-9])?)|(0\.\d*[0-9])'],
    ...
    protected $message  = [
        ...
        'cost_price.require'                            => '本店成本价必须',
        'cost_price.regex'                              => '本店成本价格式不对',
        'cost_operating.regex'                          => '本店运营成本价格式不对',
        'weight.regex'                                  => '重量格式不对',
    ...
    {
        if ($value > 0) {
            $goods = Db::name('goods_red')->where('goods_id', $data['goods_id'])->find();
            if (!empty($goods)) {
    ...
    protected function checkGoodsId($value){
        $spec_goods_price = Db::name('spec_red_goods_price')->where('goods_id',$value)->where('prom_type','gt',0)->find();
        if($spec_goods_price){
            return '该商品规格：'.$spec_goods_price['key_name'].'正在参与活动，不能编辑该商品信息';
        }
        $goods= Db::name('goods_red')->where('goods_id',$value)->find();
        if($goods['prom_type'] > 0){
