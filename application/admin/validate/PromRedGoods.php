...
use think\Validate;
use think\Db;
class PromRedGoods extends Validate
{
    // 验证规则
    protected $rule = [
        'id'=>'checkId',
...
//        'group.require'         => '请选择适合用户范围',
    ...
    protected function checkExpression($value, $rule ,$data){
        ...
        if ($data['type'] == 2 ||$data['type'] == 1) {
            ...
            if($no_spec_goods){
                $minGoodsPrice = Db::name('goods_red')->where('goods_id','in',$no_spec_goods)->order('shop_price')->find();
                if($data['expression'] > $minGoodsPrice['shop_price']){
                    return '优惠金额不能大于商品为'.$minGoodsPrice['goods_name'].'的价格：'.$minGoodsPrice['shop_price'];
                }
            }
            if($item_ids){
                $minSpecGoodsPrice = Db::name('spec_red_goods_price')->where('item_id', 'in', $item_ids)->order('price')->find();
    ...
    {
        $isHaveOrder = Db::name('order_red_goods')->where(['prom_type'=>3,'prom_id'=>$value])->find();
        if($isHaveOrder){
