...
use think\Validate;
use think\Db;
class FlashRedSale extends Validate
{
    // 验证规则
    protected $rule = [
        ['id','checkId'],
    ...
    protected function checkPrice($value, $rule ,$data)
    {
        if($data['item_id'] > 0){
            //商品规格
            $price = Db::name("spec_red_goods_price")->where(['item_id'=>$data['item_id']])->getField('price');
        }else{
            $price = Db::name('goods_red')->where('goods_id',$data['goods_id'])->getField('shop_price');
        }
    ...
    protected function checkGoodsNum($value, $rule ,$data)
    {
        ...
        if($data['item_id'] > 0){
            //商品规格
            $store_count = Db::name("spec_red_goods_price")->where(['item_id'=>$data['item_id']])->getField('store_count');
        }else{
            $store_count = Db::name("goods_red")->where(['goods_id'=>$data['goods_id']])->getField('store_count');
        }
    ...
    {
        $isHaveOrder = Db::name('order_red_goods')->where(['prom_type'=>1,'prom_id'=>$value])->find();
        if($isHaveOrder){
