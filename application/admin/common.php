 ...
 * @param $user_id or $store_id 涉及申请用户ID或商家ID
 */
function expenseLogRed($data){
	$data['addtime'] = NOW_TIME;
	$data['admin_id'] = session('admin_id');
	M('expense_red_log')->add($data);
}
 ...
 * @param $user_id or $store_id 涉及申请用户ID或商家ID
 */
function expenseLogYxyp($data){
	$data['addtime'] = NOW_TIME;
	$data['admin_id'] = session('admin_id');
	M('expense_yxyp_log')->add($data);
}
//订单商品售后退款
function updateRefundGoods($rec_id,$refund_type=0){
	...
	if(($return_goods['refund_deposit']>0 || $return_goods['refund_integral']>0)){
		accountLog($return_goods['user_id'],$return_goods['refund_deposit'],0,$return_goods['refund_integral'],'用户申请商品退款',0,0,$return_goods['order_id'],$return_goods['order_sn']);
	}
	//在线支付金额退到余额去
	if($refund_type==1 && $return_goods['refund_money']>0){
		accountLog($return_goods['user_id'],$return_goods['refund_money'],0,0,'用户申请商品退款',0,0,$return_goods['order_id'],$return_goods['order_sn']);
	}
	...
	if($return_goods['is_receive'] == 1){
		...
		if($order_goods['give_integral']>0){
			...
			if($order_goods['give_integral']<$user['pay_points']){
				accountLog($return_goods['user_id'],0,0,-$order_goods['give_integral'],'退货积分追回',0,0,$return_goods['order_id'],$return_goods['order_sn']);
			}
		}
		//追回订单商品赠送的优惠券
		...
		if(!empty($coupon_info)){
			if($coupon_info['status'] == 1) { //如果优惠券被使用,那么从退款里扣
				$coupon = M('coupon_red')->where(array('id' => $coupon_info['cid']))->find();
...
//红包商城
// TK 2018年4月21日15:54:12
//订单商品售后退款
function updateRefundGoodsRed($rec_id,$refund_type=0){
	...
	$updata = array('refund_type'=>$refund_type,'refund_time'=>time(),'status'=>3);
	//使用积分或者余额抵扣部分原路退还
	if(($return_goods['refund_deposit']>0 || $return_goods['refund_integral']>0 || $return_goods['refund_midou']>0)){
		accountLog($return_goods['user_id'], $return_goods['refund_deposit'],$return_goods['refund_midou'], $return_goods['refund_integral'], '用户申请 米豆专区 商品退款', 0, 0, $return_goods['order_id'],$return_goods['order_sn']);
	}
	//在线支付金额退到余额去
	if($refund_type==1 && $return_goods['refund_money']>0){
		accountLog($return_goods['user_id'], $return_goods['refund_money'],$return_goods['refund_midou'], $return_goods['refund_integral'], '用户申请 米豆专区 商品退款', 0, 0, $return_goods['order_id'],$return_goods['order_sn']);
	}
	M('return_red_goods')->where(array('rec_id'=>$rec_id))->save($updata);//更新退款申请状态
	M('order_red_goods')->where(array('rec_id'=>$rec_id))->save(array('is_send'=>3));//修改订单商品状态
	if($return_goods['is_receive'] == 1){
		//赠送积分追回
		if($order_goods['give_integral']>0){
			$user = get_user_info($return_goods['user_id']);
			if($order_goods['give_integral']<$user['pay_points']){
				accountLog($return_goods['user_id'],0,0,-$order_goods['give_integral'],'红包商城退货积分追回',0,0,$return_goods['order_id'],$return_goods['order_sn']);
			}
		}
		//追回订单商品赠送的优惠券
		$coupon_info = M('coupon_red_list')->where(array('uid'=>$return_goods['user_id'],'get_order_id'=>$return_goods['order_id']))->find();
		if(!empty($coupon_info)){
			if($coupon_info['status'] == 1) { //如果优惠券被使用,那么从退款里扣
				$coupon = M('coupon_red')->where(array('id' => $coupon_info['cid']))->find();
				if ($return_goods['refund_money'] > $coupon['money']) {
					//退款金额大于优惠券金额直接扣
					$return_goods['refund_money'] = $return_goods['refund_money'] - $coupon['money'];//更新实际退款金额
					M('return_red_goods')->where(['id' => $return_goods['id']])->save(['refund_money' => $return_goods['refund_money']]);
				}else{
					//否则从退还余额里扣
					$return_goods['refund_deposit'] = $return_goods['refund_deposit'] - $coupon['money'];//更新实际退还余额
					M('return_red_goods')->where(['id' => $return_goods['id']])->save(['refund_deposit' => $return_goods['refund_deposit']]);
				}
			}else {
				M('coupon_red_list')->where(array('id' => $coupon_info['id']))->delete();
				M('coupon_red')->where(array('id' => $coupon_info['cid']))->setDec('send_num');//优惠券追回
			}
		}
	}
	//退还使用的优惠券
	$order_goods_count =  M('order_red_goods')->where(array('order_id'=>$return_goods['order_id']))->sum('goods_num');
	$return_goods_count = M('return_red_goods')->where(array('order_id'=>$return_goods['order_id']))->sum('goods_num');
	if($order_goods_count == $return_goods_count){
		$coupon_list = M('coupon_red_list')->where(['uid'=>$return_goods['user_id'],'order_id'=>$return_goods['order_id']])->find();
		if(!empty($coupon_list)){
			$update_coupon_data = ['status'=>0,'use_time'=>0,'order_id'=>0];
			M('coupon_red_list')->where(['id'=>$coupon_list['id'],'status'=>1])->save($update_coupon_data);//符合条件的，优惠券就退给他
		}
	}
	$expense_data = array('money'=>$return_goods['refund_money']+$return_goods['refund_deposit'],'log_type_id'=>$rec_id,'type'=>3,'user_id'=>$return_goods['user_id']);
	expenseLogRed($expense_data);//退款记录日志
}

//红包商城
// TK 2018年4月21日15:54:12
//订单商品售后退款
function updateRefundGoodsYxyp($rec_id,$refund_type=0){
	$order_goods = M('order_yxyp_goods')->where(array('rec_id'=>$rec_id))->find();
	$return_goods = M('return_yxyp_goods')->where(array('rec_id'=>$rec_id))->find();
	$updata = array('refund_type'=>$refund_type,'refund_time'=>time(),'status'=>3);
	//使用积分或者余额抵扣部分原路退还
	if(($return_goods['refund_deposit']>0 || $return_goods['refund_integral']>0 || $return_goods['refund_midou']>0)){
		accountLog($return_goods['user_id'], $return_goods['refund_deposit'],$return_goods['refund_midou'], $return_goods['refund_integral'], '用户申请 米豆专区 商品退款', 0, 0, $return_goods['order_id'],$return_goods['order_sn']);
	...
	M('return_yxyp_goods')->where(array('rec_id'=>$rec_id))->save($updata);//更新退款申请状态
	M('order_yxyp_goods')->where(array('rec_id'=>$rec_id))->save(array('is_send'=>3));//修改订单商品状态
	if($return_goods['is_receive'] == 1){
		//赠送积分追回
		if($order_goods['give_integral']>0){
			$user = get_user_info($return_goods['user_id']);
		...
		//追回订单商品赠送的优惠券
		$coupon_info = M('coupon_yxyp_list')->where(array('uid'=>$return_goods['user_id'],'get_order_id'=>$return_goods['order_id']))->find();
		if(!empty($coupon_info)){
			if($coupon_info['status'] == 1) { //如果优惠券被使用,那么从退款里扣
				$coupon = M('coupon_yxyp')->where(array('id' => $coupon_info['cid']))->find();
				if ($return_goods['refund_money'] > $coupon['money']) {
					//退款金额大于优惠券金额直接扣
					$return_goods['refund_money'] = $return_goods['refund_money'] - $coupon['money'];//更新实际退款金额
					M('return_yxyp_goods')->where(['id' => $return_goods['id']])->save(['refund_money' => $return_goods['refund_money']]);
				}else{
					//否则从退还余额里扣
					$return_goods['refund_deposit'] = $return_goods['refund_deposit'] - $coupon['money'];//更新实际退还余额
					M('return_yxyp_goods')->where(['id' => $return_goods['id']])->save(['refund_deposit' => $return_goods['refund_deposit']]);
				}
			}else {
				M('coupon_yxyp_list')->where(array('id' => $coupon_info['id']))->delete();
				M('coupon_yxyp')->where(array('id' => $coupon_info['cid']))->setDec('send_num');//优惠券追回
			}
		}
	}
	//退还使用的优惠券
	$order_goods_count =  M('order_yxyp_goods')->where(array('order_id'=>$return_goods['order_id']))->sum('goods_num');
	$return_goods_count = M('return_yxyp_goods')->where(array('order_id'=>$return_goods['order_id']))->sum('goods_num');
	if($order_goods_count == $return_goods_count){
		$coupon_list = M('coupon_yxyp_list')->where(['uid'=>$return_goods['user_id'],'order_id'=>$return_goods['order_id']])->find();
		if(!empty($coupon_list)){
			$update_coupon_data = ['status'=>0,'use_time'=>0,'order_id'=>0];
			M('coupon_yxyp_list')->where(['id'=>$coupon_list['id'],'status'=>1])->save($update_coupon_data);//符合条件的，优惠券就退给他
		}
	}
	$expense_data = array('money'=>$return_goods['refund_money']+$return_goods['refund_deposit'],'log_type_id'=>$rec_id,'type'=>3,'user_id'=>$return_goods['user_id']);
	expenseLogYxyp($expense_data);//退款记录日志
}
...
function updateRefundOrder($order,$type=0){
    ...
    if ($order['user_money'] > 0 || $order['integral'] > 0) {
        $update_money_res = accountLog($order['user_id'], $order['user_money'], 0, $order['integral'], '用户申请订单退款', 0, 0, $order['order_id'], $order['order_sn']);
        if(!$update_money_res){
            return false;
        }
    }
    ...
    if($order['order_amount']>0 && $type == 1){
    	...
    	accountLog($order['user_id'],$order['order_amount'],0,0,'用户取消订单退款',0,0,$order['order_id'],$order['order_sn']);
    }
    //符合条件的，该笔订单使用的优惠券就退还
 ...
 * @return bool
 */
function updateRefundOrderRed($order,$type=0){
    //使用积分或者余额抵扣部分一一退还
    if ($order['user_money'] > 0 || $order['integral'] > 0) {
        $update_money_res = accountLog($order['user_id'], $order['user_money'],0, $order['integral'], '用户申请红包商城订单退款', 0,0, $order['order_id'], $order['order_sn']);
        if(!$update_money_res){
            return false;
        }
    }
    ...
    if($order['order_amount']>0 && $type == 1){
    	//改方法已经是退回余额, 不需要判断原路退回还是退还到余额
    	accountLog($order['user_id'],$order['order_amount'],0,0,'用户取消红包商城订单退款',0,0,$order['order_id'],$order['order_sn']);
    }
    if($order['midou'] > 0){
    	change_min_midou($order);
    }
    //符合条件的，该笔订单使用的优惠券就退还
    $coupon_list = M('coupon_red_list')->where(['uid'=>$order['user_id'],'order_id'=>$order['order_id']])->find();
    if(!empty($coupon_list)){
        $update_coupon_data = ['status'=>0,'use_time'=>0,'order_id'=>0];
        M('coupon_red_list')->where(['id'=>$coupon_list['id'],'status'=>1])->save($update_coupon_data);
    }
    ...
    expenseLogRed($expense_data);//平台支出记录
    return true;
}
...
/*
TK 2018年4月27日15:37:42
查询所有子公司*/
function get_company_list(){
	$company_list = S('company_list');
	if(empty($company_list)){
		$company_list = M('company')->where('parent_id','eq','0')->select();
		S('company_list',$company_list);
	}
	return $company_list;
}
 ...
 * 供货商名称
 */
function gonghuoshang($cid)
{
   
    $map = array();
    $map['suppliers_id'] = $cid;
    $shiti = M('Suppliers')->where($map)->find();
    return $shiti['suppliers_name'];
}
...
function shangpin($id)
{
    ...
    $map = array();
    $map['goods_id'] = $id;
    $shiti = M('GoodsRed')->where($map)->find();
    return $shiti['goods_name'];
}
...
function guige($id)
{
    $map = array();
    $map['item_id'] = $id;
    $shiti = M('SpecRedGoodsPrice')->where($map)->find();
    return $shiti['key_name'];
}
/**
 * 成员数量
 */
function chengyuan($id)
{
    $map = array();
    $map['parent_id'] = $id;
    $chengyuan = M('CompanyMember')->where($map)->count();
    return $chengyuan;
}
...
function yuangong($id)
{
    $map = array();
    $map['company_id'] = $id;
    $yuangong = M('staff')->where($map)->count();
    return $yuangong;
}
 ...
 * 商品名称
 */
function tuiguangyuan($id)
{
   
    $map = array();
    $map['id'] = $id;
    $shiti = M('staff')->where($map)->find();
    return $shiti['uname'];
}
...
function tgy_name($id)
{
    $map = array();
    $map['user_id'] = $id;
    $user = db('users u')->where($map)->join('staff staff', 'staff.id = u.staff_id')->getField('staff.real_name');
    return $user;
}


function tgy_money($tgy_map)
{
    $str= db('staff_commission')->where($tgy_map)->sum('money');
    return $str;
}

function staff_money($staff_money)
{
    $str = db('staff_commission')->where($staff_money)->sum('money');
    return $str;
}

function fh_store_money($store_map)
{
    $str = db('member_commission')->where($store_map)->sum('money');
    return $str;
}

function company_money($company_map)
{
    $str = db('member_commission')->where($company_map)->sum('money');
    return $str;
}

function storebili($company_id,$store_id)
{
    $str = M('company_sign')->where("company_id={$company_id} and store_id={$store_id}")->select();
    return $str;
}
function companybili($company_id)
{
    $str = M('company_sign')->where("company_id={$company_id} and store_id = 0")->select();
    return $str;
}

function profit_sum($company_level)
{
    $str = M('company_level')->where("id",$company_level)->value('profit');
    return $str;
}
function profitratio($store_id)
{
    $str = M('company')->field('proportion,tgyxxbl,yscbl')->find($store_id);
    return $str;
}