...
class SpecYxyp extends Model {
   ...
   /**
    ...
    public function afterSave($id)
    {
        ...
        $model = M("SpecYxypItem"); // 实例化User对象
        ...
        foreach($db_items as $key => $val)
        {
            ...
            {       
                ...
                M("SpecYxypGoodsPrice")->where("`key` REGEXP '^{$key}_' OR `key` REGEXP '_{$key}_' OR `key` REGEXP '_{$key}$' or `key` = '{$key}'")->delete(); // 删除规格项价格表
                M("SpecYxypItem")->where('id='.$key)->delete(); // 删除规格项
            }
        }        
    }    
}
