    ...
    public function afterSave($id)
    {
        ...
        foreach($db_items as $key => $val)
        {
            ...
            {       
                ...
                M("SpecRedGoodsPrice")->where("`key` REGEXP '^{$key}_' OR `key` REGEXP '_{$key}_' OR `key` REGEXP '_{$key}$' or `key` = '{$key}'")->delete(); // 删除规格项价格表
                M("SpecRedItem")->where('id='.$key)->delete(); // 删除规格项
            }
        }        
    }    
}
