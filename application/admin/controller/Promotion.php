...
class Promotion extends Base
{
    ...
    public function prom_goods_info()
    {
        ...
        if ($prom_id > 0) {
            ...
            $Goods = new Goods();
            $prom_goods = $Goods->with('SpecGoodsPrice')->where(['prom_id' => $info['id'], 'prom_type' => $info['type']])->select();
    ...
    public function prom_goods_save()
    {
        ...
        M("goods")->where(['prom_id' => $prom_id, 'prom_type' => 5])->save(array('prom_id' => 0, 'prom_type' => 0));
        M("goods")->where("goods_id", "in", $goods_ids)->save(array('prom_id' => $last_id, 'prom_type' => $prom_type));
        Db::name('spec_goods_price')->where(['prom_id' => $prom_id, 'prom_type' => $prom_type])->update(['prom_id' => 0, 'prom_type' => 0]);
...
//        }
     ...
     * 特别推荐商品列表
     */
    public function elite_goods_list()
    {
        $type = I('type',1);
        ...
        $where['type'] = ['eq',$type];
        $count = M('elite_goods')->alias('e')->where($where)
            ->join("{$table_name} g",'g.goods_id = e.goods_id','left')
            ->count();
        ...
        $list = M('elite_goods')
            ->alias('e')
            ->where($where)
            ->field('g.goods_id,g.goods_name,g.store_count,g.shop_price,g.is_check,g.is_on_sale,e.type,e.id,e.addtime')
            ->join("{$table_name} g",'g.goods_id = e.goods_id','left')
    ...
    {
        $prom_id = I('id/d');
        $prom_type = I('type/d',0);
        $Goods = new Goods();
        $prom_where = ['prom_id' => $prom_id,'prom_type'=>$prom_type];
    ...
    public function search_goods()
    {
        ...
        $where    = ['is_on_sale' => 1,'is_check' => 1, 'store_count' => ['gt', 0],'is_virtual'=>0,'exchange_integral'=>0];
    ...
    public function search_elite_goods()
    {
        $type = input('type',1);
        $goods_id = input('goods_id');
        $cat_id   = input('cat_id');
    ...
    public function do_save_elitegoods(){
        $goods_id = I('goods_id/d',0);
        $type = I('type/d',1);

        $result = M('elite_goods')->save(['goods_id'=>$goods_id,'type'=>$type,'addtime'=>time()]);
        if($result){
            $res['status'] = 1;
        }else{
            $res['status'] = 0;
    ...
    public function del_elitegoods(){
        $id = I('id/d',0);
        $result = M('elite_goods')->delete($id);
        if($result){
            $res['status'] = 1;
            $res['info'] = '操作成功';
        }else{
