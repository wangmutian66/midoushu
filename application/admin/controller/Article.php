...
class Article extends Base {
    ...
    public function top(){
        ...
        $t = I('GET.t');
        if($article_id > 0){
            $result = M('article_notice')->where('article_id='.$article_id)->setField('is_top',$t);
            if($result != false){
                $this->ajaxReturn(['status' => 1, 'msg' => '操作成功','data'=>$t]);
            }else{
    ...
    public function noticeHandle()
    {
        ...
        $result = $this->validate($data, 'Notice.'.$data['act'], [], true);
        if ($result !== true) {
            $this->ajaxReturn(['status' => 0, 'msg' => '参数错误', 'result' => $result]);
        }
        ...
        if ($data['act'] == 'add') {
            ...
            $r = M('article_notice')->where('article_id='.$data['article_id'])->save($data);
            adminLog('编辑公告');
        } elseif ($data['act'] == 'del') {
            $r = M('article_notice')->where('article_id='.$data['article_id'])->delete();
            adminLog('删除公告');
        }
