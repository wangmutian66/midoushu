 ...
 //   var $is_red;
    ...
    public function ajaxindex(){
        ...
        foreach ($orderList as $k => $val) {
            ...
            foreach ($orderGoods as $k2 => $val2) {
                ...
                $resname[$k2] = $val2['goods_name'];
            }

            // $res = Db::name('order_red_goods')->where('order_id='.$val2['order_id'])->field('goods_name')->limit(1)->select();
            // foreach ($orderGoods as $kc=>$row){
    ...
    public function virtual_cancel(){
        ...
        if(IS_POST){
            ...
            if($order){
                $r = M('order_red')->where(array('order_id'=>$order_id))->save(array('order_status'=>3,'admin_note'=>$admin_note));
                if($r){
                    $orderLogic = new RedOrderLogic();
                    $orderLogic->orderActionLog($order_id,$admin_note, '取消订单');
                    adminLog('取消米豆订单(order_id:'.$order_id.')');
    ...
    public function verify_code(){
        if(IS_POST){
            ...
            $vr_code_info = M('vr_red_order_code')->where(array('vr_code'=>$vr_code))->find();
            $order = M('order_red')->where(['order_id'=>$vr_code_info['order_id']])->field('order_status,order_sn,user_note')->find();
            ...
            M('vr_red_order_code')->where(array('vr_code'=>$vr_code))->save($update);
            //检查订单是否完成
            $condition = array();
            ...
            $vr_order = M('vr_red_order_code')->where($condition)->select();
            if(empty($vr_order)){
                $data['order_status'] = 2;  //此处不能直接为4，不然前台不能评论
                $data['confirm_time'] = time();
                M('order_red')->where(['order_id'=>$vr_code_info['order_id']])->save($data);
                M('order_red_goods')->where(['order_id'=>$vr_code_info['order_id']])->save(['is_send'=>1]);  //把订单状态改为已收货
            }
            $order_goods = M('order_red_goods')->where(['order_id'=>$vr_code_info['order_id']])->find();
            if($order_goods){
                $result = [
                    'vr_code'=>$vr_code,
    ...
    public function generateVirtualCode(){
        ...
        $vr_order_code = Db::name('vr_red_order_code')->where('order_id',$order_id)->find();
        if(!empty($vr_order_code)){
            adminLog('生成米豆区兑换码(order_id:'.$order_id.')');
    ...
    public function refund_order_list(){
        ...
        $show = $Page->show();
        $orderList = M('order_red')->where($condition)->limit($Page->firstRow.','.$Page->listRows)->order('add_time DESC')->select();
        $this->assign('orderList',$orderList);
        $this->assign('page',$show);// 赋值分页输出
        $this->assign('pager',$Page);
        return $this->fetch();
    ...
    public function export_refund_order_list(){
        ...
        $orderList = M('order_red')->where($condition)->order('add_time DESC')->select();
        ...
        $strTable .='</table>';
        unset($orderList);
        downloadExcel($strTable,'refund_order_list_red');
        adminLog('导出米豆订单列表');
        exit();
    }
    ...
    public function refund_order(){
        ...
        $total_fee = M('order_red')->where('transaction_id',$order['transaction_id'])->sum('order_amount');
		//
        if(!$order){
        ...
        if($data['pay_status'] == 3){
            if($data['refund_type'] == 1){
                //取消订单退款退余额
                if(updateRefundOrderRed($order,1)){
                    M('order_red')->where(array('order_id'=>$order['order_id']))->save($data);
            ...
            if($data['refund_type']== 0){
                ...
                if($order['pay_code'] == 'weixin' || $order['pay_code'] == 'weixinH5'  || $order['pay_code'] == 'alipay' || $order['pay_code'] == 'alipayMobile'){
                    ...
                    if($order['pay_code'] == 'weixin' || $order['pay_code'] == 'weixinH5' ){
                        ...
                        // dump($return_goods);die;
                        $result = $payment_obj->payment_refund($data);
                        if($result['return_code'] == 'SUCCESS'  && $result['result_code'] == 'SUCCESS'){
                            $data['pay_status']   = 3;
                            $data['order_status'] = 3;
                            $data['admin_note']   = I('admin_note/s');
                            M('order_red')->where(array('order_id'=>$order['order_id']))->save($data);//更改订单状态    
                            change_min_midou($order);
                            adminLog('米豆区订单退款(order_id:'.$order['order_id'].')');
                            $this->success('退款成功',U('/admin/OrderRed/refund_order_info/',['order_id'=>$order['order_id']]));
                        }else{
            ...
            M('order_red')->where(array('order_id'=>$order['order_id']))->save($data);
            adminLog('米豆区拒绝退款(order_id:'.$order['order_id'].')');
    ...
    public function detail($order_id){
        ...
        foreach ($orderGoods as $k => $val) {
            if( $val['is_z_back'] == 1) $midou_back_percent = tpCache('shoppingred.midou_back_percent');
            else $midou_back_percent = $val['midou_back_percent'];
            $goods_price = $val['midou_money']*$midou_back_percent/100;
            $md = $goods_price/tpCache('shoppingred.midou_rate');
            $val['back_midou'] = num_float2($md);
            $orderGoods[$k] = $val;
        }
        ...
        $return_list = db('return_red_goods')->where($return_where)->select_key('rec_id');
        ...
        if(IS_POST)
        {
            ...
            if($goods_id_arr){
                ...
                foreach($new_goods as $key => $val)
                {
                    ...
                    $rec_id = M('order_red_goods')->add($val);//订单添加商品
                    if(!$rec_id)
                        $this->error('添加失败');
                }
            }
            ...
            foreach ($orderGoods as $val){
                if(empty($old_goods[$val['rec_id']])){
                    M('order_red_goods')->where("rec_id=".$val['rec_id'])->delete();//删除商品
                }else{
                    //修改商品数量
                    if($old_goods[$val['rec_id']] != $val['goods_num']){
                        $val['goods_num'] = $old_goods[$val['rec_id']];
                        M('order_red_goods')->where("rec_id=".$val['rec_id'])->save(array('goods_num'=>$val['goods_num']));
                    }
                    $old_goods_arr[] = $val;
                }
            }
            ...
            $result   = calculate_price_red($order['user_id'],$goodsArr,$order['shipping_code'],0,$order['province'],$order['city'],$order['district'],0,0,0,0);
            if($result['status'] < 0)
            {
            ...
            $order['midou']          = $result['result']['order_midou'];       // 米豆
            $order['order_midou']    = $result['result']['order_midou'];       // 米豆
            $order['midou_money']    = $result['result']['order_midou_money']; // 现金部分
            $order['max_midou']      = $result['result']['order_max_midou'];   // 米豆
            $order['midou_rate']     = tpCache('shoppingred.midou_rate');      // 米豆兑换比
            $order['tk_cost_price']  = $result['result']['tk_cost_price'];
            $o = M('order_red')->where('order_id='.$order_id)->save($order);
            adminLog('操作米豆区订单(order_id:'.$order_id.')');
            $l = $orderLogic->orderActionLog($order_id,'修改订单','修改订单');//操作日志
            if($o && $l){
                $this->success('修改成功',U('Admin/OrderRed/editprice',array('order_id'=>$order_id)));
            }else{
                $this->success('修改失败',U('Admin/OrderRed/detail',array('order_id'=>$order_id)));
            }
            exit;
        }
        // 获取省份
...
//        exit();
    ...
    public function split_order(){
        ...
        if(IS_POST){
            ...
            $result = calculate_price_red($order['user_id'],$oldArr,$order['shipping_code'],0,$order['province'],$order['city'],$order['district'],0,0,0);
            if($result['status'] < 0)
            {
                $this->error($result['msg']);
            }
            ...
            M('order_red')->where("order_id=".$order_id)->save($res);
            //################################原单处理结束
            ...
            foreach($brr as $goods){
                $result = calculate_price_red($order['user_id'],$goods,$order['shipping_code'],0,$order['province'],$order['city'],$order['district'],0,0,0);
                if($result['status'] < 0)
                {
                    $this->error($result['msg']);
                }
                ...
                $new_order_id = DB::name('order_red')->insertGetId($new_order);//插入订单表
                foreach ($goods as $vv){
                    $vv['order_id'] = $new_order_id;
                    unset($vv['rec_id']);
                    $nid = M('order_red_goods')->add($vv);//插入订单商品表
                }
            }
            //################################新单处理结束
            $this->success('操作成功',U('Admin/OrderRed/detail',array('order_id'=>$order_id)));
            exit;
        }
    ...
    public function editprice($order_id){
        ...
        if(IS_POST){
            ...
            $update['dismidou']       = I('post.dismidou');
            $update['dismidou_money'] = I('post.dismidou_money');
            $update['shipping_price'] = I('post.shipping_price');
            $update['midou']          = $order['order_midou'] + $update['dismidou'];
            $update['order_amount']   = $order['midou_money'] + $update['shipping_price'] - $update['dismidou_money'] - $order['user_money'] - $order['integral_money'] - $order['coupon_price'];
            $row = M('order_red')->where(array('order_id'=>$order_id))->save($update);
            if(!$row){
                adminLog('米豆区价钱修改');
                $this->success('没有更新数据',U('Admin/OrderRed/editprice',array('order_id'=>$order_id)));
            }else{
                $this->success('操作成功',U('Admin/OrderRed/detail',array('order_id'=>$order_id)));
            }
            exit;
    ...
    public function delete_order(){
        ...
        $orderLogic = new RedOrderLogic();
        $order_sn= M('order_red')->where('order_id',$order_id)->getField('order_sn');
    ...
    public function deliveryHandle(){
        ...
        if($res){
            adminLog('米豆区生成发货单(order_id:'.$data['order_id'].')');
            $this->success('操作成功',U('Admin/OrderRed/delivery_info',array('order_id'=>$data['order_id'])));
        }else{
            $this->success('操作失败',U('Admin/OrderRed/delivery_info',array('order_id'=>$data['order_id'])));
        }
    }
    ...
    public function delivery_info(){
        ...
        $delivery_record = M('delivery_red_doc')->alias('d')->where('d.order_id='.$order_id)->select();
        if($delivery_record){
    ...
    */
    public function ajaxdelivery(){
        ...
        $show = $Page->show();
        $orderList = M('order_red')->where($condition)->limit($Page->firstRow.','.$Page->listRows)->order('add_time DESC')->select();
   //     echo M('order_red')->getlastsql();die;
        $this->assign('orderList',$orderList);
        $this->assign('page',$show);// 赋值分页输出
        $this->assign('pager',$Page);
        return $this->fetch();
    ...
    public function export_delivery(){
        ...
        $strTable .='</table>';
        unset($orderList);
        downloadExcel($strTable,'delivery_red');
        adminLog('导出米豆区订单列表');
        exit();
    }
    ...
    public function return_del(){
        $id = I('get.id');
        M('return_red_goods')->where("id = $id")->delete();
        adminLog('删除米豆区退换货申请(id:'.$id.')');
        $this->success('成功删除!');
    }
    ...
    public function return_info()
    {
        $orderLogic   = new RedOrderLogic();
        $return_id    = I('id');
        $return_goods = M('return_red_goods')->where("id = $return_id")->find();

        $order_goods_num = M('order_red_goods')->where("order_id=".$return_goods['order_id'])->sum('goods_num');
        ...
        if(IS_POST)
        {
            ...
            if($data['refund_midou'] > $return_goods['midou'])
                $this->error('退款金额不可大于最大退回米豆');
            ...
            $result = M('return_red_goods')->where("id= $return_id")->save($data);
            if($result && $data['status']==1)
            {
                $order_data_status['order_status'] = 7;
                $edit_order_status = M('order_red')->where("order_id=".$return_goods['order_id'])->save($order_data_status);
                // $order_data_pay['pay_status'] = 5;
                // $edit_order_status = M('order_red')->where("order_id=".$return_goods['order_id'])->save($order_data_pay);
                #老张 修改  判断  获取订单商品总数
                $order_goods_num = M('order_red_goods')->where("order_id=".$return_goods['order_id'])->sum('goods_num');
                $return_goods_num = M('return_red_goods')->where("order_id=".$return_goods['order_id']." AND status = 3")->sum('goods_num');

                if($return_goods_num >= $order_goods_num){
                    $order_data['order_status'] = 6;
                    M('order_red')->where("order_id=".$return_goods['order_id'])->save($order_data); // 更新订单状态
                }
                

                //审核通过才更改订单商品状态，进行退货，退款时要改对应商品修改库存
                $order = \app\common\model\OrderRed::get($return_goods['order_id']);
                $commonOrderLogic = new \app\common\logic\RedOrderLogic();
            ...
            adminLog('米豆区退换货(order_id:'.$return_goods['order_id'].')');
            $this->success('修改成功!');
            exit;
        }
        ...
        // dump($return_goods);die;
        $order = M('order_red')->where(array('order_id'=>$return_goods['order_id']))->find();
    ...
    public function refund_back(){
        $return_id = I('id');
        $return_goods = M('return_red_goods')->where("id= $return_id")->find();
        $rec_goods = M('order_red_goods')->where(array('order_id'=>$return_goods['order_id'],'goods_id'=>$return_goods['goods_id']))->find();
        $order = M('order_red')->where(array('order_id'=>$rec_goods['order_id']))->find();
        $total_fee = M('order_red')->where('transaction_id',$order['transaction_id'])->sum('order_amount');
        ...
        if($order['pay_code'] == 'weixin'  || $order['pay_code'] == 'weixinH5' || $order['pay_code'] == 'alipay' || $order['pay_code'] == 'alipayMobile'){
            ...
            if($order['pay_code'] == 'weixin' || $order['pay_code'] == 'weixinH5' ){
                ...
                if($result['return_code'] == 'SUCCESS'  && $result['result_code'] == 'SUCCESS'){
                     $order_data_pay['pay_status'] = 5;
                     $edit_order_status = M('order_red')->where("order_id=".$return_goods['order_id'])->save($order_data_pay);
                    updateRefundGoodsRed($return_goods['rec_id']);//订单商品售后退款
                    // adminLog('米豆区退款(rec_id:'.$rec_goods['order_id'].')');
                    adminLog('米豆区退款');
                    $this->success('退款成功');
                }else{
                ...
                change_min_midou($order);
                accountLog($order['user_id'], $return_goods['refund_deposit'], $return_goods['refund_midou'], $return_goods['refund_integral'], $desc = '退款', $midou_all = 0, $distribut_money = 0, $order['order_id'], $order['order_sn']);
                ...
                $result = $payment_obj->payment_red_refund($data);
                if($result['msg'] == 'Success'){
                    $order_data_pay['pay_status'] = 5;
                    $edit_order_status = M('order_red')->where("order_id=".$return_goods['order_id'])->save($order_data_pay);
                    adminLog('米豆区退款');
    ...
    public function refund_balance(){
        $data = I('POST.'); 
        $return_goods = M('return_red_goods')->where(array('rec_id'=>$data['rec_id']))->find();
        if(empty($return_goods)) $this->ajaxReturn(['status'=>0,'msg'=>"参数有误"]); 
        $order_data_pay['pay_status'] = 5;
        $edit_order_status = M('order_red')->where("order_id=".$return_goods['order_id'])->save($order_data_pay);
        M('return_red_goods')->where(array('rec_id'=>$data['rec_id']))->save($data);
        updateRefundGoodsRed($data['rec_id'],1);//售后商品退款
        tk_midou_log($return_goods['user_id'],$return_goods['refund_midou'],'退款返回米豆',$return_goods['order_id'],$return_goods['order_sn']);
        $this->ajaxReturn(['status'=>1,'msg'=>"操作成功",'url'=>U("Admin/OrderRed/return_list")]);
    }
    ...
    public function add_return_goods()
   {
            ...
            $return_goods = M('return_red_goods')->where("order_id = $order_id and goods_id = $goods_id")->find();
            if(!empty($return_goods))
            {
                $this->error('已经提交过退货申请!',U('Admin/OrderRed/return_list'));
                exit;
            }
            $order = M('order_red')->where("order_id = $order_id")->find();
            ...
            M('return_red_goods')->add($data);
            adminLog('生成米豆区申请退货单');
            $this->success('申请成功,现在去处理退货',U('Admin/OrderRed/return_list'));
            exit;
    }
    ...
    public function order_action(){     
        ...
        if($action && $order_id){
            ...
            if($res !== false && $a !== false){
                if ($action == 'remove') {
                    exit(json_encode(array('status' => 1, 'msg' => '操作成功', 'data' => array('url' => U('Admin/OrderRed/index')))));
                }
                exit(json_encode(array('status' => 1,'msg' => '操作成功')));
            }else{
                if ($action == 'remove') {
                    exit(json_encode(array('status' => 0, 'msg' => '操作失败', 'data' => array('url' => U('Admin/OrderRed/index')))));
                }
                exit(json_encode(array('status' => 0,'msg' => '操作失败')));
            }
        }else{
            $this->error('参数错误',U('Admin/OrderRed/detail',array('order_id'=>$order_id)));
        }
    }
    ...
    public function order_log(){
     ...
     //   $condition['is_red'] = ['eq',$this->is_red];
        ...
        if ($orderIds) {
            $users = M("users")->alias('u')->join('__ORDER_RED__ o', 'o.user_id = u.user_id')->getField('o.order_id,u.nickname');
        }
    ...
    public function export_order_log(){
        ...
        $strTable .='</table>';
        unset($orderList);
        downloadExcel($strTable,'order_log_red');
        adminLog('导出米豆区订单日志');
        exit();
    }
    ...
    public function return_percentage_fileput()
    {
        ...
        $list = M('order_red')->where($where)->alias('a')
            ->field('a.*,b.goods_num,b.goods_price,b.member_goods_price,b.rec_id,b.goods_id,b.goods_sn,b.spec_key_name,c.goods_name,c.cat_id,c.tax_rate,c.is_z_back,c.midou_back_percent,d.name d_name,e.suppliers_name,e.suppliers_id e_suppliers_id')
            ->join('order_red_goods b', 'a.order_id = b.order_id', 'left')
            ->join('goods_red c', 'b.goods_id = c.goods_id', 'left')
            ->join('goods_red_category d', 'c.cat_id = d.id', 'left')
            ->join('suppliers e', 'a.suppliers_id = e.suppliers_id', 'left')
            ->order("a.order_id desc")->page("$p,$size")->select();
        $count = M('order_red')->where($where)->alias('a')
            ->field('a.*,b.goods_num,b.goods_price,b.member_goods_price,b.rec_id,b.goods_id,b.goods_sn,b.spec_key_name,c.goods_name,c.cat_id,c.tax_rate,c.is_z_back,c.midou_back_percent,d.name d_name,e.suppliers_name,e.suppliers_id e_suppliers_id')
            ->join('order_red_goods b', 'a.order_id = b.order_id', 'left')
            ->join('goods_red c', 'b.goods_id = c.goods_id', 'left')
            ->join('goods_red_category d', 'c.cat_id = d.id', 'left')
        ...
        $dir_url = "./public/orderRed/data_" . $user_id . "/";
        if (!is_dir($dir_url)) {
    ...
    public function export_order()
    {
        ...
        $strTable .= '<td style="text-align:center;font-size:12px;width:120px;">订单编号</td>';
		$strTable .= '<td style="text-align:center;font-size:12px;width:120px;">线上/线下</td>';
		$strTable .= '<td style="text-align:center;font-size:12px;width:120px;">实体店</td>';
        ...
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">商品总价格</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">赠送米豆</td>';
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">商品实际总价</td>';
        ...
        if(is_array($orderList)){
            ...
            foreach($orderList as $k=>$val){
                ...
                $goods_price = $val['midou_money']*$midou_back_percent/100;
                $md = $goods_price/tpCache('shoppingred.midou_rate');
                $back_midou += $val['goods_num']*$md; // 订单赠送米豆累计
                $val['back_midou'] = num_float2($val['back_midou']);
				$isOnline = ($val['is_store'] == 1)? '线下' : '线上';#线上/线下
                $store_name = $val['store_id'] > 0 ? shitis($val['store_id']) : "";
                $strTable .= '<tr>';
                $strTable .= '<td style="text-align:center;font-size:12px;">&nbsp;'.$n.'</td>';
                $strTable .= '<td style="text-align:center;font-size:12px;">&nbsp;'.$val['order_id'].'</td>';
                $strTable .= '<td style="text-align:center;font-size:12px;">&nbsp;'.$val['order_sn'].'</td>';
				$strTable .= '<td style="text-align:center;font-size:12px;">&nbsp;'.$isOnline.'</td>';
				$strTable .= '<td style="text-align:center;font-size:12px;">&nbsp;'.$store_name.'</td>';
                ...
                $strTable .= '<td style="text-align:left;font-size:12px;">'.$val['shipping_price'].'</td>';
                $strTable .= '<td style="text-align:left;font-size:12px;">'.$val['midou'].'+￥'.$val['midou_money'].'</td>';
                $strTable .= '<td style="text-align:left;font-size:12px;">'.$val['back_midou'].'</td>';
                ...
                $goods_price = $goods['midou_money']*$midou_back_percent/100;
                $md = $goods_price/tpCache('shoppingred.midou_rate');
        ...
        $strTable .='</table>';
        downloadExcel($strTable, '米豆商品订单导出');
   ...
   public function removeDir($dirName)
   {
    ...
    public function ajax_return_list(){
        ...
        $show = $Page->show();
        $list = M('return_red_goods')->where($where)->order("$order_by $sort_order")->limit("{$Page->firstRow},{$Page->listRows}")->select();
        $goods_id_arr = get_arr_column($list, 'goods_id');
        if(!empty($goods_id_arr)){
            $goods_list = M('goods_red')->where("goods_id in (".implode(',', $goods_id_arr).")")->getField('goods_id,goods_name');
        }
    ...
    public function export_return_list(){
        ...
        $strTable .='</table>';
        unset($orderList);
        downloadExcel($strTable,'return_list_red');
        exit();
    }
    ...
    public function add_order()
    {
        ...
        if(IS_POST)
        {
            ...
            foreach ($order_goods_arr as $key => $value) {
                ...
                $result = calculate_price_red($order['user_id'],$value,$order['shipping_code'],0,$order['province'],$order['city'],$order['district'],0,0,0,0,$midou[$key], $midou_money[$key], $max_midou[$key]);  //获取订单总额
                if($result['status'] < 0)   
                {
                     $this->error($result['msg']);      
                }
                ...
                if($px > 1){
                    $order['order_sn']   = 'midou'.date('YmdHis').mt_rand(1000,9999); // 订单编号;
                    $order['parent_sn']  = $parent_sn;                    // 父单单号 
                }    
               
                // 添加订单
                $order_id = M('order_red')->add($order);
                ...
                if($order_id)
                {
                    ...
                    {
                        $val['order_id'] = $order_id;
                        $rec_id = M('order_red_goods')->add($val);
                        if(!$rec_id)
                            $this->error('添加失败');                                  
                    }
      
                    M('order_red_action')->add([
                        'order_id'      => $order_id,
            ...
            $this->success('添加商品成功',U("Admin/OrderRed/detail",array('order_id'=>$order_insert_id)));
            exit();           
        } 
    ...
    public function search_goods()
    {
        ...
        $brandList =  M("brand_red")->select();
        $categoryList =  M("goods_red_category")->select();
        ...
        if(I('cat_id')){
            ...
            $grandson_ids = getCatGrandsonRed(I('cat_id')); 
        ...
        $goods_count =M('goods_red')->where($where)->count();
        $Page = new Page($goods_count,C('PAGESIZE'));
        $goodsList = M('goods_red')->where($where)->order('goods_id DESC')->limit($Page->firstRow,$Page->listRows)->select();
                
        foreach($goodsList as $key => $val)
        {   
            // 米豆换算
            $midouInfo                      = getMidou($val['goods_id']);
            $goodsList[$key]['goods_midou']       = $midouInfo['midou'];
            $goodsList[$key]['goods_midou_money'] = $midouInfo['midou_money']; 

            $spec_goods = M('spec_red_goods_price')->where("goods_id = {$val['goods_id']}")->select();
            foreach ($spec_goods as $k => $v) {
                $midouInfo                     = getMidou($val['goods_id'],$v['item_id']);
                $spec_goods[$k]['midou']       = $midouInfo['midou'];
                $spec_goods[$k]['midou_money'] = $midouInfo['midou_money']; 
            }
    ...
    public function ajaxOrderNotice(){
        $order_amount = M('order_red')->where("order_status=0 and (pay_status=1 or pay_code='cod')")->count();
        echo $order_amount;
    }
    ...
    public function batchDelivery(){
        ...
        if(IS_POST){
            ...
            foreach ($arr as $k=>$row){
                $order = db('order_red')->where(["order_id"=>$row['order_id']])->field("shipping_status,shipping_name")->find();
                $order_goods = db('order_red_goods')->where(["order_id"=>$row['order_id']])->column('rec_id');
    ...
    public function syncDelivery(){
        ...
        $flag = false;
        $doc_list = db('delivery_red_doc')->field('order_id,shipping_code,invoice_no')->select();

        $red_log = db('delivery_red_log');
        foreach($doc_list as $value){
            $rst = $red_log->where(['order_id'=>$value['order_id']])->find();

            if(!$rst){

                $kdata['order_id'] = $value['order_id'];
                $kdata['shipping_code'] = $value['shipping_code'];
                $kdata['invoice_no'] = $value['invoice_no'];
                $kdata['addtime'] = time();
                M('delivery_red_log')->save($kdata);
