...
class Withdrawals extends Base {
        ...
        if($export == 1){
            ...
            if(is_array($remittanceList)){
                foreach($remittanceList as $k=>$val){
                    ...
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.($val['money'] * $val['service_charge'] / 100).' </td>';
    ...
    //    dump($where);die;
        foreach ($list as $key => $value) {
            $list[$key]['service_charge']   =   $value['money'] * $value['service_charge'] / 100;
        }
    ...
    public function withdrawals_update_member(){
        ...
        if($status < 0){
            ...
            foreach ($log_list as $key => $value) {
                $total = bcadd($value['money'],$value['taxfee'],9);
                if($member_list[$value['member_id']]['frozen'] < $value['money']){
                    $error_msg .= "成员：{$value['real_name']} 提款记录ID：{$value['id']} 失败，可用冻结余额不足<br>";
                }
                member_accountLog($value['member_id'], $total,"管理员拒绝提现申请");
                $save_ids[] =   $value['id'];
                $member_money_list[] = ['id'=>$value['member_id'],
                                        'frozen'=>['exp',"frozen - {$total}"],
                                        'money'=>['exp',"money + {$total}"]
                                    ];
                \Think\cache::rm("member_{$value['member_id']}");
            }
    ...
    public function transfer(){
        ...
        foreach($log_list as $val){
            ...
            $money = db('staff_balance')->where('phone',$staff_info['phone'])->value('frozen'); //30
            $total = bcadd($val['money'],$val['taxfee'],9); // 总
            ...
            if($val['status'] < 2){
                ...
                if($money < $total)
                {
                    ...
                    $this->error('账户冻结余额不足，请联系程序员查询操作记录');
                }else{
                    ...
                    if($atype == 'online'){
                        ...
                        db('staff_balance')->where('phone',$staff_info['phone'])->update($update_data);

                        //db('staff_balance')->where('phone',$staff_info['phone'])->setDec('balance',$total);
    ...
    public function transfer_member(){
        ...
        foreach($log_list as $val){
      #      dump($val);die;
            $member_info = M('company_member')
                ->alias('member')
                ->field('member.*,level.service_charge')
                ->join('company_level level','level.id = member.company_level','left')
                ->where('member.id','eq',$val['member_id'])->find();
            $money = db('member_balance')->where('phone',$member_info['phone'])->value('frozen');
            $total = bcadd($val['money'],$val['taxfee'],9);
            if($money < $total)
            {
             //   $data = array('status'=>-2,'remark'=>'账户冻结余额不足');
            //    M('member_withdrawals')->where("id = {$val['id']}")->update($data);
                $this->error('提现人：'.$val['real_name'] .'账户冻结余额不足');
            }else{
                ...
                if($atype == 'online'){
                    ...
                    /*
                    $update_data = array(
                        'frozen' => ['exp','frozen +'.($money * -1)],
                    );
                    $update = M('company_member')->cache("member_{$val['member_id']}")->where("id = {$val['member_id']}")->update($update_data);
                       */
                    $update_data = array(
                        'frozen' => ['exp','frozen +'.($total * -1)],
                    );
                    db('member_balance')->where('phone',$member_info['phone'])->update($update_data);
