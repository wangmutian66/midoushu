...
class PollRed extends Base
{

    public function index()
    {
        $poll = db("delivery_red_log");
        ...
        $count = $poll
            ->alias('poll')
            ->join('order_red o','o.order_id=poll.order_id','left')
    ...
    {
        $id = I('id/d',0);
        $data = db('delivery_red_log')
            ->alias('poll')
            ->field('poll.*,o.order_sn')
            ->join('order_red o','o.order_id=poll.order_id')
            ->find($id);
    ...
    {
        $id = I('id/d',0);
        $data = db('delivery_red_log')->find($id);
        ...
        $post_data["param"] = '{"company":"'.$shipping_code.'","number":"'.$data['invoice_no'].'","from":"","to":"","key":"'.$invoice_key.'","parameters":{"callbackurl":"'.$callbackurl.'?red=1&orderid='.$data['order_id'].'","salt":"'.$salt.'","resultv2":"1"}}';
        ...
        db('delivery_red_log')->where("id=$id")->update(array('shipping_code'=>$shipping_code,'subtime'=>time(),'orderstatus'=>$kuaidi['returnCode'],'ordermessage'=>$kuaidi['message']));
        $arr = array('errno'=>0);
