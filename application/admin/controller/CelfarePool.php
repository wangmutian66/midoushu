...
class CelfarePool extends Base {

  //  public  $mod;
    
    public function _initialize() {
        parent::_initialize();        
     //   $this->mod = db('celfare_pool');
    }
    
    public function index(){
        $item = M('celfare_pool')->where('id=1')->field('c.*,a.user_name admin_name')->alias('c')->join('__ADMIN__ a ','c.admin_id = a.admin_id','left')->find();  // 查询资金管理池
    ...
    /*资金池冲全返*/
    function out_back(){
        $money = M('celfare_pool')->where('id=1')->getField('money');
      #  dump($money);die;
    ...
    public function log_list(){
        ...
        $count = M('celfare_pool_log')->where($where)->count(); // 查询满足要求的总记录数
    ...
    function modify(){
        ...
        $celfare_data['money'] =   ['exp',"money + {$money}"];
        $log_data['admin_id']   =   $celfare_data['admin_id']   =   $_SESSION['admin_id'];
        $log_data['create_time']    =  $celfare_data['update_time']    =   NOW_TIME;
        ...
        if(empty($log_data['remark'])){
            ...
            if($log_data['admin_id'] == 1){
                $tims = NOW_TIME;
                $r = db('celfare_pool')->where('id=1')->find();
                if(($money < 0) && abs($money) > $r['money']){
                    $msg['status']  =   0;
                    $msg['info']    =   '公益池资金不足!';
                }else{
                    db('celfare_pool')->where('id = 1')->save($celfare_data);
                    $log_data['act']    =   'modify';
                    $log_data['control']    =   'admin';
                    db('celfare_pool_log')->add($log_data);
                    $msg['status']  =   1;
                }
            }else{
                $msg['status']  =   0;
                $msg['info']    =   '公益池只有超级管理员能修改，您无权限修改!';
            }
        }
        $this->ajaxReturn($msg);
    }
    ...
    function do_back(){
        ...
        $celfare_moeny = M('celfare_pool')->where('id=1')->getField('money');  
       /* $res['status']  =   1;
   ...
   #     $total_ratio = explode('|',tpCache('proportion.total_ratio'));
    ...
    #    $back_money = bcmul($money,$total_ratio[1],9);
        ...
        $previous_order_list = M('order_yxyp')->field('order_id,user_id,order_amount,shipping_price,already_rebate,confirm_time')->where("confirm_time < ".NOW_TIME." and order_status in(2,4) and already_rebate < (order_amount - shipping_price)")->order('confirm_time asc')->select();
       
        // dump(M('order_yxyp')->getlastsql());
        // die;
...
/*        dump($back_proportion);
      ...
      /*  dump($order_update_rebate);
        ...
        if($order_update_rebate){
            model("OrderYxyp")->saveAll($order_update_rebate);
        }
  

        $celfare_data['money'] =   ['exp',"money - {$total_money}"];
        $log_data['admin_id']   =   $celfare_data['admin_id']   =   $_SESSION['admin_id'];
        $log_data['create_time']    =  $celfare_data['update_time']    =   NOW_TIME;
        db('celfare_pool')->where('id = 1')->update($celfare_data);
