...
class Plugin extends Base {
    ...
    public function index(){
        $plugin_list = M('plugin')->where('suppliers_id = 0')->select();
    ...
    public function shipping_default(){
        if(IS_GET){
            ...
            $code = I('code');
            $data['is_default'] = 0; 
            $r = M('plugin')->where(array('type'=>'shipping','suppliers_id'=>0))->save($data);
            $data2['is_default'] = 1;
            $r2 = M('plugin')->where(array('code'=>$code,'type'=>'shipping','suppliers_id'=>0))->save($data2);
            if($r2 !== false){
                adminLog('设置默认物流');
    ...
    public function add_shipping()
    {
        ...
        $name = I('name');     // 物流名字
        $desc = I('desc', ''); // 插件描述
        $suppliers_id = I('suppliers_id') ? I('suppliers_id') : '';
