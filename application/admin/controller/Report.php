...
class Report extends Base{
	...
	public function index(){
		...
		$today['today_order']  = M('order')->where("add_time>$now and (pay_status=1 or pay_code='cod')")->count(); //今日支付订单数
        ...
        $res = Db::name("order".$select_year)
            ->field(" COUNT(*) as tnum, sum(order_amount) as amount, sum(shipping_price) as shipping_amount,sum(tk_cost_price) as cost_amount,sum(tk_cost_operating) as cost_operating, sum(goods_price) as goods_amount, FROM_UNIXTIME(add_time,'%Y-%m-%d') as gap ")
            ->where($where)
            ->group('gap')
            ->select();
		...
		for($i=$this->begin;$i<=$this->end;$i=$i+24*3600){
            ...
            $tmp_shipping_amount = empty($grr[date('Y-m-d',$i)]) ? 0 : $grr[date('Y-m-d',$i)];
			...
			$sign_arr[]           = $tmp_sign;
			$date                 = date('Y-m-d',$i);
			$list[]               = array('day'=>$date,'order_num'=>$tmp_num,'amount'=>$tmp_amount,'cost_amount'=>$tmp_cost_amount,'cost_operating'=>$tmp_cost_operating,'goods_amount'=>$tmp_goods_amount,'goods_lr'=>$tmp_goods_lr,'shipping_amount'=>$tmp_shipping_amount, 'sign'=>$tmp_sign,'end'=>date('Y-m-d',$i+24*60*60));
			$day[]              = $date;
		}

		rsort($list);  // 对数组 $list 中的元素按字母进行降序排序
		$this->assign('list',$list);
		$result = array('order'=>$order_arr,'amount'=>$amount_arr,'sign'=>$sign_arr,'time'=>$day,'cost_amount'=>$cost_amount_arr,'cost_operating'=>$cost_operating_arr,'goods_amount'=>$goods_amount_arr,'goods_lr'=>$goods_lr_arr,'shipping_amount'=>$shipping_amount_arr);
	...
	public function saleTop(){
		$sql = "select rec_id,goods_name,goods_sn,sum(goods_num) as sale_num,sum(goods_num*goods_price) as sale_amount,sum(goods_num*cost_price) as cost_amount,sum(goods_num*cost_operating) as operating_amount,sum(goods_num*goods_price-goods_num*cost_price-goods_num*cost_operating) as lr_amount from __PREFIX__order_goods ";
		$sql .=" where is_send = 1";
		// liyi 2018.04.18
		$sp = I('sp','','intval');
        if($sp && $sp == 1){
    ...
    public function export_saleTop(){
        $sql = "select goods_name,goods_sn,sum(goods_num) as sale_num,sum(goods_num*goods_price) as sale_amount,sum(goods_num*cost_price) as cost_amount,sum(goods_num*cost_operating) as operating_amount,sum(goods_num*goods_price-goods_num*cost_price-goods_num*cost_operating) as lr_amount from __PREFIX__order_goods ";
        $sql .=" where is_send = 1";
        // liyi 2018.04.18
        $sp = I('sp','','intval');
        if($sp && $sp == 1){
	...
	public function supplierTop(){
		$suppliers_phone = I('mobile');
		$suppliers_name  = I('email');
        $order_where = [
		...
		if($suppliers_phone){
			$suppliers_where['suppliers_phone'] =array('like',"%$suppliers_phone%");
		}		
		if($suppliers_name){
            $suppliers_where['suppliers_name'] = array('like',"%$suppliers_name%");
		}
        ...
        $count = Db::name('order')->alias('o')->where($order_where)->group('o.suppliers_id')->count();  //统计数量
    ...
    public function saleOrder(){
        ...
        $order_list  = Db::name('order')->alias('o')
            ->field('o.order_id,o.order_sn,o.goods_price,o.shipping_price,o.total_amount,o.order_amount,o.tk_cost_price,o.tk_cost_operating,o.add_time,u.user_id,u.nickname')
    ...
    public function export_saleOrder(){
        ...
        $strTable .='</table>';
        unset($order_list);
        downloadExcel($strTable,'saleOrder');
        exit();
    }
    ...
    public function rechargeOrder(){
        $end_time    = $this->begin+24*60*60;
        $order_where = "o.ctime>$this->begin and o.ctime<$end_time";  //交易成功的有效订单
        $order_count = Db::name('recharge')->alias('o')->where($order_where)->whereIn('pay_status','1')->count();
        $Page        = new Page($order_count,20);
        $order_list  = Db::name('recharge')->alias('o')
            ->field('o.order_id,o.order_sn,o.account,o.ctime,u.user_id,u.nickname,o.pay_name,o.pay_code')
            ->join('users u','u.user_id = o.user_id','left')
            ->where($order_where)->whereIn('pay_status','1')
            ->limit($Page->firstRow,$Page->listRows)->select();
        $this->assign('order_list',$order_list);
        $this->assign('page',$Page->show());
        return $this->fetch();
    ...
    public function export_rechargeOrder(){
        $end_time    = $this->begin+24*60*60;
        $order_where = "o.ctime>$this->begin and o.ctime<$end_time";  //交易成功的有效订单
        ...
        $strTable .='</table>';
        unset($order_list);
        downloadExcel($strTable,'rechargeOrder');
        exit();
    }
	...
	public function saleList(){
        ...
        $res = Db::name('order_goods')->alias('og')->field('og.*,o.order_id,o.order_sn,o.shipping_name,o.pay_name,o.add_time')
            ->join('order o','og.order_id=o.order_id ','left')
            ->join('goods g','og.goods_id = g.goods_id','left')
        ...
        foreach ($res as $k=>$row){
            $res[$k]['price']=0;
            if($row['goods_price'] != $row['member_goods_price']){
                $res[$k]['price'] = $row['member_goods_price'];
            }

            #2018-10-26 销售明细中将已退的商品数量减掉
            $rg = M('return_goods')->field('goods_num')->where("order_id=".$row['order_id']." and goods_id=".$row['goods_id']." and status=3")->find();
            if($rg){
                $res[$k]['goods_num'] -= $rg['goods_num'];
            }


//             $this->getFlashSale($row['goods_id'],$row['order_id']);
        }
     ...
     * @date 2018-10-08
     */
	public function getFlashSale($goodid,$orderid){
        $FlashSale = new FlashSale();
        $add_time = db('order')->where(['order_id'=>$orderid])->value("add_time");
        $condition['start_time'] = ['lt',$add_time];
        $condition['end_time'] = ['gt',$add_time];
        $condition['goods_id'] = $goodid;
        $price = $FlashSale->where($condition)->value('price');
        return $price;
	}
    ...
    public function export_saleList(){
        ...
        $order_list = Db::name('order_goods')->alias('og')->field('og.*,o.order_sn,o.shipping_name,o.pay_name,o.add_time, g.tax_rate')
            ->join('order o','og.order_id=o.order_id ','left')
            ->join('goods g','og.goods_id = g.goods_id','left')
            ->where($where)
            ->order('o.add_time desc')->select();
        ...
        foreach ($order_list as $k=>$row){
            ...
            $rg = M('return_goods')->field('goods_num')->where("order_id=".$row['order_id']." and goods_id=".$row['goods_id']." and status=3")->find();
            if($rg){
                $order_list[$k]['goods_num'] -= $rg['goods_num'];
            }
		...
		$strTable .= '<td style="text-align:center;font-size:12px;" width="*">赠送米豆数</td>';
        $strTable .= '</tr>';
        if(is_array($order_list)){
            $FlashSale = new FlashSale();
            foreach($order_list as $k=>$val){
//                $price = $this->getFlashSale($val['goods_id'],$val['order_id']);
//                $condition['start_time'] = ['lt',$val['add_time']];
//                $condition['end_time'] = ['gt',$val['add_time']];
//                $condition['goods_id'] = $val['goods_id'];
//                $price = $FlashSale->where($condition)->value('price');
                $price = "";
                if($val['goods_price'] != $val['member_goods_price']){
                ...
                if($price!=""){
                    $strTable .= '<span style="color: red;">'.$price.'</span>&nbsp;<span style="text-decoration: line-through;">'.$val['goods_price'].'</span>';
                }else{
                ...
                if($price!=""){
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.($price*$val['goods_num']).'</td>';
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.($price*$val['goods_num'] - $val['cost_price']*$val['goods_num'] - $val['cost_operating']*$val['goods_num']).'</td>';
                }else{
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.($val['goods_price']*$val['goods_num']).'</td>';
                    $strTable .= '<td style="text-align:left;font-size:12px;">'.($val['goods_price']*$val['goods_num'] - $val['cost_price']*$val['goods_num'] - $val['cost_operating']*$val['goods_num']).'</td>';
                }


                $strTable .= '<td style="text-align:left;font-size:12px;">'.date('Y-m-d H:i:s',$val['add_time']).'</td>';
				$strTable .= '<td style="text-align:left;font-size:12px;">'.$val['tax_rate'].'</td>';
				$midouTotal = returnMidou($val['goods_id']);
				if(empty($midouTotal)) {$midouTotal = array('midou'=>0);}
				$strTable .= '<td style="text-align:left;font-size:12px;">'.$midouTotal['midou'].'</td>';
                $strTable .= '</tr>';
            }
        }
        $strTable .='</table>';
	...
	public function finance(){
		$sql = "SELECT sum(a.order_amount) as amount,sum(b.goods_num*b.member_goods_price) as goods_amount,sum(a.shipping_price) as shipping_amount,sum(a.tk_cost_price) as cost_price,sum(a.tk_cost_operating) as cost_operating,";
		...
		foreach ($res as $val){
            ...
            //$frr[$val['gap']] = $val['amount'] - $val['shipping_amount'] - $val['cost_price'] - $val['cost_operating'];
            $cb = $val['shipping_amount'] + $val['cost_price'] + $val['cost_operating'];
            $frr[$val['gap']] = bcsub($val['amount'],$cb,2);
            $grr[$val['gap']] = $val['amount'];
		}
		...
		for($i=$this->begin;$i<=$this->end;$i=$i+24*3600){
            ...
            $tmp_amount          = empty($grr[$date]) ? 0 : $grr[$date];
			...
			$list[] = array('day'=>$date,'goods_amount'=>$tmp_goods_amount,'cost_amount'=>$tmp_cost_amount,'operating_amount'=>$tmp_operating_amount,
					'shipping_amount'=>$tmp_shipping_amount,'coupon_amount'=>$tmp_coupon_amount,'amount'=>$tmp_amount,'lr_amount'=>$tmp_lr_amount,'end'=>date('Y-m-d',$i+24*60*60));
		}
        rsort($list);
		$this->assign('list',$list);
		$result = array('goods_arr'=>$goods_arr,'cost_arr'=>$cost_arr,'operating_arr'=>$operating_arr,'shipping_arr'=>$shipping_arr,'coupon_arr'=>$coupon_arr,'lr_arr'=>$lr_arr,'amount_arr'=>$amount_arr,'time'=>$day);
		$this->assign('result',json_encode($result));
		return $this->fetch();
	}
  ...
  /**
    ...
    public function financeDetail(){
        ...
        $order_list = Db::name('order')->alias('o')
            ->field('o.order_id,o.order_sn,o.order_prom_amount,o.coupon_price,o.goods_price,o.tk_cost_price,o.tk_cost_operating,o.shipping_price,o.total_amount,o.order_amount,o.add_time,u.user_id,u.nickname')
	...
	**/
	public function export_goodsList(){
        ...
        $order_list = Db::name('order_goods')->alias('og')->field('og.*,o.order_sn,o.shipping_name,o.pay_name,o.add_time, g.tax_rate, g.cat_id, g.is_goods_report ')
            ->join('order o','og.order_id=o.order_id ','left')
            ->join('goods g','og.goods_id = g.goods_id','left')
            ->where($where)
        ...
        if(is_array($order_list)){
            ...
            foreach($order_list as $k=>$val){
				...
				M('goods')->data($dataUp)->where("`goods_id`='{$val['goods_id']}'")->save();
				$catName = M('goods_category')->where("id=".$val['cat_id'])->getField('name');
