...
class System extends Base
{
	...
	{
        $param = I('post.');
        if($param['lockPassword'] != ''){
            $param['lockPassword'] = md5(I('post.lockPassword'));
        }else{
            $where['name'] = 'lockPassword';
            $where['inc_type'] = 'basic';
            $config = M('config')->where($where)->find();
            $param['lockPassword'] = $config['value'];
        }
...
/*	public function send_email(){
    ...
    {
        /*** 随机清空购物车的垃圾数据*/                     
        $time = NOW_TIME - 3600; // 删除购物车数据  1小时以前的
        M("Cart")->where("user_id = 0 and  add_time < $time")->delete();  
        M("CartRed")->where("user_id = 0 and  add_time < $time")->delete();            
        $today_time = time();
		
		// 删除 cart表垃圾数据 删除一个月以前的 
		$time = NOW_TIME - 2592000; 
        M("cart")->where("add_time < $time")->delete();		
        M("cart_red")->where("add_time < $time")->delete();
		...
		$time = NOW_TIME - $auto_confirm_date; // 比如7天以前的可用自动确认收货
        ...
        $order_id_red = M('order_red')->where("order_status = 1 and shipping_status = 1 and shipping_time < $time")->getField('order_id',true);       
        foreach($order_id_red as $k => $v)
        {
            confirm_order_red($v);     
        }
    ...
    function right_list(){
     	$group = array('system'=>'系统设置','content'=>'内容管理','goods'=>'商品中心','goodsred'=>'米豆商品中心','member'=>'会员中心','suppliers'=>'供货商中心','finance'=>'财务管理',
     			'order'=>'订单中心','orderred'=>'米豆订单中心','marketing'=>'营销推广','tools'=>'插件工具','count'=>'统计报表','countred'=>'米豆统计报表','company'=>'子公司');
    ...
    public function edit_right(){
     	...
     	$group = array('system'=>'系统设置','content'=>'内容管理','goods'=>'商品中心','goodsred'=>'米豆商品中心','member'=>'会员中心','suppliers'=>'供货商中心','finance'=>'财务管理',
     			'order'=>'订单中心','orderred'=>'米豆订单中心','marketing'=>'营销推广','tools'=>'插件工具','count'=>'统计报表','countred'=>'米豆统计报表','company'=>'子公司','chat'=>'聊天系统','yxyp'=>'一乡一品');
