...
use app\admin\logic\YxypOrderLogic;
use think\AjaxPage;
use think\Page;
use think\Db;
use think\Request;

class OrderYxyp extends Base {
    public  $order_status;
    public  $pay_status;
 ...
 //   var $is_red;
    ...
    public function ajaxindex(){
        ...
        $count = M('order_yxyp')->where($condition)->count();
        ...
        foreach ($orderList as $k => $val) {
            ...
            $res = Db::name('order_yxyp_goods')->where('order_id='.$val['order_id'])->field('goods_num,goods_price,member_goods_price,rec_id,goods_name')->select();
    ...
    public function virtual_cancel(){
        ...
        if(IS_POST){
            ...
            $order = M('order_yxyp')->where(array('order_id'=>$order_id))->find();
            if($order){
                $r = M('order_yxyp')->where(array('order_id'=>$order_id))->save(array('order_status'=>3,'admin_note'=>$admin_note));
                if($r){
                    $orderLogic = new YxypOrderLogic();
    ...
    public function verify_code(){
        if(IS_POST){
            ...
            $vr_code_info = M('vr_yxyp_order_code')->where(array('vr_code'=>$vr_code))->find();
            $order = M('order_yxyp')->where(['order_id'=>$vr_code_info['order_id']])->field('order_status,order_sn,user_note')->find();
            ...
            M('vr_yxyp_order_code')->where(array('vr_code'=>$vr_code))->save($update);
            //检查订单是否完成
            $condition = array();
            ...
            $vr_order = M('vr_yxyp_order_code')->where($condition)->select();
            if(empty($vr_order)){
                $data['order_status'] = 2;  //此处不能直接为4，不然前台不能评论
                $data['confirm_time'] = time();
                M('order_yxyp')->where(['order_id'=>$vr_code_info['order_id']])->save($data);
                M('order_yxyp_goods')->where(['order_id'=>$vr_code_info['order_id']])->save(['is_send'=>1]);  //把订单状态改为已收货
            }
            $order_goods = M('order_yxyp_goods')->where(['order_id'=>$vr_code_info['order_id']])->find();
            if($order_goods){
                $result = [
                    'vr_code'=>$vr_code,
    ...
    public function generateVirtualCode(){
        ...
        $vr_order_code = Db::name('vr_yxyp_order_code')->where('order_id',$order_id)->find();
    ...
    public function refund_order_list(){
        ...
        $condition['order_status'] = array('gt',1);
        ...
        $show = $Page->show();
        $orderList = M('order_yxyp')->where($condition)->limit($Page->firstRow.','.$Page->listRows)->order('add_time DESC')->select();
        $this->assign('orderList',$orderList);
        $this->assign('page',$show);// 赋值分页输出
        $this->assign('pager',$Page);
        return $this->fetch();
    ...
    public function export_refund_order_list(){
        ...
        $orderList = M('order_yxyp')->where($condition)->order('add_time DESC')->select();
    ...
    public function refund_order(){
        ...
        $total_fee = M('order_yxyp')->where('transaction_id',$order['transaction_id'])->sum('order_amount');
		//
        if(!$order){
        ...
        if($data['pay_status'] == 3){
            if($data['refund_type'] == 1){
                ...
                if(updateRefundOrderRed($order,1)){
                    M('order_yxyp')->where(array('order_id'=>$order['order_id']))->save($data);
                ...
                if($order['pay_code'] == 'weixin' || $order['pay_code'] == 'weixinH5'  || $order['pay_code'] == 'alipay' || $order['pay_code'] == 'alipayMobile'){
                    ...
                    if($order['pay_code'] == 'weixin' || $order['pay_code'] == 'weixinH5' ){
                        ...
                        if($result['return_code'] == 'SUCCESS'  && $result['result_code'] == 'SUCCESS'){
                            ...
                            M('order_yxyp')->where(array('order_id'=>$order['order_id']))->save($data);//更改订单状态    
                            change_min_midou($order);
                            adminLog('一乡一品订单退款');
                            $this->success('退款成功',U('/admin/OrderYxyp/refund_order_info/',['order_id'=>$order['order_id']]));
                        }else{
    ...
    public function detail($order_id){
        ...
        // 获取操作记录
        $action_log = M('order_yxyp_action')->where(array('order_id'=>$order_id))->order('log_time desc')->select();
        $has_user   = false;
        $adminIds   = [];

        //拒绝发货记录
        $refuse_info = M('order_yxyp_refuse')->where(array('order_id'=>$order_id,'refuse_status'=>1))->find();
        $this->assign('refuse',$refuse_info);

        $refuse_no_info = M('order_yxyp_refuse')->where(array('order_id'=>$order_id,'refuse_status'=>3))->find();
        ...
        $return_list = db('return_yxyp_goods')->where($return_where)->select_key('rec_id');
        ...
        if(IS_POST)
        {
            ...
            if($goods_id_arr){
                ...
                foreach($new_goods as $key => $val)
                {
                    ...
                    $rec_id = M('order_yxyp_goods')->add($val);//订单添加商品
                    if(!$rec_id)
                        $this->error('添加失败');
                }
            }
            ...
            foreach ($orderGoods as $val){
                if(empty($old_goods[$val['rec_id']])){
                    M('order_yxyp_goods')->where("rec_id=".$val['rec_id'])->delete();//删除商品
                }else{
                    //修改商品数量
                    if($old_goods[$val['rec_id']] != $val['goods_num']){
                        $val['goods_num'] = $old_goods[$val['rec_id']];
                        M('order_yxyp_goods')->where("rec_id=".$val['rec_id'])->save(array('goods_num'=>$val['goods_num']));
                    }
                    $old_goods_arr[] = $val;
                }
            }
            ...
            $result   = calculate_price_yxyp($order['user_id'],$goodsArr,$order['shipping_code'],0,$order['province'],$order['city'],$order['district'],0,0,0,0);
            if($result['status'] < 0)
            {
            ...
            $o = M('order_yxyp')->where('order_id='.$order_id)->save($order);

            $l = $orderLogic->orderActionLog($order_id,'修改订单','修改订单');//操作日志
            if($o && $l){
                adminLog('修改一乡一品订单');
                $this->success('修改成功',U('Admin/OrderYxyp/editprice',array('order_id'=>$order_id)));
            }else{
                $this->success('修改失败',U('Admin/OrderYxyp/detail',array('order_id'=>$order_id)));
            }
            exit;
        }
        // 获取省份
    ...
    public function split_order(){
        ...
        if(IS_POST){
            ...
            $result = calculate_price_yxyp($order['user_id'],$oldArr,$order['shipping_code'],0,$order['province'],$order['city'],$order['district'],0,0,0);
            if($result['status'] < 0)
            {
                $this->error($result['msg']);
            }
            ...
            M('order_yxyp')->where("order_id=".$order_id)->save($res);
            //################################原单处理结束
            ...
            foreach($brr as $goods){
                $result = calculate_price_yxyp($order['user_id'],$goods,$order['shipping_code'],0,$order['province'],$order['city'],$order['district'],0,0,0);
                if($result['status'] < 0)
                {
                    $this->error($result['msg']);
                }
                ...
                $new_order_id = DB::name('order_yxyp')->insertGetId($new_order);//插入订单表
                foreach ($goods as $vv){
                    $vv['order_id'] = $new_order_id;
                    unset($vv['rec_id']);
                    $nid = M('order_yxyp_goods')->add($vv);//插入订单商品表
                }
            }
            //################################新单处理结束
            $this->success('操作成功',U('Admin/OrderYxyp/detail',array('order_id'=>$order_id)));
            exit;
        }
    ...
    public function editprice($order_id){
        ...
        if(IS_POST){
            ...
            $row = M('order_yxyp')->where(array('order_id'=>$order_id))->save($update);
            if(!$row){
                adminLog('修改一乡一品订单价格');
                $this->success('没有更新数据',U('Admin/OrderYxyp/editprice',array('order_id'=>$order_id)));
            }else{
                $this->success('操作成功',U('Admin/OrderYxyp/detail',array('order_id'=>$order_id)));
            }
            exit;
    ...
    public function deliveryHandle(){
        ...
        if($res){
            adminLog('生成一乡一品发货单');
            $this->success('操作成功',U('Admin/OrderYxyp/delivery_info',array('order_id'=>$data['order_id'])));
        }else{
            $this->success('操作失败',U('Admin/OrderYxyp/delivery_info',array('order_id'=>$data['order_id'])));
        }
    }
    ...
    public function delivery_info(){
        ...
        $delivery_record = M('delivery_yxyp_doc')->alias('d')->where('d.order_id='.$order_id)->select();
        if($delivery_record){
    ...
    */
    ...
    public function return_del(){
        $id = I('get.id');
        M('return_yxyp_goods')->where("id = $id")->delete();
        adminLog('删除一乡一品退换货申请');
        $this->success('成功删除!');
    }
    ...
    public function return_info()
    {
        $orderLogic   = new YxypOrderLogic();
        $return_id    = I('id');
        $return_goods = M('return_yxyp_goods')->where("id = $return_id")->find();

        $order_goods_num = M('order_yxyp_goods')->where("order_id=".$return_goods['order_id'])->sum('goods_num');
        ...
        $goods      = M('goods_yxyp')->where("goods_id = {$return_goods[goods_id]}")->find();
        ...
        if(IS_POST)
        {
            ...
            $result = M('return_yxyp_goods')->where("id= $return_id")->save($data);
            if($result && $data['status']==1)
            {
                $return_goods_num = M('return_yxyp_goods')->where("order_id=".$return_goods['order_id']." AND status = 3")->sum('goods_num');

                if($return_goods_num >= $return_goods_num){
                    $order_data['order_status'] = 6;
                    M('order_yxyp')->where("order_id=".$return_goods['order_id'])->save($order_data); // 更新订单状态
                }

                //审核通过才更改订单商品状态，进行退货，退款时要改对应商品修改库存
                $order = \app\common\model\OrderYxyp::get($return_goods['order_id']);
                $commonOrderLogic = new \app\common\logic\YxypOrderLogic();
        ...
        // dump($return_goods);die;
        $order = M('order_yxyp')->where(array('order_id'=>$return_goods['order_id']))->find();
    ...
    public function refund_back(){
        $return_id = I('id');
        $return_goods = M('return_yxyp_goods')->where("id= $return_id")->find();
        $rec_goods = M('order_yxyp_goods')->where(array('order_id'=>$return_goods['order_id'],'goods_id'=>$return_goods['goods_id']))->find();
        $order = M('order_yxyp')->where(array('order_id'=>$rec_goods['order_id']))->find();
        $total_fee = M('order_yxyp')->where('transaction_id',$order['transaction_id'])->sum('order_amount');
        ...
        if($order['pay_code'] == 'weixin'  || $order['pay_code'] == 'weixinH5' || $order['pay_code'] == 'alipay' || $order['pay_code'] == 'alipayMobile'){
            ...
            if($order['pay_code'] == 'weixin' || $order['pay_code'] == 'weixinH5' ){
                ...
                $result = $payment_obj->payment_refund($data);
                if($result['return_code'] == 'SUCCESS'  && $result['result_code'] == 'SUCCESS'){
                    updateRefundGoodsYxyp($return_goods['rec_id']);//订单商品售后退款
                    adminLog('一乡一品退款');
                    $this->success('退款成功');
                }else{
                ...
                $result = $payment_obj->payment_yxyp_refund($data);
                if($result['msg'] == 'Success'){
                    adminLog('一乡一品退款');
                    $this->success('退款成功');
                } else {
    ...
    public function refund_balance(){
        $data = I('POST.'); 
        $return_goods = M('return_yxyp_goods')->where(array('rec_id'=>$data['rec_id']))->find();
        if(empty($return_goods)) $this->ajaxReturn(['status'=>0,'msg'=>"参数有误"]); 
        M('return_yxyp_goods')->where(array('rec_id'=>$data['rec_id']))->save($data);
        updateRefundGoodsYxyp($data['rec_id'],1);//售后商品退款
        adminLog('一乡一品退款返回米豆');
        tk_midou_log($return_goods['user_id'],$return_goods['refund_midou'],'退款返回米豆',$return_goods['order_id'],$return_goods['order_sn']);
        $this->ajaxReturn(['status'=>1,'msg'=>"操作成功",'url'=>U("Admin/OrderYxyp/return_list")]);
    }
    ...
    public function add_return_goods()
   {
            ...
            $return_goods = M('return_yxyp_goods')->where("order_id = $order_id and goods_id = $goods_id")->find();
            if(!empty($return_goods))
            {
                $this->error('已经提交过退货申请!',U('Admin/OrderYxyp/return_list'));
                exit;
            }
            $order = M('order_yxyp')->where("order_id = $order_id")->find();
            ...
            M('return_yxyp_goods')->add($data);
            adminLog('申请一乡一品退换货');
            $this->success('申请成功,现在去处理退货',U('Admin/OrderYxyp/return_list'));
            exit;
    }
    ...
    public function order_action(){     
        ...
        if($action && $order_id){
            ...
            if($res !== false && $a !== false){
                if ($action == 'remove') {
                    exit(json_encode(array('status' => 1, 'msg' => '操作成功', 'data' => array('url' => U('Admin/OrderYxyp/index')))));
                }
                adminLog('操作一乡一品订单');
                exit(json_encode(array('status' => 1,'msg' => '操作成功')));
            }else{
                if ($action == 'remove') {
                    exit(json_encode(array('status' => 0, 'msg' => '操作失败', 'data' => array('url' => U('Admin/OrderYxyp/index')))));
                }
                exit(json_encode(array('status' => 0,'msg' => '操作失败')));
            }
        }else{
            $this->error('参数错误',U('Admin/OrderYxyp/detail',array('order_id'=>$order_id)));
        }
    }
    ...
    public function order_log(){
     ...
     //   $condition['is_red'] = ['eq',$this->is_red];
        ...
        if ($orderIds) {
            $users = M("users")->alias('u')->join('__ORDER_YXYP__ o', 'o.user_id = u.user_id')->getField('o.order_id,u.nickname');
        }
    ...
    public function export_order()
    {
        ...
        I('pay_status1')     != '' ? $where['pay_status']   = I('pay_status1')  : false;
        ...
        I('user_id1')      ? $where['user_id']      = trim(I('user_id1')) : false;
        ...
        $orderList = Db::name('order_yxyp')->field("*,FROM_UNIXTIME(add_time,'%Y-%m-%d') as create_time")->where($where)->order('order_id')->select();
        $strTable ='<table width="500" border="1">';
        $strTable .= '<tr>';
        ...
        if(is_array($orderList)){
            ...
            foreach($orderList as $k=>$val){
                ...
                foreach ($orderGoods as $k2 => $val2) {
                    ...
                    $md = $goods_price/tpCache('shoppingred.midou_rate');
                    $val['back_midou'] += $val2['goods_num']*$md; // 订单赠送米豆累计
                }
                ...
                $orderGoods = D('order_yxyp_goods')->where('order_id='.$val['order_id'])->select();
                $strGoods="";
                $goods_num = 0;
                foreach($orderGoods as $goods){

                    if( $goods['is_z_back'] == 1) $midou_back_percent = tpCache('shoppingred.midou_back_percent');
                    else $midou_back_percent = $goods['midou_back_percent'];
                    $goods_price = $goods['midou_money']*$midou_back_percent/100;
                    $md = $goods_price/tpCache('shoppingred.midou_rate');
                    $goods['back_midou'] = num_float2($md);

                    $goods_num = $goods_num + $goods['goods_num'];
                    $strGoods .= "商品编号：".$goods['goods_sn']." 商品名称：".$goods['goods_name'];
                    if ($goods['spec_key_name'] != '') $strGoods .= " 规格：".$goods['spec_key_name'];
                    $strGoods .= " 赠送米豆：".$goods['back_midou'];
                    $strGoods .= "<br />";
                }
                unset($orderGoods);
        ...
        $strTable .='</table>';
        unset($orderList);
        downloadExcel($strTable,'orderRed');
        adminLog('导出一乡一品订单');
        exit();
    }
    ...
    public function ajax_return_list(){
        ...
        $count = M('return_yxyp_goods')->where($where)->count();
        $Page  = new AjaxPage($count,13);
        $show = $Page->show();
        $list = M('return_yxyp_goods')->where($where)->order("$order_by $sort_order")->limit("{$Page->firstRow},{$Page->listRows}")->select();
    ...
    public function export_return_list(){
        ...
        $goods_id_arr = get_arr_column($list, 'goods_id');
        if(!empty($goods_id_arr)){
            $goods_list = M('goods_yxyp')->where("goods_id in (".implode(',', $goods_id_arr).")")->getField('goods_id,goods_name');
        }
    ...
    public function add_order()
    {
        ...
        if(IS_POST)
        {
            ...
            foreach ($order_goods_arr as $key => $value) {
                ...
                $result = calculate_price_yxyp($order['user_id'],$value,$order['shipping_code'],0,$order['province'],$order['city'],$order['district'],0,0,0,0,$midou[$key], $midou_money[$key], $max_midou[$key]);  //获取订单总额
                if($result['status'] < 0)   
                {
                     $this->error($result['msg']);      
                }
                ...
                // 添加订单
                $order_id = M('order_yxyp')->add($order);
                ...
                if($order_id)
                {
                    ...
                    {
                        $val['order_id'] = $order_id;
                        $rec_id = M('order_yxyp_goods')->add($val);
                        if(!$rec_id)
                            $this->error('添加失败');                                  
                    }
      
                    M('order_yxyp_action')->add([
                        'order_id'      => $order_id,
            ...
            adminLog('添加一乡一品商品');
            $this->success('添加商品成功',U("Admin/OrderYxyp/detail",array('order_id'=>$order_insert_id)));
            exit();           
        } 
    ...
    public function search_goods()
    {
        ...
        $brandList =  M("brand_yxyp")->select();
        $categoryList =  M("goods_yxyp_category")->select();
        ...
        $goods_count =M('goods_yxyp')->where($where)->count();
        $Page = new Page($goods_count,C('PAGESIZE'));
        $goodsList = M('goods_yxyp')->where($where)->order('goods_id DESC')->limit($Page->firstRow,$Page->listRows)->select();
        ...
        {   
            ...
            $spec_goods = M('spec_yxyp_goods_price')->where("goods_id = {$val['goods_id']}")->select();
    ...
    public function ajaxOrderNotice(){
        $order_amount = M('order_yxyp')->where("order_status=0 and (pay_status=1 or pay_code='cod')")->count();
        echo $order_amount;
    }
    ...
    public function batchDelivery(){
        ...
        if(IS_POST){
            ...
            foreach ($arr as $k=>$row){
                $order = db('order_yxyp')->where(["order_id"=>$row['order_id']])->field("shipping_status,shipping_name")->find();
                $order_goods = db('order_yxyp_goods')->where(["order_id"=>$row['order_id']])->column('rec_id');
