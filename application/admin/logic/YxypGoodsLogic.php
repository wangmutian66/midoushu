...
class YxypGoodsLogic extends Model
{
    ...
    public function goods_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
                ...
                $sql = "SELECT * FROM  __PREFIX__goods_yxyp_category ORDER BY parent_id , sort_order ASC";
    ...
    {            
        $GoodsCategory = M("GoodsYxypCategory"); // 实例化User对象
        $cat = $GoodsCategory->where("id = $id")->find(); // 找出他自己
        // 刚新增的分类先把它的值重置一下
        if($cat['parent_id_path'] == '')
        {
            ($cat['parent_id'] == 0) && Db::execute("UPDATE __PREFIX__goods_yxyp_category set  parent_id_path = '0_$id', level = 1 where id = $id"); // 如果是一级分类               
            Db::execute("UPDATE __PREFIX__goods_yxyp_category AS a ,__PREFIX__goods_yxyp_category AS b SET a.parent_id_path = CONCAT_WS('_',b.parent_id_path,'$id'),a.level = (b.level+1) WHERE a.parent_id=b.id AND a.id = $id");                
        ...
        Db::execute("UPDATE `__PREFIX__goods_yxyp_category` SET parent_id_path = REPLACE(parent_id_path,'{$cat['parent_id_path']}','$replace_str'), level = (level - $replace_level) WHERE  parent_id_path LIKE '{$cat['parent_id_path']}%'");        
    }    
    ...
    public function getAttrInput($goods_id,$type_id)
    {
        ...
        $GoodsAttribute = D('GoodsYxypAttribute');
    ...
    {     
        ...
        foreach ($spec_arr_sort as $key =>$val)
        {
         ...
         $spec = M('SpecYxyp')->getField('id,name'); // 规格表
         $specItem = M('SpecYxypItem')->getField('id,item,spec_id');//规格项
         $keySpecGoodsPrice = M('SpecYxypGoodsPrice')->where('goods_id = '.$goods_id)->getField('key,key_name,price,cost_price,cost_operating,store_count,bar_code,sku');//规格项
    ...
    function GetSpecCheckboxList($type_id, $checked = array()){
        $list = M('SpecYxyp')->where("type_id = $type_id")->order('`order` desc')->select();        
    ...
    function GetAttrCheckboxList($type_id, $checked = array()){                
        $list = M('GoodsYxypAttribute')->where("type_id = $type_id and attr_index > 0 ")->order('`order` desc')->select();        
        //$list = M('Spec')->where("1=1")->order('`order` desc')->select();        
        $str = '';
    ...
    function find_parent_cat($cat_id)
    {
        ...
        $cat_list =  M('goods_yxyp_category')->getField('id,parent_id,level');
    ...
    function getSortBrands()
    {
        //关闭功能
        return false;
        $brandList = S('getSortBrandsYxyp');
        if(!empty($brandList)){
            return $brandList;
        }
        $brandList =  M("BrandYxyp")->cache(true)->select();
        $brandIdArr =  M("BrandYxyp")->cache(true)->where("name in (select `name` from `".C('database.prefix')."brand` group by name having COUNT(id) > 1)")->getField('id,cat_id');
        $goodsCategoryArr = M('goodsYxypCategory')->cache(true)->where("level = 1")->getField('id,name');
        $nameList = array();
    ...
    {
        $categoryList = S('categoryListYxyp');
        if($categoryList)
        {
            return $categoryList;
        }
        $categoryList =  M("GoodsYxypCategory")->cache(true)->getField('id,name,parent_id,level');
        $nameList = array();
        ...
        S('categoryListYxyp',$categoryList);
        return $categoryList;
    }
    ...
    function savePreSell($data)
    {
        ...
        if (array_key_exists('shut_down', $data) && $data['shut_down'] == 1) {
            if (!empty($data['id'])) {
                $pre_sell_shut_down = M('goods_yxyp_activity')->where(array('act_id' => $data['id']))->save(array('end_time' => time()));
        ...
        $goods_main = M('goods_yxyp')->where(array('goods_id'=>$data['goods_id']))->find();
        ...
        if (empty($data['id'])) {
            ...
            $is_have_goods = M('goods_yxyp_activity')->where($is_have_where)->find();
            if($is_have_goods){
        ...
        if(!empty($data['id']) && !empty($data['goods_id'])){
            $goods_activity = M('goods_yxyp_activity')->where(array('act_id'=>$data['id']))->find();
        ...
        if (empty($data['id'])) {
            ...
            M('goods_yxyp')->where(array('goods_id'=>$data['goods_id']))->save($goods_save_data);
            $r = M('goods_yxyp_activity')->add($add_data);
            ...
            $goods_save_data['prom_type'] = 4;//预售商品类型
            M('goods_yxyp')->where(array('goods_id'=>$data['goods_id']))->save($goods_save_data);
            $r = M('goods_yxyp_activity')->where(array('act_id' => $data['id'], 'act_type' => 1))->save($save_data);
            if ($r !== false) {
