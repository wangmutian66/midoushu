...
class GoodsLogic extends Model
{
    ...
    {     
        ...
        foreach ($spec_arr_sort as $key =>$val)
        {
         ...
         $keySpecGoodsPrice = M('SpecGoodsPrice')->where('goods_id = '.$goods_id)->getField('key,key_name,price,cost_price,cost_operating,store_count,bar_code,sku');//规格项
    ...
    {
        $categoryList = S('newcategoryList_'.$parent_id);
        if($categoryList)
        {
            return $categoryList;
        }
        $categoryList =  M("GoodsCategory")->where(['parent_id'=>$parent_id])->order('parent_id,sort_order ASC')->getField('id,name,parent_id,level');

        S('newcategoryList_'.$parent_id,$categoryList);
        return $categoryList;
    }
    ...
    function getSuppliers()
    {
        ...
        foreach($suppliersList as $k => $v)
        {
            ...
            $name = getFirstCharter(mb_substr($v['suppliers_name'],0,1,'utf-8')) .' '. $v['suppliers_name']; // 前面加上拼音首字母
            $str = "";
            switch ($v['is_check']) {
                case '0':
                    $str = "(未审核)";
