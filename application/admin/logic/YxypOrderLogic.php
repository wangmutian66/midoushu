...
use think\Model;
use think\Db;
class YxypOrderLogic extends Model
{
    /**
    ...
    public function getOrderList($condition,$order='',$start=0,$page_size=20){
        $res = M('order_yxyp')->where($condition)->limit("$start,$page_size")->order($order)->select();
        return $res;
    }
    ...
    public function getOrderGoods($order_id,$is_send =''){
        ...
        $sql = "SELECT g.*,o.*,(o.goods_num * o.member_goods_price) AS goods_total FROM __PREFIX__order_yxyp_goods o ".
    ...
    public function get_spec_goods($goods_id_arr){
		...
		{
			$arr   = array();
			$goods = M('goods_yxyp')->where("goods_id = $key")->find();
			...
			foreach($val as $k => $v)
			{
				...
				{
					$arr['spec_key']           = $k;
					$spec_goods                = M('spec_yxyp_goods_price')->where("goods_id = $key and `key` = '{$k}'")->find();
    ...
    public function orderActionLog($order_id,$action,$note=''){
        ...
        return M('order_yxyp_action')->add($data);//订单操作记录
    }
    ...
    public function orderRefuseLog($order_id,$suppliers_id=0,$action,$refuse_status,$note=''){
        ...
        return M('order_yxyp_refuse')->add($data);//订单操作记录
    }
    ...
    public function getGoodsAmount($order_id){
        $sql = "SELECT SUM(goods_num * goods_price) AS goods_amount FROM __PREFIX__order_yxyp_goods WHERE order_id = {$order_id}";
        $res = DB::query($sql);
...
//        /* 选择一个随机的方案 */send_http_status('310');
    ...
    public function orderProcessHandle($order_id,$act,$ext=array()){
    	...
    	switch ($act){
    		case 'pay': //付款
               	$order_sn = M('order_yxyp')->where("order_id = $order_id")->getField("order_sn");
    	...
    	return M('order_yxyp')->where("order_id=$order_id")->save($updata);//改变订单状态
    }
    ...
    {
    	//如果这笔订单已经取消付款过了
    	$count = M('order_yxyp')->where("order_id = $order_id and pay_status = 1")->count();   // 看看有没已经处理过这笔订单  支付宝返回不重复处理操作
    	if($count == 0) return false;
    	// 找出对应的订单
    	$order = M('order_yxyp')->where("order_id = $order_id")->find();
    	// 增加对应商品的库存
        $orderGoodsArr = M('OrderYxypGoods')->where("order_id = $order_id")->select();
    	foreach($orderGoodsArr as $key => $val)
    	{
    		...
    		{   // 先到规格表里面增加数量 再重新刷新一个 这件商品的总数量
				...
				$specGoodsPrice = null;
    			M('goods_yxyp')->where("goods_id = {$val['goods_id']}")->setInc('store_count',$val['goods_num']); // 增加商品总数量
    		}
    		M('goods_yxyp')->where("goods_id = {$val['goods_id']}")->setDec('sales_sum',$val['goods_num']); // 减少商品销售量
			//更新活动商品购买量
    	...
    	M('order_yxyp')->where("order_id=$order_id")->save(array('pay_status'=>0));
    ...
    public function deliveryHandle($data){
		...
		$did = M('delivery_yxyp_doc')->add($data);
		$is_delivery = 0;
		foreach ($orderGoods as $k=>$v){
			...
			if($v['is_send'] == 0 && in_array($v['rec_id'],$selectgoods)){
				$res['is_send'] = 1;
				$res['delivery_id'] = $did;
				$r = M('order_yxyp_goods')->where("rec_id=".$v['rec_id'])->save($res);//改变订单商品发货状态
				$is_delivery++;
			}
		}
		...
		M('order_yxyp')->where("order_id=".$data['order_id'])->save($updata);//改变订单状态
    ...
    public function updateOrderShipping($data=[],$order=[]){
        $updata['shipping_code'] = $data['shipping_code'];
        $updata['shipping_name'] = $data['shipping_name'];
        M('order_yxyp')->where(['order_id'=>$data['order_id']])->save($updata); //改变物流信息
        $updata['invoice_no'] = $data['invoice_no'];
        $updata['note']       = $data['note'];
        $delivery_res = M('delivery_yxyp_doc')->where(['order_id'=>$data['order_id']])->save($updata);  //改变售后的信息
        if ($delivery_res){
    ...
    function delOrder($order_id){
    	...
    	$del_order = M('order_yxyp')->where(array('order_id'=>$order_id))->delete();
    	$del_order_goods = M('order_yxyp_goods')->where(array('order_id'=>$order_id))->delete();
	...
	{
		$order_goods_list  = Db::name('order_yxyp_goods')->where(['order_id' => $order_id])->select();
		...
		if ($order_goods_count == $order_goods_return_count) {
			 $res = Db::name('order_yxyp')->where(['order_id' => $order_id])->update(['order_status' => 5]);
            if(!$res){
                return false;
            }
		}
    ...
    public function disposereRurnOrderCoupon($return_info){
        $coupon_list = M('coupon_yxyp_list')->where(['uid'=>$return_info['user_id'],'order_id'=>$return_info['order_id']])->find();    //有没有关于这个商品的优惠券
        ...
        //追回赠送优惠券,一般退款才会走这里
        $coupon_info = M('coupon_yxyp_list')->where(['uid'=>$return_info['user_id'],'get_order_id'=>$return_info['order_id']])->find();
        if(!empty($coupon_info)){
            if($coupon_info['status'] == 1) { //如果优惠券被使用,那么从退款里扣
                ...
                if ($return_info['refund_money'] > $coupon['money']) {
                    ...
                    M('return_yxyp_goods')->where(['id' => $return_info['id']])->save(['refund_money' => $return_info['refund_money']]);
                }else{
                    $return_info['refund_deposit'] = $return_info['refund_deposit'] - $coupon['money'];
                    M('return_yxyp_goods')->where(['id' => $return_info['id']])->save(['refund_deposit' => $return_info['refund_deposit']]);
                }
            }else {
                M('coupon_yxyp_list')->where(array('id' => $coupon_info['id']))->delete();
                M('coupon_yxyp')->where(array('id' => $coupon_info['cid']))->setDec('send_num');
            }
        }
    }
    ...
    public function getRefundGoodsMoney($return_goods){
    	$order_goods = M('order_yxyp_goods')->where(array('rec_id'=>$return_goods['rec_id']))->find();
