...
class RedGoodsLogic extends Model
{
    ...
    public function goods_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
    {
                ...
                $sql = "SELECT * FROM  __PREFIX__goods_red_category ORDER BY parent_id , sort_order ASC";
    ...
    {            
        $GoodsCategory = M("GoodsRedCategory"); // 实例化User对象
        $cat = $GoodsCategory->where("id = $id")->find(); // 找出他自己
        // 刚新增的分类先把它的值重置一下
        if($cat['parent_id_path'] == '')
        {
            ($cat['parent_id'] == 0) && Db::execute("UPDATE __PREFIX__goods_red_category set  parent_id_path = '0_$id', level = 1 where id = $id"); // 如果是一级分类               
            Db::execute("UPDATE __PREFIX__goods_red_category AS a ,__PREFIX__goods_red_category AS b SET a.parent_id_path = CONCAT_WS('_',b.parent_id_path,'$id'),a.level = (b.level+1) WHERE a.parent_id=b.id AND a.id = $id");                
        ...
        Db::execute("UPDATE `__PREFIX__goods_red_category` SET parent_id_path = REPLACE(parent_id_path,'{$cat['parent_id_path']}','$replace_str'), level = (level - $replace_level) WHERE  parent_id_path LIKE '{$cat['parent_id_path']}%'");        
    }    
    ...
    public function getAttrInput($goods_id,$type_id)
    {
        ...
        $GoodsAttribute = D('GoodsRedAttribute');
    ...
    {     
        ...
        foreach ($spec_arr_sort as $key =>$val)
        {
         ...
         $spec = M('SpecRed')->getField('id,name'); // 规格表
         $specItem = M('SpecRedItem')->getField('id,item,spec_id');//规格项
         $keySpecGoodsPrice = M('SpecRedGoodsPrice')->where('goods_id = '.$goods_id)->getField('key,key_name,price,cost_price,cost_operating,store_count,bar_code,sku');//规格项
       ...
       foreach ($spec_arr2 as $k => $v) 
       {
            ...
            $keySpecGoodsPrice[$item_key][cost_price]     ? false : $keySpecGoodsPrice[$item_key][cost_price] = 0;     //成本默认为0
            $keySpecGoodsPrice[$item_key][cost_operating] ? false : $keySpecGoodsPrice[$item_key][cost_operating] = 0; //成本默认为0
            if($goodsinfo['is_lock'] == 0  && $goodsinfo){
                $str .="<td><input name='item[$item_key][price]' value='{$keySpecGoodsPrice[$item_key][price]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' readonly='readonly'/></td>";
                $str .="<td><input name='item[$item_key][store_count]' value='{$keySpecGoodsPrice[$item_key][store_count]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' readonly='readonly' /></td>";            
                $str .="<td><input name='item[$item_key][cost_price]' value='{$keySpecGoodsPrice[$item_key][cost_price]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' readonly='readonly' />";
                $str .="<td><input name='item[$item_key][cost_operating]' value='{$keySpecGoodsPrice[$item_key][cost_operating]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' readonly='readonly' />";
                $str .="<td><input name='item[$item_key][sku]' value='{$keySpecGoodsPrice[$item_key][sku]}' readonly='readonly' />
                    <input type='hidden' name='item[$item_key][key_name]' value='$item_name' readonly='readonly' /></td>";
            }else{
                ...
                $str .="<td><input name='item[$item_key][cost_price]' value='{$keySpecGoodsPrice[$item_key][cost_price]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' />";
                $str .="<td><input name='item[$item_key][cost_operating]' value='{$keySpecGoodsPrice[$item_key][cost_operating]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' />";
    ...
    function GetSpecCheckboxList($type_id, $checked = array()){
        $list = M('SpecRed')->where("type_id = $type_id")->order('`order` desc')->select();        
    ...
    function GetAttrCheckboxList($type_id, $checked = array()){                
        $list = M('GoodsRedAttribute')->where("type_id = $type_id and attr_index > 0 ")->order('`order` desc')->select();        
        //$list = M('Spec')->where("1=1")->order('`order` desc')->select();        
        $str = '';
    ...
    function find_parent_cat($cat_id)
    {
        ...
        $cat_list =  M('goods_red_category')->getField('id,parent_id,level');
    ...
    function getSortBrands()
    {
        //关闭功能
        return false;
        $brandList = S('getSortBrandsRed');
        if(!empty($brandList)){
            return $brandList;
        }
        $brandList =  M("BrandRed")->cache(true)->select();
        $brandIdArr =  M("BrandRed")->cache(true)->where("name in (select `name` from `".C('database.prefix')."brand` group by name having COUNT(id) > 1)")->getField('id,cat_id');
        $goodsCategoryArr = M('goodsRedCategory')->cache(true)->where("level = 1")->getField('id,name');
        $nameList = array();
    ...
    {
        $categoryList = S('categoryListRed');
        if($categoryList)
        {
            return $categoryList;
        }
        $categoryList =  M("GoodsRedCategory")->cache(true)->getField('id,name,parent_id,level');
        $nameList = array();
        ...
        S('categoryListRed',$categoryList);
        return $categoryList;
    }
    ...
    {
        $categoryList = S('newredcategoryList_'.$parent_id);
        if($categoryList)
        {
            return $categoryList;
        }
        $categoryList =  M("GoodsRedCategory")->where(['parent_id'=>$parent_id])->order('parent_id,sort_order ASC')->getField('id,name,parent_id,level');

        S('newredcategoryList_'.$parent_id,$categoryList);
        return $categoryList;
    }
    ...
    {
        $suppliersList = S('suppliersList');
        if($suppliersList)
        {
            return $suppliersList;
        }
        $suppliersList =  M("suppliers")->cache(true)->getField('suppliers_id,suppliers_name,user_id,is_check,status');
        $nameList = array();
        foreach($suppliersList as $k => $v)
        {

            $name = getFirstCharter($v['suppliers_name']) .' '. $v['suppliers_name']; // 前面加上拼音首字母
            $str = "";
            switch ($v['is_check']) {
                case '0':
                    $str = "(未审核)";
            ...
            $nameList[] = $v['suppliers_name'] = $name.$str;
            $suppliersList[$k] = $v;
        }
        array_multisort($nameList,SORT_STRING,SORT_ASC,$suppliersList);

        S('suppliersList',$suppliersList);
        return $suppliersList;
    }
    ...
    function savePreSell($data)
    {
        ...
        if (array_key_exists('shut_down', $data) && $data['shut_down'] == 1) {
            if (!empty($data['id'])) {
                $pre_sell_shut_down = M('goods_red_activity')->where(array('act_id' => $data['id']))->save(array('end_time' => time()));
        ...
        $goods_main = M('goods_red')->where(array('goods_id'=>$data['goods_id']))->find();
        ...
        if (empty($data['id'])) {
            ...
            $is_have_goods = M('goods_red_activity')->where($is_have_where)->find();
            if($is_have_goods){
        ...
        if(!empty($data['id']) && !empty($data['goods_id'])){
            $goods_activity = M('goods_red_activity')->where(array('act_id'=>$data['id']))->find();
        ...
        if (empty($data['id'])) {
            ...
            M('goods_red')->where(array('goods_id'=>$data['goods_id']))->save($goods_save_data);
            $r = M('goods_red_activity')->add($add_data);
            ...
            $goods_save_data['prom_type'] = 4;//预售商品类型
            M('goods_red')->where(array('goods_id'=>$data['goods_id']))->save($goods_save_data);
            $r = M('goods_red_activity')->where(array('act_id' => $data['id'], 'act_type' => 1))->save($save_data);
            if ($r !== false) {
