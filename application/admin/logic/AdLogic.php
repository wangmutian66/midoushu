...
class AdLogic extends Model
{
     ...
     * @return  mix
     */
    public function ads_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0,$status= 1)
    {
                global $ads_category, $ads_category2;                
                $sql = "SELECT * FROM  __PREFIX__ad_category where status = {$status} ORDER BY parent_id , sort_order ASC";
                $ads_category = DB::query($sql);
                $ads_category = convert_arr_key($ads_category, 'id');
                ...
                return $ads_category2;       
                        
    }
    
    /**
     * 获取指定id下的 所有分类      
     * @global type $ads_category 所有广告分类
    ...
    {
        global $ads_category, $ads_category2;          
        $ads_category2[$id] = $ads_category[$id];    
        foreach ($ads_category AS $key => $value){
             if($value['parent_id'] == $id)
             {                 
                $this->get_cat_tree($value['id']);  
                $ads_category2[$id]['have_son'] = 1; // 还有下级
             }
        }               
    }
    ...
    public function get_child_cat($id){
        if($id >0){
            $child_cat =  M('ad_category')->where('parent_id='.$id)->select();
            return $child_cat;
        }else{
            return '';
        }
    ...
    {            
        $AdCategory = M("ad_category"); // 实例化User对象
        $cat = $AdCategory->where("id = $id")->find(); // 找出他自己
        // 刚新增的分类先把它的值重置一下
        if($cat['parent_id_path'] == '')
        {
            ($cat['parent_id'] == 0) && Db::execute("UPDATE __PREFIX__ad_category set  parent_id_path = '0_$id', level = 1 where id = $id"); // 如果是一级分类               
            Db::execute("UPDATE __PREFIX__ad_category AS a ,__PREFIX__ad_category AS b SET a.parent_id_path = CONCAT_WS('_',b.parent_id_path,'$id'),a.level = (b.level+1) WHERE a.parent_id=b.id AND a.id = $id");                
            $cat = $AdCategory->where("id = $id")->find(); // 从新找出他自己
        }        
        ...
        if($cat['parent_id'] == 0) //有可能是顶级分类 他没有老爸
        {
            ...
            $parent_cat = $AdCategory->where("id = {$cat['parent_id']}")->find(); // 找出他老爸的parent_id_path
        }        
        ...
        Db::execute("UPDATE `__PREFIX__ad_category` SET parent_id_path = REPLACE(parent_id_path,'{$cat['parent_id_path']}','$replace_str'), level = (level - $replace_level) WHERE  parent_id_path LIKE '{$cat['parent_id_path']}%'");        
    }    
    ...
    function find_parent_cat($cat_id)
    {
        ...
        $cat_list =  M('ad_category')->getField('id,parent_id,level');
