...
use think\Model;
use think\Db;
class RedOrderLogic extends Model
{
    /**
    ...
    public function getOrderList($condition,$order='',$start=0,$page_size=20){
        $res = M('order_red')->where($condition)->limit("$start,$page_size")->order($order)->select();
        return $res;
    }
    ...
    public function getOrderGoods($order_id,$is_send =''){
        ...
        $sql = "SELECT g.*,o.*,(o.goods_num * o.midou) AS midou_total,(o.goods_num * o.midou_money) AS midou_money_total FROM __PREFIX__order_red_goods o ".
            "LEFT JOIN __PREFIX__goods_red g ON o.goods_id = g.goods_id WHERE o.order_id = $order_id ".$where;
        $res = DB::query($sql);
        return $res;
    }
    ...
    public function get_spec_goods($goods_id_arr){
		...
		{
			$arr   = array();
			$goods = M('goods_red')->where("goods_id = $key")->find();
            ...
            $arr['midou_use_percent']  = tpCache('shoppingred.midou_use_percent');
            if($goods['is_z_change'] != 1) $arr['midou_use_percent'] = $goods['midou_use_percent'];  // 购买商品 使用米豆 比率

            $midouInfo                 = getMidou($key);
            $arr['max_midou']          = $arr['midou']        = $midouInfo['midou'];
            $arr['goods_midou_money']  = $midouInfo['midou_money']; 
			...
			foreach($val as $k => $v)
			{
				...
				{
					$arr['spec_key']           = $k;
					$spec_goods                = M('spec_red_goods_price')->where("goods_id = $key and `key` = '{$k}'")->find();
                    ...
                    $arr['cost_price']         = $spec_goods['cost_price'];
                    $midouInfo                 = getMidou($key,$spec_goods['item_id']);
                    $arr['max_midou']          = $arr['midou']        = $midouInfo['midou'];
                    $arr['midou_money']        = $midouInfo['midou_money']; 
				}
				$order_goods[] = $arr;
			}
		}
    ...
    public function orderRefuseLog($order_id,$suppliers_id=0,$action,$refuse_status,$note=''){
        ...
        return M('order_red_refuse')->add($data);//订单操作记录
    }
    ...
    public function getGoodsAmount($order_id){
        $sql = "SELECT SUM(goods_num * goods_price) AS goods_amount FROM __PREFIX__order_red_goods WHERE order_id = {$order_id}";
        $res = DB::query($sql);
...
//        /* 选择一个随机的方案 */send_http_status('310');
    ...
    public function orderProcessHandle($order_id,$act,$ext=array()){
    	...
    	switch ($act){
    		case 'pay': //付款
                ...
                update_pay_status_red($order_sn,$ext); // 调用确认收货按钮
    			return true;    			
    		...
    		case 'remove': //移除订单
            //移除订单
                exit('请完善移除订单，位置：applacation/admin/RedOrderLogic:224行');
    			$this->delOrderRed($order_id);
    			break;
    		case 'delivery_confirm'://确认收货
    			confirm_order_red($order_id); // 调用确认收货按钮
    			return true;
            case 'refuse': // 拒绝发货申请
    	...
    	return M('order_red')->where("order_id=$order_id")->save($updata);//改变订单状态
    }
    ...
    {
    	//如果这笔订单已经取消付款过了
    	$count = M('order_red')->where("order_id = $order_id and pay_status = 1")->count();   // 看看有没已经处理过这笔订单  支付宝返回不重复处理操作
    	if($count == 0) return false;
    	// 找出对应的订单
    	$order = M('order_red')->where("order_id = $order_id")->find();
    	// 增加对应商品的库存
        $orderGoodsArr = M('OrderRedGoods')->where("order_id = $order_id")->select();
    	foreach($orderGoodsArr as $key => $val)
    	{
    		...
    		{   // 先到规格表里面增加数量 再重新刷新一个 这件商品的总数量
    			...
    			refresh_stock_red($val['goods_id']);
    		}else{
				$specGoodsPrice = null;
    			M('goods_red')->where("goods_id = {$val['goods_id']}")->setInc('store_count',$val['goods_num']); // 增加商品总数量
    		}
    		M('goods_red')->where("goods_id = {$val['goods_id']}")->setDec('sales_sum',$val['goods_num']); // 减少商品销售量
			//更新活动商品购买量
            //TK  活动商品取消  2018年4月23日09:35:00
			if ($val['prom_type'] == 1 || $val['prom_type'] == 2) {
				$GoodsPromFactory = new \app\common\logic\GoodsRedPromFactory();
    	...
    	M('order_red')->where("order_id=$order_id")->save(array('pay_status'=>0));
    	update_user_level_red($order['user_id']);
    	// 记录订单操作日志
    	logOrderRed($order['order_id'],'订单取消付款','付款取消',$order['user_id']);
    	//分销设置
    ...
    public function deliveryHandle($data){
        ...
        if(is_array($reslist) && !empty($reslist)) {
            ...
            $post_data["param"] = '{"company":"'.$data['shipping_code'].'","number":"'.$data['invoice_no'].'","from":"","to":"","key":"'.$invoice_key.'","parameters":{"callbackurl":"'.$callbackurl.'?red=1&orderid='.$data['order_id'].'","salt":"'.get_rand_str(15,0,1).'","resultv2":"1"}}';
            #订阅请求地址
		...
		$did = M('delivery_red_doc')->add($data);
		$is_delivery = 0;
		foreach ($orderGoods as $k=>$v){
			...
			if($v['is_send'] == 0 && in_array($v['rec_id'],$selectgoods)){
				$res['is_send'] = 1;
				$res['delivery_id'] = $did;
				$r = M('order_red_goods')->where("rec_id=".$v['rec_id'])->save($res);//改变订单商品发货状态
				$is_delivery++;
			}
		}
		...
		M('order_red')->where("order_id=".$data['order_id'])->save($updata);//改变订单状态
		$s = $this->orderActionLog($order['order_id'],'delivery',$data['note']);//操作日志

        foreach($orderGoods as $key=>$val){
            update_stock_log_red($order['user_id'], (-1*$val['goods_num']), $val, $order['order_sn']);//库存日志
        }

        #快递推送记录存储
        #张洪凯  2018-11-27
        $kresult = M('delivery_red_log')->where("order_id=".$data['order_id'])->find();
        if(!$kresult){
    ...
    public function updateOrderShipping($data=[],$order=[]){
        ...
        $updata['shipping_code'] = $data['shipping_code'];
        $updata['shipping_name'] = $data['shipping_name'];
        M('order_red')->where(['order_id'=>$data['order_id']])->save($updata); //改变物流信息
        $updata['invoice_no'] = $data['invoice_no'];
        $updata['note']       = $data['note'];
        $delivery_res = db('delivery_red_doc')->where(['order_id'=>$data['order_id']])->save($updata);
        ...
        if ($delivery_res !== false){
            ...
            if(!$kresult){
                ...
                M('delivery_red_log')->where(['id'=>$kresult['id']])->update($kdata);
            }
    ...
    function delOrder($order_id){
    	...
    	$del_order = M('order_red')->where(array('order_id'=>$order_id))->delete();
    	$del_order_goods = M('order_red_goods')->where(array('order_id'=>$order_id))->delete();
	...
	{
		$order_goods_list  = Db::name('order_red_goods')->where(['order_id' => $order_id])->select();
		...
		if ($order_goods_count == $order_goods_return_count) {
			 $res = Db::name('order_red')->where(['order_id' => $order_id])->update(['order_status' => 5]);
            if(!$res){
                return false;
            }
		}
    ...
    public function disposereRurnOrderCoupon($return_info){
        $coupon_list = M('coupon_red_list')->where(['uid'=>$return_info['user_id'],'order_id'=>$return_info['order_id']])->find();    //有没有关于这个商品的优惠券
        ...
        //追回赠送优惠券,一般退款才会走这里
        $coupon_info = M('coupon_red_list')->where(['uid'=>$return_info['user_id'],'get_order_id'=>$return_info['order_id']])->find();
        if(!empty($coupon_info)){
            if($coupon_info['status'] == 1) { //如果优惠券被使用,那么从退款里扣
                ...
                if ($return_info['refund_money'] > $coupon['money']) {
                    ...
                    M('return_red_goods')->where(['id' => $return_info['id']])->save(['refund_money' => $return_info['refund_money']]);
                }else{
                    $return_info['refund_deposit'] = $return_info['refund_deposit'] - $coupon['money'];
                    M('return_red_goods')->where(['id' => $return_info['id']])->save(['refund_deposit' => $return_info['refund_deposit']]);
                }
            }else {
                M('coupon_red_list')->where(array('id' => $coupon_info['id']))->delete();
                M('coupon_red')->where(array('id' => $coupon_info['cid']))->setDec('send_num');
            }
        }
    }
    ...
    public function getRefundGoodsMoney($return_goods){
    	$order_goods = M('order_red_goods')->where(array('rec_id'=>$return_goods['rec_id']))->find();
