 ...
 * @package admin\Logic
 */
class YxypSearchWordLogic extends Model
{
    public function initGoodsSearchWord(){
        header("Content-Type:text/html;charset=utf-8");
        Db::query('TRUNCATE table '.C('database.prefix').'search_word');
        $goodsKeyWords = Db::name('GoodsYxyp')->field('keywords')->where('keywords','<>','')->distinct('keywords')->select();
        ...
        foreach($goodsKeyWords as $key=>$val){
            ...
            $goods_num = Db::name('goods_yxyp')->where('goods_name|keywords','like',$goods_where_like)->count();
            $search_add_data['goods_num'] = $goods_num;
            $success = Db::name('search_yxyp_word')->insert($search_add_data);
            if($success !== false){
                $successNum++;
            }
