...
class Order extends Base {
    ...
    public function return_goods_list()
    {
        ...
        if(!empty($goods_id_arr))
            $goodsList = M('goods')->where("goods_id","in", implode(',',$goods_id_arr))->getField('goods_id,goods_name,shop_price');
    ...
    */
    function buy_post_shipping(){
        ...
        if(db('return_goods')->where("user_id = {$this->user_id}")->cache(true)->find($return_id)){
    ...
    public function comment_list()
    {
        ...
        if (empty($order_id) || empty($rec_id)) {
            ...
            $order_info = M('order')->field('order_sn,order_id,add_time,order_prom_type,suppliers_id') ->where($order_comment_where)->find();
            //查找评价商品
