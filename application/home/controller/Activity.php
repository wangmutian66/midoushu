...
class Activity extends Base
{
    ...
    public function new_sale()
    {
        ...
        $show = $Page->show_cx();// 分页显示输出
        $this->assign('page', $show);// 赋值分页输出
        $list = M('goods')->where($map)->limit($Page->firstRow.','.$Page->listRows)->order('new_sort desc')->select();
        foreach ($list as $k => $val) {
            // 可返米豆
            $midouInfo = returnMidou($val['goods_id']);
            $val['back_midou'] = $midouInfo['midou'];
    ...
    {
        $goods_where['p.start_time'] = array('lt',time()); // 开始时间
        $goods_where['p.end_time']   = array('gt',time()); // 结束时间
        $goods_where['p.is_end']     = 0;  // 是否结束
        $goods_where['g.prom_type']  = 3;  // 商品促销
        $goods_where['g.is_on_sale'] = 1;  // 是否上架
        $goods_where['g.is_check']   = 1;  // 是否审核
        // 数量
        $count = Db::name('goods')
