...
class User extends MobileBase
{
    ...
    */
    ...
    public function index()
    {
        ...
        $comment_count = M('comment_red')->where("user_id", $user_id)->count();   // 我的评论数
    ...
    public function logout()
    {
        ...
        header("Location:" . U('Mobilered/Index/index'));
        exit();
    }
    ...
    public function reg()
    {
        ...
        if($this->user_id > 0) {
            $this->redirect(U('Mobilered/User/index'));
        }
        ...
        if (IS_POST) {
            ...
            if ($data['status'] != 1){
               # $this->error($data['msg']);
                ...
                $data['url']=U('/Mobilered/Cart/cart2', array('address_id' => $data['result'],'goods_id'=>$goods_id,'goods_num'=>$goods_num,'item_id'=>$item_id,'action'=>$action));
                $this->ajaxReturn($data);
            } elseif ($_POST['source'] == 'integral') {
                $data['url']=U('/Mobilered/Cart/integral', array('address_id' => $data['result'],'goods_id'=>$goods_id,'goods_num'=>$goods_num,'item_id'=>$item_id));
                $this->ajaxReturn($data);
            } elseif($source == 'pre_sell_cart'){
                $data['url']=U('/Mobilered/Cart/pre_sell_cart', array('address_id' => $data['result'],'act_id'=>$post_data['act_id'],'goods_num'=>$post_data['goods_num']));
                $this->ajaxReturn($data);
            } elseif($_POST['source'] == 'team'){
                $data['url']= U('/Mobilered/Team/order', array('address_id' => $data['result'],'order_id'=>$order_id));
                $this->ajaxReturn($data);
            }else{
                $data['url']= U('/Mobilered/User/address_list');
                $this->success($data['msg'], U('/Mobilered/User/address_list'));
            } 
    ...
    public function set_default()
    {
        ...
        if ($source == 'cart2') {
            header("Location:" . U('Mobilered/Cart/cart2'));
            exit;
        } else {
            header("Location:" . U('Mobilered/User/address_list'));
        }
    }
    ...
    */
    ...
    {
        if ($this->user_id > 0) {
            $this->redirect('Mobilered/User/index');
        }
    ...
    {
        $count = M('goods_red_visit')->where('user_id', $this->user_id)->count();
    ...
    public function del_visit_log()
    {
        ...
        $row = M('goods_red_visit')->where('visit_id','IN', $visit_ids)->delete();
    ...
    {
        $row = M('goods_red_visit')->where('user_id', $this->user_id)->delete();
    ...
    function already_rebate(){
        ...
        $list = M('order_red')->alias('order')->where($where)
                                    ->order("add_time asc")
                                    ->field("add_time,order_amount,shipping_price,already_rebate,midou_money")
                                    ->limit($page->firstRow . ',' . $page->listRows)
                                    ->select();
    ...
    public function doassignment(){
        ...
        //检测支付密码是否正确
        $yzuser = M('users')->where(['paypwd'=>encrypt($paypwd),'user_id'=>$user_id])->find();
        ...
        if($yzuser["mobile"] == $account){
            $result['error'] = 1;
            $result['msg'] = "不能将米豆转到本账号";
            $this->ajaxReturn($result);
        }

        //检测转账账号是否存在
        $users = M('users')->where(['mobile'=>$account])->find();
        $result['error'] = 0;
        ...
        if($users['real_name'] != $post["realname"]){
            $result['error'] = 1;
            $result['msg'] = "真实姓名不匹配";
            $this->ajaxReturn($result);
        }
        ...
        $account = accountLog($users['user_id'],0,$midou,0,"从{$yzuser['mobile']},收到{$midou}米豆",$midou);
        if(!$account){
            $result['error'] = 1;
            $result['msg'] = "操作失败，请稍后重试";
            $this->ajaxReturn($result);
        }
