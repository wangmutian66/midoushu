...
use app\common\logic\RedTeamFoundLogic;
use app\common\logic\RedTeamOrderLogic;
use app\common\model\GoodsRed;
use app\common\model\OrderRed;
use app\common\model\OrderRedGoods;
use app\common\model\ShippingArea;
use app\common\model\TeamRedActivity;
use app\common\model\TeamRedFollow;
...
class Team extends MobileBase
{
    ...
    {
        $goods_category = Db::name('goods_red_category')->where(['level' => 1, 'is_show' => 1])->select();
        $this->assign('goods_red_category', $goods_category);
        return $this->fetch();
    }
    ...
    public function category()
    {
        ...
        $goods_category_level_one = Db::name('goods_red_category')->where(['id' => $id])->find();
        $goods_category_level_two = Db::name('goods_red_category')->where(['parent_id' => $goods_category_level_one['id']])->select();//二级分类
    ...
    public function AjaxTeamList(){
        ...
        $goods_where = [];
        if($id && $two_all_ids){
            $category_three_ids = Db::name('goods_red_category')->where(['parent_id' => ['in',$two_all_ids]])->getField('id',true);//三级分类id
            $goods_where['cat_id'] = ['in',$category_three_ids];
        }
        if($tid){
            $category_three_ids = Db::name('goods_red_category')->where(['parent_id' => $tid])->getField('id',true);//三级分类id
            $goods_where['cat_id'] = ['in',$category_three_ids];
        }
        $team_where = ['t.status' => 1, 't.is_lottery' => 0, 'g.is_on_sale' => 1];
        if(count($goods_where) > 0){
            $goods_ids = Db::name('goods_red')->where($goods_where)->getField('goods_id', true);
            if(!empty($goods_ids)){
        ...
        $TeamActivity = new TeamRedActivity();
        $list = $TeamActivity->alias('t')->join('__GOODS_RED__ g', 'g.goods_id = t.goods_id')
    ...
    public function info(){
        ...
        if(empty($goods_id)){
            $this->error('参数错误', U('Mobilered/Team/index'));
        }
        $TeamActivity = new TeamRedActivity();
        $Goods = new GoodsRed();
        ...
        if (empty($teamList)) {
            $this->error('该商品拼团活动不存在或者已被删除', U('Mobilered/Team/index'));
        }
        if(empty($goods)){
            $this->error('此商品不存在或者已下架', U('Mobilered/Team/index'));
        }
        ...
        if($user_id){
            $collect = Db::name('goods_red_collect')->where(array("goods_id"=>$goods_id ,"user_id"=>$user_id))->count();
            $this->assign('collect',$collect);
        }
        $spec_goods_price = Db::name('spec_red_goods_price')->where("goods_id",$goods_id)->getField("key,price,store_count,item_id,prom_id"); // 规格 对应 价格 库存表
        if($spec_goods_price){
        ...
        $this->assign('spec_red_goods_price', json_encode($spec_goods_price,true));
        $goods_images_list = Db::name('goods_red_images')->where("goods_id" , $goods_id)->select(); // 商品图册
    ...
    public function addOrder()
    {
        ...
        $team = TeamRedActivity::get($team_id);
        if($found_id){
            $teamFound = TeamRedFound::get(['found_id' => $found_id, 'status' => 1]);
            if(empty($teamFound)) {
            ...
            $teamFoundLogic = new RedTeamFoundLogic();
        ...
        $teamOrderLogic = new RedTeamOrderLogic();
        if (!empty($teamFound)) {
    ...
    public function order()
    {
        ...
        $Order = new OrderRed();
        $OrderGoods = new OrderRedGoods();
        $order = $Order->where(['order_id'=>$order_id,'user_id'=>$this->user_id])->find();
        if(empty($order)){
            $this->error('订单不存在或者已取消', U("Mobilered/Order/order_list"));
        }
        if ($address_id) {
        ...
        if(empty($address)){
            header("Location: ".U('Mobilered/User/add_address',array('source'=>'team','order_id'=>$order_id)));
            exit;
        }else{
        ...
        if($order['pay_status'] == 1){
            $order_detail_url = U("Mobilered/Order/order_detail",array('id'=>$order_id));
            header("Location: $order_detail_url");
        }
        if($order['order_status'] == 3 ){   //订单已经取消
            $this->error('订单已取消',U("Mobilered/Order/order_list"));
        }
        //微信浏览器
    ...
    public function found()
    {
        ...
        if(time() - $teamFound['found_time'] > $team['time_limit']){
            ...
            if($teamFound['join'] < $teamFound['need']){
                ...
                $teamFound->save();
                //更新团员成团失败
                Db::name('team_red_follow')->where(['found_id'=>$found_id,'status'=>1])->update(['status'=>3]);
            }
        }
    ...
    public function lottery(){
        ...
        $team_lottery = Db::name('team_red_lottery')->where('team_id',$team_id)->select();
        $TeamActivity = new TeamRedActivity();
