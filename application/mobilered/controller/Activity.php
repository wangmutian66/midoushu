...
class Activity extends MobileBase {
    ...
    public function new_sale()
    {
       $p = I('p/d',1);
        ...
        $this->assign('goodsList', $list);
        if(I('is_ajax/d')){
            // dump(M('goods_red')->getlastsql());die();
            // dump($midou_rate);
            // dump($start_price);dump($end_price);
            return $this->fetch('ajax_hot_sale');
        }
        $this->assign('sort', $sort);
        $this->assign('sort_asc', $sort_asc);
    ...
    public function group_list()
    {
    	...
    	$count =  $GroupBuy->alias('gb')->join('__GOODS_RED__ g', 'g.goods_id = gb.goods_id')->where($group_by_where)->count();// 查询满足要求的总记录数
        ...
        $list = $GroupBuy
            ->alias('gb')
            ->join('__GOODS_RED__ g', 'gb.goods_id=g.goods_id AND g.prom_type=2')
    ...
    public function discount_list(){
        ...
        $count =  M('goods_red')->where($where)->count(); // 查询满足要求的总记录数
        $pagesize = C('PAGESIZE');  //每页显示数
        $Page = new Page($count,$pagesize); //分页类
        $prom_list = Db::name('goods_red')->where($where)->limit($Page->firstRow.','.$Page->listRows)->select(); //活动对应的商品
        $spec_goods_price = Db::name('specRedGoodsPrice')->where(['prom_type'=>3,'prom_id'=>$prom_id])->select(); //规格
        ...
        foreach($prom_list as $gk =>$goods){  //计算优惠价格
            $PromGoodsLogicuse = new \app\common\logic\RedPromGoodsLogic($goods,$goods['spec_goods_price']);
    ...
    public function promote_goods(){
        ...
        $count = M('prom_red_goods')->where($where)->count();  // 查询满足要求的总记录数
        $pagesize = C('PAGESIZE');  //每页显示数
        $Page  = new Page($count,$pagesize); //分页类
        $promote = M('prom_red_goods')->field('id,title,start_time,end_time,prom_img')->where($where)->limit($Page->firstRow.','.$Page->listRows)->select();    //查询活动列表
    ...
    public function ajax_flash_sale()
    {
        ...
        $flash_sale_goods = $FlashSale->alias('fl')->join('__GOODS_RED__ g', 'g.goods_id = fl.goods_id')->with(['specGoodsPrice','goods'])
    ...
    public function pre_sell()
    {
    	...
    	if (empty($pre_sell_info)) {
    		$this->error('对不起，该预售商品不存在或者已经下架了', U('Mobilered/Activity/pre_sell_list'));
    		exit();
    	}
    	...
    	$spec_goods_price = M('spec_red_goods_price')->where(array('goods_id' => $goods['goods_id']))->getField("key,price,store_count"); // 规格 对应 价格 库存表
