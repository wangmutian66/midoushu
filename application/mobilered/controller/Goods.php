...
class Goods extends MobileBase {
    ...
    public function search(){
        ...
        if($sel)
        {
            ...
            $where = "1=1"; 
            $Goods = M("goods_red")->where("goods_id", "in", implode(',', $filter_goods_id));
            //查看全部
            if($sel == 'selall'){
                $where .= '';
            }
...
//        }
        ...
        $goods_id = db('store_goods_stock')->where(['store_id'=>$store_id,'is_examine'=>1])->column("goods_id");
        ...
        if(input('is_ajax')){
            return $this->fetch('ajaxGoodsListstore');
        }else{
            return $this->fetch();
        }
