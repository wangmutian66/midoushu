...
class Article extends MobileBase {
    ...
    public function noticeList(){
        ...
        $Page   = new Page($count, 20); // 实例化分页类 传入总记录数和每页显示的记录数
        $article = Db::name('article_notice')->where("(article_type = 0 OR article_type = 1) AND is_open = 1")->page($p,C('PAGESIZE'))->order('add_time desc')->select();
        if($article){
            $noticeList['articleList']=noticeimgurl($article);
        }
        $noticeList['page']['nowPages']=$p;
    ...
    public function notice_detail(){
        ...
        if($article){
            ...
            $notice_detail=contentURL($article);
        }
