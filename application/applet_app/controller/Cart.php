...
class Cart extends MobileBase {
    ...
    public function index(){
        ...
        foreach($cartList as $k => $val){
            ...
            $goods_arr[$k]['suppliers_id'] = $val['suppliers_id'];
        ...
        $resultsss = array();
        foreach($resultss as $k3=>$v3){
            $resultsss['goodlist'][] = $v3;
        }
        dump($resultsss);die;
        return formt($resultss);
...
/**
    ...
    public function cartSure(){
        ...
        foreach ($cartList['cartList'] as $k => $val) {
            ...
            $goods_arr[$k]['shippinglist'] =  M('Plugin')->where("`type` = 'shipping' and is_default = 1 and suppliers_id = ".$val['suppliers_id'])->field('name,desc')->select();  // 物流公司
        }
