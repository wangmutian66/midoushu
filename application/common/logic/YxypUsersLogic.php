 ...
 * @package Home\Logic
 */
class YxypUsersLogic extends Model
{
    protected $user_id=0;
    ...
    public function get_info($user_id)
    {
        ...
        $activityLogic = new \app\common\logic\YxypActivityLogic;             //获取能使用优惠券个数
        ...
        $user['return_count'] = M('return_yxyp_goods')->where("user_id=$user_id and status<3 and status>-1")->count();   //退换货数量
        $user['waitPay']     = M('order_yxyp')->where("user_id = :user_id ".C('WAITPAY').$where)->bind(['user_id'=>$user_id])->count(); //待付款数量
        $user['waitSend']    = M('order_yxyp')->where("user_id = :user_id ".C('WAITSEND').$where)->bind(['user_id'=>$user_id])->count(); //待发货数量
        $user['waitReceive'] = M('order_yxyp')->where("user_id = :user_id ".C('WAITRECEIVE').$where)->bind(['user_id'=>$user_id])->count(); //待收货数量
        $user['order_count'] = $user['waitPay'] + $user['waitSend'] + $user['waitReceive'];
        $commentLogic = new YxypCommentLogic;
    ...
    public function getApiUserInfo($user_id)
    {
        ...
        $user['collect_count'] = $this->getGoodsCollectNum($user_id);; //获取收藏数量
        $user['visit_count']   = M('goods_yxyp_visit')->where('user_id', $user_id)->count();   //商品访问记录数
        $user['return_count'] = M('return_yxyp_goods')->where("user_id=$user_id and status<2")->count();   //退换货数量
        
        $user['waitPay']     = M('order_yxyp')->where("user_id = :user_id ".C('WAITPAY'))->bind(['user_id'=>$user_id])->count(); //待付款数量
        $user['waitSend']    = M('order_yxyp')->where("user_id = :user_id ".C('WAITSEND'))->bind(['user_id'=>$user_id])->count(); //待发货数量
        $user['waitReceive'] = M('order_yxyp')->where("user_id = :user_id ".C('WAITRECEIVE'))->bind(['user_id'=>$user_id])->count(); //待收货数量
        $user['order_count'] = $user['waitPay'] + $user['waitSend'] + $user['waitReceive'];
    ...
    public function get_last_order($user_id){
        $last_order = M('order_yxyp')->where("user_id", $user_id)->order('order_id DESC')->find();
        return $last_order;
    }
    ...
    public function get_order_goods($order_id){
        $sql = "SELECT og.*,g.commission,rg.status as rstatus FROM __PREFIX__order_yxyp_goods og LEFT JOIN __PREFIX__goods_yxyp g ON g.goods_id = og.goods_id LEFT JOIN __PREFIX__return_yxyp_goods rg ON rg.goods_id = og.goods_id and rg.order_id = og.order_id WHERE og.order_id = :order_id";
    ...
    {
        $activityLogic = new \app\common\logic\YxypActivityLogic;
    ...
    {
        $count = M('goods_yxyp_collect')->alias('c')
    ...
    public function get_goods_collect($user_id){
        ...
        //获取我的收藏列表
        $result = M('goods_yxyp_collect')->alias('c')
    ...
    public function get_comment($user_id,$status=2){
        if($status == 1){
            ...
            $commented_count = Db::name('comment')
                ->alias('c')
                ->join('__ORDER_YXYP_GOODS__ g','c.goods_id = g.goods_id and c.order_id = g.order_id', 'inner')
                ->where('c.user_id',$user_id)
                ->count();
            $page = new Page($commented_count,10);
            $comment_list = Db::name('comment')
                ->alias('c')
                ->field('c.*,g.*,(select order_sn from  __PREFIX__order_yxyp where order_id = c.order_id ) as order_sn')
                ->join('__ORDER_YXYP_GOODS__ g','c.goods_id = g.goods_id and c.order_id = g.order_id', 'inner')
                ->where('c.user_id',$user_id)
            ...
            $comment_count = Db::name('order_yxyp_goods')->alias('og')->join('__ORDER_YXYP__ o','o.order_id = og.order_id','left')->where($comment_where)->count();
            $page = new Page($comment_count,10);
            $comment_list = Db::name('order_yxyp_goods')
                ->alias('og')
                ->join('__ORDER_YXYP__ o','o.order_id = og.order_id','left')
                ->where($comment_where)
    ...
    public function add_comment($add){
        ...
        //检查订单是否已完成
        $order = M('order_yxyp')->field('order_status')->where("order_id", $add['order_id'])->where('user_id', $add['user_id'])->find();
        if($order['order_status'] != 2)
            return array('status'=>-1,'msg'=>'该笔订单还未确认收货','result'=>'');

        //检查是否已评论过
        $goods = M('comment_yxyp')->where(['order_id'=>$add['order_id'],'goods_id'=>$add['goods_id']])->find();
        if($goods)            
        ...
        {
            //更新订单商品表状态
            M('order_yxyp_goods')->where(array('goods_id'=>$add['goods_id'],'order_id'=>$add['order_id']))->save(array('is_comment'=>1));
            M('goods')->where(array('goods_id'=>$add['goods_id']))->setInc('comment_count',1); // 评论数加一
            // 查看这个订单是否全部已经评论,如果全部评论了 修改整个订单评论状态            
            $comment_count   = M('order_yxyp_goods')->where("order_id", $add['order_id'])->where('is_comment', 0)->count();
            if($comment_count == 0) // 如果所有的商品都已经评价了 订单状态改成已评价
            {
                M('order_yxyp')->where("order_id",$add['order_id'])->save(array('order_status'=>4));
            }
    ...
    {
        $visit = M('goods_yxyp_visit')->alias('v')
            ->field('v.visit_id, v.goods_id, v.visittime, g.goods_name, g.shop_price, g.cat_id')
            ->join('__GOODS_YXYP__ g', 'v.goods_id=g.goods_id')
