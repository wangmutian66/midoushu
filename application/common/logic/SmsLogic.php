...
class SmsLogic 
{
    ...
    {
        $smsTemp   = M('sms_template')->where("send_scene", $scene)->find();    //短信模板
        ...
        $money     = !empty($params['money']) ? $params['money'] : false;
        $store_name     = !empty($params['store_name']) ? $params['store_name'] : false;
        $store_contact     = !empty($params['store_contact']) ? $params['store_contact'] : false;
        $store_mobile     = !empty($params['store_mobile']) ? $params['store_mobile'] : false;
        ...
        $smsParams = array(
            ...
            8 => "{\"code\":\"$code\"}",                                       //6. 修改登录密码 (验证码类型短信只能有一个变量)
            9 => "{\"store_name\":\"$store_name\",\"store_contact\":\"$store_contact\",\"store_mobile\":\"$store_mobile\",\"money\":\"$money\"}", 
        );
