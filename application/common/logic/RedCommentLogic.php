 ...
 */
class RedCommentLogic extends Model
{
    ...
    public function addGoodsComment($add)
    {
        ...
        //检查订单是否已完成
        $order = M('order_red')->where(['order_id' => $add['order_id'], 'user_id' => $add['user_id']])->find();
        ...
        //更新订单商品表状态
        M('order_red_goods')->where(['goods_id'=>$add['goods_id'],'order_id'=>$add['order_id']])->save(['is_comment'=>1]);
        M('goods_red')->where(['goods_id'=>$add['goods_id']])->setInc('comment_count',1); // 评论数加一
        //
        // 查看这个订单是否全部已经评论,如果全部评论了 修改整个订单评论状态
        $comment_count = M('order_red_goods')->where(['order_id' => $add['order_id'], 'is_comment' => 0])->count();
        if ($comment_count == 0) {
            // 如果所有的商品都已经评价了 订单状态改成已评价
            M('order_red')->where("order_id ='{$add['order_id']}'")->save(['order_status' => 4]);
        }
    ...
    public function getComment($user_id, $status = 2)
    {
        if ($status == 1) {
            //已评论
            $query = M('comment_red')->alias('c')
                ->join('__ORDER_RED__ o', 'o.order_id = c.order_id')
                ->join('__ORDER_RED_GOODS__ og','c.goods_id = og.goods_id AND c.order_id = og.order_id AND og.is_comment=1')
            ...
            $query = M('order_red_goods')->alias('og')
                ->join('__ORDER_RED__ o',"o.order_id = og.order_id AND o.user_id=$user_id AND o.order_status IN (2,4)")
                ->where($comment_where);
            $query2 = clone($query);
    ...
    {
        $num = M('comment_red')->alias('c')
                ->join('__ORDER_RED__ o', 'o.order_id = c.order_id')
                ->join('__ORDER_RED_GOODS__ g','c.goods_id = g.goods_id AND c.order_id = g.order_id AND g.is_comment=1')
                ->where('c.user_id', $user_id)
                ->count();
        return $num;
    }
    ...
    public function getWaitCommentNum($user_id)
    {
        ...
        $num = M('order_red_goods')->alias('og')
            ->join('__ORDER_RED__ o',"o.order_id = og.order_id AND o.user_id=$user_id AND o.order_status IN (2,4)",'inner')
