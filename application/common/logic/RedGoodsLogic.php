 ...
 */
class RedGoodsLogic extends Model
{
    /**
     * @param $goods_id_arr
     * @param $filter_param
    ...
    public function get_filter_brand($goods_id_arr, $filter_param, $action)
    {
        ...
        $map['brand_id'] = ['>', 0];
        $brand_id_arr = M('goods_red')->where($map)->column('brand_id');
        $list_brand = M('brand_red')->where('id','in',$brand_id_arr)->limit('30')->select();
    ...
    public function get_filter_spec($goods_id_arr, $filter_param, $action, $mode = 0)
    {
        ...
        $spec_key = DB::query("select group_concat(`key` separator  '_') as `key` from __PREFIX__spec_red_goods_price where goods_id in($goods_id_str)");  //where("goods_id in($goods_id_str)")->select();
        ...
        $spec = M('spec_red')->where(array('search_index'=>1))->getField('id,name');
        $spec_item = M('spec_red_item')->where(array('spec_id'=>array('in',array_keys($spec))))->getField('id,spec_id,item');
    ...
    public function get_filter_attr($goods_id_arr = array(), $filter_param, $action, $mode = 0)
    {
        $goods_id_str = implode(',', $goods_id_arr);
        $goods_id_str = $goods_id_str ? $goods_id_str : '0';
        $goods_attr = M('goods_red_attr')->where(['goods_id'=>['in',$goods_id_str],'attr_value'=>['<>','']])->select();
        // $goods_attr = M('goods_attr')->where("attr_value != ''")->select();
        $goods_attribute = M('goods_red_attribute')->where("attr_index = 1")->getField('attr_id,attr_name,attr_index');
        if (empty($goods_attr)) {
            if ($mode == 1) return array();
            return array('status' => 1, 'msg' => '', 'result' => array());
        }
    ...
    public function commentStatistics($goods_id)
    {
        ...
        $c1 = M('comment_red')->where($commentWhere)->where('ceil((deliver_rank + goods_rank + service_rank) / 3) in (4,5)')->count();
        $c2 = M('comment_red')->where($commentWhere)->where('ceil((deliver_rank + goods_rank + service_rank) / 3) in (3)')->count();
        $c3 = M('comment_red')->where($commentWhere)->where('ceil((deliver_rank + goods_rank + service_rank) / 3) in (0,1,2)')->count();
        $c4 = M('comment_red')->where($commentWhere)->where("img !='' and img NOT LIKE 'N;%'")->count(); // 晒图
        $c0 = $c1 + $c2 + $c3; // 所有评论
        if($c0 <= 0){
            $rate1 = 100;
            $rate2 = 0;
    ...
    public function collect_goods($user_id, $goods_id)
    {
        ...
        $count = M('GoodsRedCollect')->where("user_id",$user_id)->where("goods_id", $goods_id)->count();
        if ($count > 0) return array('status' => -3, 'msg' => '商品已收藏', 'result' => array());
        M('GoodsRedCollect')->add(array('goods_id' => $goods_id, 'user_id' => $user_id, 'add_time' => time()));
     ...
     * @return array
     */
    public function get_spec($goods_id,$store_id = 0)
    {
    ...
    public function get_siblings_cate($cat_id)
    {
        if (empty($cat_id))
            return array();
        $cate_info = M('goods_red_category')->where("id", $cat_id)->find();
        $siblings_cate = M('goods_red_category')->where(['id'=>['<>',$cat_id],'parent_id'=>$cate_info['parent_id']])->select();
    ...
    {
        return M('goods_red')->where(['goods_id'=>['<>',$goods['goods_id']],'cat_id'=>['<>',$goods['cat_id']],'is_on_sale'=>1,'is_check'=>1])->limit(12)->select();
    }
    ...
    function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5)
    {
        ...
        $goods_id_str = implode(',', $goods_id_arr);
        $goods_id_str = $goods_id_str ? $goods_id_str : '0';
        $priceList = M('goods_red')->where("goods_id", "in", $goods_id_str)->getField('shop_price', true);  //where("goods_id in($goods_id_str)")->select();
        rsort($priceList);
    ...
    function get_filter_menu($filter_param, $action)
    {
        ...
        // 品牌
        if (!empty($filter_param['brand_id'])) {
            $brand_list = M('brand_red')->getField('id,name');
        ...
        if (!empty($filter_param['spec'])) {
            $spec = M('spec_red')->getField('id,name');
            $spec_item = M('spec_red_item')->getField('id,item');
        ...
        if (!empty($filter_param['attr'])) {
            $goods_attribute = M('goods_red_attribute')->getField('attr_id,attr_name');
    ...
    function get_goods_cate(&$goodsCate)
    {
        ...
        if ($goodsCate['level'] == 1) {
            ...
            $parent = M('GoodsRedCategory')->where("id", $goodsCate['parent_id'])->order('`sort_order` desc')->find();//父类
    ...
    function getGoodsIdByBrandPrice($brand_id, $price)
    {
        ...
        if ($brand_id) // 品牌查询
        {
            ...
            $brand_select_goods = M('goods_red')->whereIn('brand_id',$brand_id_arr,'or')->getField('goods_id', true);
        }
        if ($price)// 价格查询
        {
            $price    = explode('-', $price);
            ...
            $price_select_goods = M('goods_red')->where($price_where)->getField('goods_id', true);
        }
    ...
    function getGoodsIdBySpec($spec)
    {
        ...
        $sql = "select * from (select *,concat('_',`key`,'_') as key2 from __PREFIX__spec_red_goods_price as a) b  $where";
        $result = Db::query($sql);
    ...
    function getGoodsDispatching($goods_id,$region_id)
    {
        ...
        $goods = M('goods_red')->where(array('goods_id'=>$goods_id))->find();
        //检查商品是否包邮
    ...
    function getFilterSelected($sel ,$cat_id = array(1)){
        $where = " 1 = 1 ";
        $Goods = M('goods_red')->where("cat_id" ,"in" ,implode(',', $cat_id));
        //查看全部
        if($sel == 'selall'){
            $where .= '';
        }
    ...
    public function add_visit_log($user_id,$goods){
        $record = M('goods_red_visit')->where(array('user_id'=>$user_id,'goods_id'=>$goods['goods_id']))->find();
        if($record){
            M('goods_red_visit')->where(array('user_id'=>$user_id,'goods_id'=>$goods['goods_id']))->save(array('visittime'=>time()));
        }else{
            $visit = array('user_id'=>$user_id,'goods_id'=>$goods['goods_id'],'visittime'=>time(),'cat_id'=>$goods['cat_id'],'extend_cat_id'=>$goods['extend_cat_id']);
            M('goods_red_visit')->add($visit);
        }
    }
    ...
    {
        $collect = M('goods_red_collect')->where(['user_id' => $user_id, 'goods_id' => $goods_id])->find();
        return $collect ? 1 : 0;
    }
    /**
     * 获取促销商品数据
    ...
    public function getPromotionGoods()
    {
        ...
        $promotion_goods = M('goods_red')
            ->alias('g')
            ->field('g.goods_id,g.goods_name,f.price AS shop_price,f.end_time')
            ->join('__FLASH_RED_SALE__ f','g.goods_id = f.goods_id')
            ->where($goods_where)
            ->limit(3)
            ->select();
...
//        $goods_where['goods_name'] = array("exp", " NOT REGEXP '华为|荣耀|小米|合约机|三星|魅族' ");//临时屏蔽,苹果APP审核过了之后注释
    ...
    public function getFlashSaleGoods($count, $page = 1, $start_time=0, $end_time=0)
    {
        ...
        $flash_sale_goods = M('flash_red_sale')->alias('f')
            ...
            ->join('__GOODS_RED__ g','g.goods_id = f.goods_id')
            ->where($where)
    ...
    {
        $goods = M('goods_red')->field('cat_id')->where('goods_id', $goods_id)->find();
        if (empty($goods)) {
            return [];
        }

        $where = ['goods_id' => ['<>', $goods_id], 'cat_id' => $goods['cat_id']];
    	$goods_list = M('goods_red')->field("goods_id,goods_name,shop_price,is_virtual")
    ...
    public function integralMall($rank, $user_id, $p = 1)
    {
        ...
        $goods_list_count = M('goods_red')->where($goods_where)->count();   //总数
        $goods_list = M('goods_red')->field('goods_id,goods_name,shop_price,market_price,exchange_integral,is_virtual')
    ...
    function getSortBrands()
    {
        ...
        $brandIdArr =  M("Brand_red")->where("name in (select `name` from `".C('database.prefix')."brand_red` group by name having COUNT(id) > 1)")->getField('id,cat_id');
        $goodsCategoryArr = M('goodsRedCategory')->where("level = 1")->getField('id,name');
        $nameList = array();
    ...
    public function getOrderPromSimpleInfo($goods)
    {
        ...
        $data = [];
        $po = M('prom_red_order')->where(['start_time' => ['<=', $cur_time], 'end_time' => ['>', $cur_time], 'is_close' => 0])->select();
        if (!empty($po)) {
            foreach ($po as $p) {
    ...
    {
        $categoryList =  M("GoodsRedCategory")->getField('id,name,parent_id,level');
        $nameList = array();
