 ...
 * @package admin\Logic
 */
class YxypFlashSaleLogic extends Prom
{
    ...
    public function __construct($goods, $specGoodsPrice)
    {
        ...
        if($this->specGoodsPrice){
            //活动商品有规格，规格和活动是一对一
            $this->flashSale = FlashYxypSale::get($specGoodsPrice['prom_id']);
        }else{
            //活动商品没有规格，活动和商品是一对一
            $this->flashSale = FlashYxypSale::get($goods['prom_id']);
        }
        if ($this->flashSale) {
            ...
            if ($this->checkActivityIsEnd() && $this->flashSale['is_end'] == 0) {
                if($this->specGoodsPrice){
                    ...
                    Db::name('goods_yxyp')->where("goods_id", $this->flashSale['goods_id'])->save(['prom_type' => 0, 'prom_id' => 0]);
                }
    ...
    public function getUserFlashOrderGoodsNum($user_id){
        ...
        $order_id_arr = Db::name('order_yxyp')->where($orderWhere)->getField('order_id', true);
        if ($order_id_arr) {
            $orderGoodsWhere = ['prom_id' => $this->flashSale['id'], 'prom_type' => 1, 'order_id' => ['in', implode(',', $order_id_arr)]];
            $goods_num = DB::name('order_yxyp_goods')->where($orderGoodsWhere)->sum('goods_num');
            if($goods_num){
                return $goods_num;
            }else{
                return 0;
