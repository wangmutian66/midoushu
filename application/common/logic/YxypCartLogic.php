 ...
 * @package Home\Logic
 */
class YxypCartLogic extends Model
{
    ...
    public function buyNow(){
        ...
        $buyGoods = [
            ...
            'prom_type'          => 0,                        // 0 普通订单,1 限时抢购, 2 团购 , 3 促销优惠 5 满包邮
            'prom_id'            => 0,                        // 活动id
            'free_money'         => 0,
        ...
        if(empty($this->specGoodsPrice)){
            ...
            $buyGoods['cost_operating']     = $this->specGoodsPrice['cost_operating']; //运营成本
        }
    ...
    public function returnbuyNow(){
        ...
        if(empty($this->specGoodsPrice)){
            echo $specGoodsPriceCount = Db::name('SpecGoodsPrice')->where("goods_id", $this->goods['goods_id'])->count('item_id');
            if($specGoodsPriceCount > 0){
                return ['status' => 0, 'msg' => '必须传递商品规格', 'result' => ''];
            }
            $prom_type = $this->goods['prom_type'];
    ...
    public function addGoodsToCart()
    {
        ...
        $userCartCount = Db::name('cart_yxyp')->where(['user_id'=>$this->user_id,'session_id'=>$this->session_id])->count();//获取用户购物车的商品有多少种
    ...
    public function addReturnGoodsToCart()
    {
        if(empty($this->goods)){
            return ['status'=>-3,'msg'=>'购买商品不存在','result'=>''];
        }
        if($this->goods['exchange_integral'] > 0){
            return ['status'=>0,'msg'=>'积分商品跳转','result'=>['url'=>U('ReturnGoods/goodsInfo',['id'=>$this->goods['goods_id'],'item_id'=>$this->specGoodsPrice['item_id']],'',true)]];
        }
    ...
    private function addNormalCart(){
        ...
        // 查询购物车是否已经存在这商品
        if (!$this->user_id) {
            $userCartGoods = CartYxyp::get(['user_id'=>$this->user_id,'session_id'=>$this->session_id,'goods_id'=>$this->goods['goods_id'],'spec_key'=>($this->specGoodsPrice['key'] ?: '')]);
        } else {
            $userCartGoods = CartYxyp::get(['user_id'=>$this->user_id,'goods_id'=>$this->goods['goods_id'],'spec_key'=>($this->specGoodsPrice['key'] ?: '')]);
        }
        // 如果该商品已经存在购物车
        if ($userCartGoods) {
            ...
            $cartAddData = array(
                ...
                'goods_num'          => $this->goodsBuyNum,          // 购买数量
                'add_time'           => time(),                      // 加入购物车时间
                'prom_type'          => $this->goods['prom_type'],   // 0 普通订单,1 限时抢购, 2 团购 , 3 促销优惠 5 满包邮
            ...
            $cartResult = Db::name('CartYxyp')->insertGetId($cartAddData);
        }
        if($cartResult !== false){
    ...
    private function addFlashSaleCart(){
        ...
        if(!$flashSaleIsAble){
            //活动没有进行中，走普通商品下单流程
            return array('status' => -1, 'msg' => '该商品必须在活动期间才能加入到购物车', 'result' => '');
        ...
        if (!$this->user_id) {
            ...
            $userCartYxypGoods = CartYxyp::get(['user_id'=>$this->user_id,'goods_id'=>$this->goods['goods_id'],'spec_key'=>($this->specGoodsPrice['key'] ?: '')]);
        }
        ...
        if($userCartGoods){
            ...
            $cartResult = Db::name('CartYxyp')->insert($cartAddFlashSaleData);
        }
        if($cartResult !== false){
            return array('status' => 1, 'msg' => '成功加入购物车', 'result' => '');
        }else{
    ...
    private function addPromGoodsCart(){
        ...
        if ($userCartGoods) {
            ...
            $cartResult = Db::name('CartYxyp')->insert($cartAddData);
        }
        if($cartResult !== false){
            return array('status' => 1, 'msg' => '成功加入购物车', 'result' => '');
        }else{
    ...
    {
        if ($this->user_id) {
            $goods_num = Db::name('cart_yxyp')->where(['user_id' => $this->user_id])->sum('goods_num');
        } else {
            $goods_num = Db::name('cart_yxyp')->where(['session_id' => $this->session_id])->sum('goods_num');
        }
    ...
    public function getUserCartGoodsTypeNum($t)
    {
        if($t == 1){
            if ($this->user_id) {
                $goods_num = Db::name('cart_yxyp')->where(['user_id' => $this->user_id])->count();
            } else {
                $goods_num = Db::name('cart_yxyp')->where(['session_id' => $this->session_id])->count();
            }
        } else {
            if ($this->user_id) {
                $goods_num = Db::name('cart_yxyp')->where(['user_id' => $this->user_id])->count();
   ...
   //  dump($cartList);die;
     ...
     * @return false|\PDOStatement|string|\think\Collection
     */
    public function getCartListPart($cartIds,$selected = 0){
        $cart = new CartYxyp();
        // 如果用户已经登录则按照用户id查询
        if ($this->user_id) {
            $cartWhere['user_id'] = $this->user_id;
   ...
   //  dump($cartList);die;
    ...
    public function getReturnCartList($selected = 0){
        $cart = new CartYxyp();
        // 如果用户已经登录则按照用户id查询
        if ($this->user_id) {
            $cartWhere['user_id'] = $this->user_id;
     ...
     * @return int|string
     */
    public function getUserCartOrderCount($t){
        if($t == 1)
            $count = Db::name('CartYxyp')->where(['user_id' => $this->user_id , 'selected' => 1])->count();
        else
            $count = Db::name('CartYxyp')->where(['user_id' => $this->user_id ,  'selected' => 1])->count(); 
        return $count;
    }
    ...
    public function doUserLoginHandle()
    {
        ...
        if (!empty($cart_id_arr)) {
            ...
            M('cart_yxyp')->delete($cart_id_arr); // 删除购物车完全相同的商品
        }
    }
    /**
     * 更改购物车的商品数量
    ...
    public function delete($cart_ids = array()){
        ...
        $delete = Db::name('cart_yxyp')->where($cartWhere)->where('id','IN',$cart_ids)->delete();
        return $delete;
    }
    /**
     *  更新购物车，并返回计算结果
