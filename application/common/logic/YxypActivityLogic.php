 ...
 * 活动逻辑类
 */
class YxypActivityLogic extends Model
{
    ...
    public function getGroupBuyCount()
    {
        ...
        $count = M('group_Yxyp_buy')->alias('b')
                ->field('b.goods_id,b.rebate,b.virtual_num,b.buy_num,b.title,b.goods_price,b.end_time,b.price,b.order_num,g.comment_count')
                ->join('__GOODS_YXYP__ g', 'b.goods_id=g.goods_id AND g.prom_type=2 AND g.is_on_sale=1 AND g.is_check=1')
    ...
    public function getCouponList($atype, $user_id, $p = 1)
    {
        ...
        $coupon_list = M('coupon_yxyp')->field("*,send_end_time-'$time' as spacing_time")
            ->where($where)->page($p, 15)->order($order)->select();
        if (is_array($coupon_list) && count($coupon_list) > 0) {
            if ($user_id) {
                $user_coupon = M('coupon_yxyp_list')->where(['uid' => $user_id, 'type' => 2])->getField('cid',true);
            }
    ...
    public function getCouponQuery($queryType, $user_id, $type = 0, $orderBy = null , $order_money = 0)
    {
        ...
        $query = M('coupon_yxyp_list')->alias('l')
            ->join('__COUPON_YXYP__ c','l.cid = c.id'.$condition)
            ->where($where);
    ...
    public function getCouponCenterList($cat_id, $user_id, $p = 1)
    {
        ...
        $query = M('coupon_yxyp')->alias('c')
            ...
            ->order('spacing_time', 'asc');
        if ($cat_id > 0) {
            $query = $query->join('__GOODS_YXYP_COUPON__ gc', 'gc.coupon_id=c.id AND gc.goods_category_id='.$cat_id);
        }
        ...
        $user_coupon = [];
        if ($user_id) {
            $user_coupon = M('coupon_yxyp_list')->where(['uid' => $user_id, 'type' => 2])->column('cid');
        }
    ...
    {
        $list = M('coupon_yxyp')->alias('c')
                ->join('__GOODS_YXYP_COUPON__ gc', 'gc.coupon_id=c.id AND gc.goods_category_id!=0')
                ->where(['type' => 2, 'status' => 1])
                ->column('gc.goods_category_id');
        
        $result = M('goods_yxyp_category')->field('id, mobile_name')->where("id", "IN", $list)->page($p, $num)->select();
        $result = $result ?: [];
    ...
    public function get_coupon($id, $user_id)
    {
        ...
        if ($user_id) {
            ...
            $coupon_info = M('coupon_yxyp')->where(array('id' => $id, 'status' => 1))->find();
            if (empty($coupon_info)) {
                ...
                if (M('coupon_yxyp_list')->where(array('cid' => $id, 'uid' => $user_id))->find()) {
                    //已经领取过
                    ...
                    M('coupon_yxyp_list')->add($data);
                    M('coupon_yxyp')->where(array('id' => $id, 'status' => 1))->setInc('send_num');
    ...
    public function getGoodsPromSimpleInfo($user, &$goods)
    {
        ...
        $goods  = M('GoodsYxyp')->where('goods_id', $goods['goods_id'])->find();
    ...
    {
        $cur_time = time();
        $sql = "select * from __PREFIX__prom_yxyp_order where start_time <= $cur_time AND end_time > $cur_time";
        $data = [];
        $po = Db::query($sql);
        if (!empty($po)) {
            foreach ($po as $p) {
    ...
    public function getOrderPayProm($order_amount=0)
    {
        ...
        $cur_time = time();
        $sql = "select * from __PREFIX__prom_yxyp_order where type<2 and start_time <= $cur_time "
