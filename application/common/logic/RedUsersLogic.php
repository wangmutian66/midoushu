 ...
 * @package Home\Logic
 */
class RedUsersLogic extends Model
{
    protected $user_id=0;
    ...
    public function oauth_bind_new($user = array())
    {
        ...
        if($unionid){ 
            ...
            if($thirdUser && !empty($thirdUser['unionid'])){ 
                return array('status'=>-1,'msg'=>'此'.$oauthCN.'已绑定其它账号','result'=>'');
            }
    ...
    public function get_info($user_id)
    {
        ...
        $activityLogic = new \app\common\logic\RedActivityLogic;             //获取能使用优惠券个数
        $user['coupon_count'] = $activityLogic->getUserCouponNum($user_id, 0);
        
        $user['collect_count'] = $this->getGoodsCollectNum($user_id);; //获取收藏数量


        $user['return_count'] = M('return_red_goods')->where("user_id=$user_id and status<3 and status>-1")->count();   //退换货数量
        $user['waitPay']     = M('order_red')->where("user_id = :user_id ".C('WAITPAY'))->bind(['user_id'=>$user_id])->count(); //待付款数量
        $user['waitSend']    = M('order_red')->where("user_id = :user_id ".C('WAITSEND'))->bind(['user_id'=>$user_id])->count(); //待发货数量
        $user['waitReceive'] = M('order_red')->where("user_id = :user_id ".C('WAITRECEIVE'))->bind(['user_id'=>$user_id])->count(); //待收货数量
    ...
    public function getApiUserInfo($user_id)
    {
        ...
        $user['collect_count'] = $this->getGoodsCollectNum($user_id);; //获取收藏数量
        $user['visit_count']   = M('goods_red_visit')->where('user_id', $user_id)->count();   //商品访问记录数
        $user['return_count'] = M('return_red_goods')->where("user_id=$user_id and status<2")->count();   //退换货数量
    ...
    public function get_last_order($user_id){
        $last_order = M('order_red')->where("user_id", $user_id)->order('order_id DESC')->find();
        return $last_order;
    }
    ...
    public function get_order_goods($order_id){
        ...
        $sql = "SELECT og.*,g.commission,rg.status as rstatus FROM __PREFIX__order_red_goods og LEFT JOIN __PREFIX__goods_red g ON g.goods_id = og.goods_id LEFT JOIN __PREFIX__return_red_goods rg ON rg.goods_id = og.goods_id and rg.order_id = og.order_id and rg.spec_key = og.spec_key WHERE og.order_id = :order_id";
        ...
        foreach ($goods_list as $k => $val) {
            // 米豆换算
            if(isset($val['rstatus'])){
                switch ($val['rstatus']) {
                    case '-2':
                        $val['rstatus_str'] = "用户取消退换服务";
                        break;
                    case '-1':
                        $val['rstatus_str'] = "退换审核不通过";
                        break;
                    case '0':
                        $val['rstatus_str'] = "退换待审核";
                        break;
                    case '1':
                        $val['rstatus_str'] = "退换通过申请";
                        break;
                    case '2':
                        $val['rstatus_str'] = "退换已发货";
                        break;
                    case '3':
                        $val['rstatus_str'] = "退换已完成";
                        break;
                    default:
                        $val['rstatus_str'] = "退换通过申请";
                        break;
    ...
    {
        $activityLogic = new \app\common\logic\RedActivityLogic;
        ...
        $return['msg'] = '获取成功';
        $return['result'] = $list;
        $return['show'] = $page->show_cxpc();
        return $return;
    }

    public function getGoodsCollectNum($user_id)
    {
        $count = M('goods_red_collect')->alias('c')
                ->join('goods_red g','g.goods_id = c.goods_id','INNER')
    ...
    public function get_goods_collect($user_id){
        ...
        //获取我的收藏列表
        $result = M('goods_red_collect')->alias('c')
            ->field('c.collect_id,c.add_time,g.goods_id,g.goods_name,g.shop_price,g.market_price,g.is_on_sale,g.store_count,g.cat_id,g.is_virtual')
            ->join('goods_red g','g.goods_id = c.goods_id','INNER')
        ...
        foreach ($result as $k => $val) {
            ...
            $result[$k]['midou']       = $midouInfo['midou'];
            $result[$k]['midou_money'] = $midouInfo['midou_money'];
            $result[$k]['midou_index'] = $midouInfo['midou_index'];
        }
    ...
    public function get_comment($user_id,$status=2){
        if($status == 1){
            ...
            $commented_count = Db::name('comment')
                ->alias('c')
                ->join('__ORDER_RED_GOODS__ g','c.goods_id = g.goods_id and c.order_id = g.order_id', 'inner')
                ->where('c.user_id',$user_id)
                ->count();
            $page = new Page($commented_count,10);
            $comment_list = Db::name('comment')
                ->alias('c')
                ->field('c.*,g.*,(select order_sn from  __PREFIX__order_red where order_id = c.order_id ) as order_sn')
                ->join('__ORDER_RED_GOODS__ g','c.goods_id = g.goods_id and c.order_id = g.order_id', 'inner')
                ->where('c.user_id',$user_id)
            ...
            $comment_count = Db::name('order_red_goods')->alias('og')->join('__ORDER_RED__ o','o.order_id = og.order_id','left')->where($comment_where)->count();
            $page = new Page($comment_count,10);
            $comment_list = Db::name('order_red_goods')
                ->alias('og')
                ->join('__ORDER_RED__ o','o.order_id = og.order_id','left')
                ->where($comment_where)
    ...
    public function add_comment($add){
        ...
        //检查订单是否已完成
        $order = M('order_red')->field('order_status')->where("order_id", $add['order_id'])->where('user_id', $add['user_id'])->find();
        if($order['order_status'] != 2)
            return array('status'=>-1,'msg'=>'该笔订单还未确认收货','result'=>'');

        //检查是否已评论过
        $goods = M('comment_red')->where(['order_id'=>$add['order_id'],'goods_id'=>$add['goods_id']])->find();
        if($goods)            
        ...
        {
            //更新订单商品表状态
            M('order_red_goods')->where(array('goods_id'=>$add['goods_id'],'order_id'=>$add['order_id']))->save(array('is_comment'=>1));
            M('goods_red')->where(array('goods_id'=>$add['goods_id']))->setInc('comment_count',1); // 评论数加一
            // 查看这个订单是否全部已经评论,如果全部评论了 修改整个订单评论状态            
            $comment_count   = M('order_red_goods')->where("order_id", $add['order_id'])->where('is_comment', 0)->count();
            if($comment_count == 0) // 如果所有的商品都已经评价了 订单状态改成已评价
            {
                M('order_red')->where("order_id",$add['order_id'])->save(array('order_status'=>4));
            }
    ...
    public function check_validate_code($code, $sender, $type ='email', $session_id=0 ,$scene = -1){
        ...
        if($type == 'email'){            
            ...
            if($scene == -1){
                ...
                $user = M('users')->where("email", $sender)->whereOr('mobile', $sender)->find();
                if ($user) {
     ...
     * @param $user_id  用户id
     */
    public function wechat_get_goods_collect($user_id){
        $count = $this->getGoodsCollectNum($user_id);
        $page = new Page($count,10);
        $show = $page->show_cxpc();
         $result = M('goods_red_collect')->alias('c')
        ...
        foreach ($result as $keys => $values) {
            if (strstr(goods_thum_images($values['goods_id'],400,400,'red'),'http')) {
                 $result[$keys]['goods_image']=goods_thum_images($values['goods_id'],400,400,'red');
            }else{
                 $result[$keys]['goods_image']=URL.goods_thum_images($values['goods_id'],400,400,'red');
            }
            // 米豆换算
            $midouInfo = getMidou($values['goods_id']);
            $result[$keys]['midou']       = $midouInfo['midou'];
            $result[$keys]['midou_money'] = $midouInfo['midou_money'];
            $result[$keys]['midou_index'] = $midouInfo['midou_index'];
        }
        $return['status'] = 1;
        $return['msg'] = '获取成功';
        $return['result'] = $result;
