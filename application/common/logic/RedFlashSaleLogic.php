 ...
 * @package admin\Logic
 */
class RedFlashSaleLogic extends Prom
{
    ...
    public function __construct($goods, $specGoodsPrice)
    {
        ...
        if($this->specGoodsPrice){
            //活动商品有规格，规格和活动是一对一
            $this->flashSale = FlashRedSale::get($specGoodsPrice['prom_id']);
        }else{
            //活动商品没有规格，活动和商品是一对一
            $this->flashSale = FlashRedSale::get($goods['prom_id']);
        }
        if ($this->flashSale) {
            //每次初始化都检测活动是否结束，如果失效就更新活动和商品恢复成普通商品
            if ($this->checkActivityIsEnd() && $this->flashSale['is_end'] == 0) {
                if($this->specGoodsPrice){
                    Db::name('spec_red_goods_price')->where('item_id', $this->specGoodsPrice['item_id'])->save(['prom_type' => 0, 'prom_id' => 0]);
                    $goodsPromCount = Db::name('spec_red_goods_price')->where('goods_id', $this->specGoodsPrice['goods_id'])->where('prom_type','>',0)->count('item_id');
                    if($goodsPromCount == 0){
                        Db::name('goods_red')->where("goods_id", $this->specGoodsPrice['goods_id'])->save(['prom_type' => 0, 'prom_id' => 0]);
                    }
                    unset($this->specGoodsPrice);
                    $this->specGoodsPrice = SpecRedGoodsPrice::get($specGoodsPrice['item_id']);
                }else{
                    Db::name('goods_red')->where("goods_id", $this->flashSale['goods_id'])->save(['prom_type' => 0, 'prom_id' => 0]);
                }
                $this->flashSale->is_end = 1;
                $this->flashSale->save();
                unset($this->goods);
                $this->goods = GoodsRed::get($goods['goods_id']);
            }
        }
    }
    ...
    public function getUserFlashOrderGoodsNum($user_id){
        ...
        $order_id_arr = Db::name('order_red')->where($orderWhere)->getField('order_id', true);
        if ($order_id_arr) {
