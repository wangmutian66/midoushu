 ...
 * @package admin\Logic
 */
class RedGroupBuyLogic extends Prom
{
    ...
    public function __construct($goods,$specGoodsPrice)
    {
        ...
        if($this->specGoodsPrice){
            //活动商品有规格，规格和活动是一对一
            $this->GroupBuy = GroupRedBuy::get($specGoodsPrice['prom_id']);
        }else{
            //活动商品没有规格，活动和商品是一对一
            $this->GroupBuy = GroupRedBuy::get($this->goods['prom_id']);
        }
        if ($this->GroupBuy) {
            ...
            if ($this->checkActivityIsEnd() && $this->GroupBuy['is_end'] == 0) {
                if($this->specGoodsPrice){
                    ...
                    Db::name('goods_red')->where("goods_id", $this->GroupBuy['goods_id'])->save(['prom_type' => 0, 'prom_id' => 0]);
                }
