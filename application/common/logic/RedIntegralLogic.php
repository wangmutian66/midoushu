...
class IntegralLogic extends Model
{
    ...
    public function buy()
    {
        ...
        if (empty($this->specGoodsPrice)) {
            $goods_spec_list = SpecRedGoodsPrice::all(['goods_id' => $this->goods['goods_id']]);
    ...
    public function addOrder($invoice_title,$user_note){
        ...
        Db::name('order_red_action')->add($action_info);
        $orderGoodsData = [
            'order_id'  => $order['order_id'],
