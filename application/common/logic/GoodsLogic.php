...
class GoodsLogic extends Model
{
     ...
     * @return array
     */
    public function collect_goods_yxyp($user_id, $goods_id)
    {
        if (!is_numeric($user_id) || $user_id <= 0) return array('status' => -1, 'msg' => '必须登录后才能收藏', 'result' => array());
    ...
    {
        return M('goods')->where(['goods_id'=>['<>',$goods['goods_id']],'cat_id'=>['<>',$goods['cat_id']],'is_on_sale'=>1,'is_check'=>1])->limit(12)->select();
    }
    ...
    function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5)
    {
        ...
        for ($i = 0; $i < $c; $i++) {
            ...
            if ($i == 0)
                $parr[] = array('value' => "{$end}元以下", 'p' => $filter_param['price'] ,'href' => urldecode(U("Goods/$action", $filter_param, '')));
            elseif($i == ($c-1) && ($max_price > $end))
                $parr[] = array('value' => "{$end}元以上", 'p' => $filter_param['price'] , 'href' => urldecode(U("Goods/$action", $filter_param, '')));
            else
                $parr[] = array('value' => "{$start}-{$end}元", 'p' => $filter_param['price'] , 'href' => urldecode(U("Goods/$action", $filter_param, '')));
        }
        return $parr;
    }
    ...
    function getParentRegionList($cid){
        //$pids = '';
        $pids = array();
        $parent_id =  M('region')->where(array('id'=>$cid))->cache(true)->getField('parent_id');
        if($parent_id != 0){
            //$pids .= $parent_id;
    ...
    {
        if($redisGoodsDispatchingCache = S("redisGoodsDispatchingCache_{$goods_id}_{$region_id}")){
            return $redisGoodsDispatchingCache;
        }
        $return_data = array('status'=>1,'msg'=>'');
        $goods = M('goods')->where(array('goods_id'=>$goods_id))->cache(true)->find();
        //检查商品是否包邮
        ...
        if(empty($goods['shipping_area_ids'])){
            $plugin_goods_shipping = M('plugin')->where(array('type'=>'shipping','status'=>1,'is_default'=>1,'suppliers_id'=>$goods['suppliers_id']))->cache(true)->select();
        ...
        $find_shipping_area_id = M('area_region')->where(array('region_id'=>array('in',$parent_region_id)))->group('shipping_area_id')->cache(true)->getField('shipping_area_id',true);
        ...
        if(count($shipping_area_id_array) == 0){
            $goods_shipping = M('shipping_area')->where(array('shipping_area_id'=>array('in',$goods_shipping_area_id_array),'is_default'=>1,'suppliers_id'=>$goods['suppliers_id']))->cache(true)->select();
            ...
            if(!empty($goods_shipping)){
                foreach($goods_shipping as $k=>$v){
                    ...
                    $goods_shipping[$k]['shipping_name'] = M('plugin')->where(array('type'=>'shipping','code'=>$goods_shipping[$k]['shipping_code']))->cache(true)->getField('name');
                }
                $return_data['msg'] = '可配送';
                $return_data['result'] = $goods_shipping;
                return $return_data;
        ...
        S("redisGoodsDispatchingCache_{$goods_id}_{$region_id}",$return_data);
        return $return_data;
    }
     ...
     * @return mixed
     */
    function getFilterSelected($sel ,$cat_id = array(1),$goods_table_name='goods'){
        $where = " 1 = 1 ";
        $Goods = M($goods_table_name)->where("cat_id" ,"in" ,implode(',', $cat_id));
        //查看全部
        if($sel == 'selall'){
            $where .= '';
        }
    ...
    public function getFreight($shipping_code, $province, $city, $district, $weight)
    {
        ...
        $redisFreightData = S("redisFreightData_{$shipping_code}_{$province}_{$city}_{$district}_{$weight}");
        if($redisFreightData){
            return $redisFreightData;
        }
        ...
        $shipping_config = M('ShippingArea')->where("shipping_area_id", $shipping_area_id)->cache(true)->getField('config');
        ...
        S("redisFreightData_{$shipping_code}_{$province}_{$city}_{$district}_{$weight}",$freight);
        return $freight;
    }
