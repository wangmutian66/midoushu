...
class OrderLogic
{
	...
	public function cancel_order($user_id,$order_id,$action_note='您取消了订单'){
		...
		if($order['user_money'] > 0 || $order['integral'] > 0){
			accountLog($user_id,$order['user_money'],0,$order['integral'],"订单取消，退回{$order['user_money']}元,{$order['integral']}积分",0,0,$order['order_id'],$order['order_sn']);
		}
		...
		$row = M('order')->where(array('order_id'=>$order_id,'user_id'=>$user_id))->save(array('order_status'=>3,'cancel_time'=>time()));
        ...
        //2018-10-12 张洪凯 取消订单成功发送微信模板消息提醒
        $wechat = new \app\common\logic\WxLogic;
        $wechat->sendTemplateMsgOnCancelOrder($order);
    ...
    public function addReturnStoreOrder($user_id,$address_id,$car_id,$suppliers_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price=[],$store_price=[],$user_note=[],$pay_name='')
    {
        // 0插入订单 order
        $address = M('UserAddress')->where("address_id", $address_id)->find();
        ...
        foreach ($car_id as $key) {
            if($shipping_code[$key]){
                $shipping = M('Plugin')->where("code ='".$shipping_code[$key]."' AND is_default = 1 AND suppliers_id = ".$suppliers_id[$key])->cache(true,TPSHOP_CACHE_TIME)->find();
            } else {
                $shipping = array('name'=>'包邮');
                $shipping_code[$key] = '';
            } 
            ...
            $data = array(
                    'order_sn'          => $order_sn, // 订单编号
                    'user_id'           => $user_id, // 用户id
                    'suppliers_id'      => $suppliers_id[$key], // 供货商id
                    'consignee'         => $address['consignee'], // 收货人
                    'province'          => $address['province'],//'省份id',
                    'city'              => $address['city'],//'城市id',
                    'district'          => $address['district'],//'县',
            ...
            if($this->action == 'buy_now'){
                ...
                $cartList = M('Cart')->where(['user_id'=>$user_id,'selected'=>1,'id'=>$key])->select();
            }
            foreach($cartList as $k => $val)
            {
                $goods = M('goods')->where("goods_id", $val['goods_id'])->cache(true,TPSHOP_CACHE_TIME)->find();
            ...
            // 4 删除已提交订单商品
            if($this->action != 'buy_now'){
                M('Cart')->where(['user_id' => $user_id,'selected' => 1,'id'=>$key])->delete();
            }       
