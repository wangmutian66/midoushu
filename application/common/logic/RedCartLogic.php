 ...
 * @package Home\Logic
 */
class RedCartLogic extends Model
{
    protected $goods;          //商品模型
    protected $specGoodsPrice; //商品规格模型
    protected $specGoodsId;    //商品规格ID
    ...
    {
        $specGoodsPriceModel  = new SpecRedGoodsPrice();
        $this->specGoodsPrice = $specGoodsPriceModel::get($item_id,'',10);
    }

    public function setSpecGoodsIdModel($item_id)
    {
        $this->specGoodsId = $item_id;
    }
    ...
    public function buyNow(){
        ...
        $user_goods_num = $this->getUserOrderGoodsNum($this->user_id);
        /*米豆商城限购程序*/
        if($this->goods->purchase > 0){
            if( ($this->goodsBuyNum + $user_goods_num) > $this->goods->purchase){
                return array('status' => 0, 'msg' => '每人限购'. $this->goods->purchase .'件', 'result' => '');
            }
        }
        
        $midou_use_percent = tpCache('shoppingred.midou_use_percent'); // 购买商品 使用米豆 比率
        $midou_rate        = tpCache('shoppingred.midou_rate');        // 米豆兑换比
        if($this->goods['is_z_change'] != 1) $midou_use_percent = $this->goods['midou_use_percent'];
        // 米豆换算
        if($this->specGoodsId > 0){
            // 米豆换算
            $midouInfo = getMidou($this->goods['goods_id'],$this->specGoodsId);
            $this->goods['midou']       = $midouInfo['midou'];
            $this->goods['midou_money'] = $midouInfo['midou_money'];
        } else {
            // 米豆换算
            $midouInfo = getMidou($this->goods['goods_id']);
            $this->goods['midou']       = $midouInfo['midou'];
            $this->goods['midou_money'] = $midouInfo['midou_money'];
        }
        ...
        $buyGoods = [
            ...
            'midou'              => $this->goods['midou'],       // 米豆
            'midou_money'        => $this->goods['midou_money'], // 部分金额
            ...
            'cost_operating'     => $this->goods['cost_operating'],
            'midou_use_percent'  => $this->goods['midou_use_percent'],
            'midou_rate'         => $midou_rate,
        ];

        if(empty($this->specGoodsPrice)){
            $specGoodsPriceCount = Db::name('SpecRedGoodsPrice')->where("goods_id", $this->goods['goods_id'])->count('item_id');
            ...
            if($this->store_id != 0){
                $store_count    = db('store_goods_stock')->where(['store_id'=>$this->store_id,"goods_id"=>$this->goods["goods_id"]])->value("stock");
            ...
            $buyGoods['cost_price']         = $this->specGoodsPrice['cost_price']; //成本
            $buyGoods['cost_operating']     = $this->specGoodsPrice['cost_operating']; //

            if($this->store_id != 0) {
                $store_count = db('store_goods_stock')->where([
     ...
     * @return float|int
     */
    public function getUserOrderGoodsNum($user_id){
        $orderWhere = [
            'user_id'=>$user_id,
            'order_status' => ['<>', 3],
            'add_time' =>   ['gt',strtotime("-6 month")],   //大于半年
        ];
        $order_id_arr = Db::name('order_red')->where($orderWhere)->where('order_status','<>',5)->getField('order_id', true);
       
        if ($order_id_arr) {
            $orderGoodsWhere = ['goods_id' => $this->goods['goods_id'], 'order_id' => ['in', implode(',', $order_id_arr)]];
            $goods_num = DB::name('order_red_goods')->where($orderGoodsWhere)->sum('goods_num');
            if($goods_num){
                return $goods_num;
            }else{
                return 0;
    ...
    public function addGoodsToCart()
    {
        ...
        $userCartCount = Db::name('cart_red')->where(['user_id'=>$this->user_id,'session_id'=>$this->session_id])->count();//获取用户购物车的商品有多少种
        ...
        if($this->goods->purchase > 0){
            if( ($this->getUserOrderGoodsNum($this->user_id) + $this->goodsBuyNum) > $this->goods->purchase){
                return array('status' => -1, 'msg' => '每人限购'. $this->goods->purchase .'件', 'result' => '');
            }
        }
        ...
        setcookie('cnred', $UserCartGoodsNum, null, '/');
        return $result;
    }
    ...
    private function addNormalCart(){
        ...
        if(empty($this->specGoodsPrice)){
            ...
            $cost_price     = $this->specGoodsPrice['cost_price'];
            $cost_operating = $this->specGoodsPrice['cost_operating'];
            $store_count    = $this->specGoodsPrice['store_count'];
            if($this->store_id) {
                $store_count = db('store_goods_stock')->where([
                    'store_id' => $this->store_id,
        ...
        // 查询购物车是否已经存在这商品
        if (!$this->user_id) {
            $userCartGoods = CartRed::get(['user_id'=>$this->user_id,'session_id'=>$this->session_id,'goods_id'=>$this->goods['goods_id'],'spec_key'=>($this->specGoodsPrice['key'] ?: '')]);
        } else {
            $userCartGoods = CartRed::get(['user_id'=>$this->user_id,'goods_id'=>$this->goods['goods_id'],'spec_key'=>($this->specGoodsPrice['key'] ?: '')]);
        }
        // 如果该商品已经存在购物车
        if ($userCartGoods) {

            $userWantGoodsNum = $this->goodsBuyNum + $userCartGoods['goods_num'];//本次要购买的数量加上购物车的本身存在的数量
            if($this->goods->purchase > 0 && $userWantGoodsNum > $this->goods->purchase){
                $userWantGoodsNum = $this->goods->purchase;
            }
            //如果有阶梯价格,就是用阶梯价格
            ...
            $cartResult = $userCartGoods->save(['goods_num' => $userWantGoodsNum,'goods_price'=>$price,'member_goods_price'=>$price,'store_id'=>$this->store_id]);
        }else{  
            //如果该商品没有存在购物车
            if($this->goodsBuyNum > $store_count){
            ...
            $cartAddData = array(
                ...
                'suppliers_id'       => $this->goods['suppliers_id'],// 供货商id
                ...
                'cost_operating'     => $cost_operating,             // 运营成本价
                'midou'              => $this->goods['midou'],       // 米豆
                'midou_money'        => $this->goods['midou_money'], // 部分金额
                'goods_num'          => $this->goodsBuyNum,          // 购买数量
                'add_time'           => time(),                      // 加入购物车时间
                'prom_type'          => 0,                           // 0 普通订单,1 限时抢购, 2 团购 , 3 促销优惠
                'prom_id'            => 0,                           // 活动id\
                'store_id'           => $this->store_id,             //实体店id
            );
            
            if($this->specGoodsPrice){
                $cartAddData['item_id']        = $this->specGoodsId;
                $cartAddData['spec_key']       = $this->specGoodsPrice['key'];
                $cartAddData['spec_key_name']  = $this->specGoodsPrice['key_name']; // 规格 key_name
                $cartAddData['cost_price']     = $this->specGoodsPrice['cost_price'];     // 规格 成本
                $cartAddData['cost_operating'] = $this->specGoodsPrice['cost_operating']; // 规格 成本
                $cartAddData['sku']            = $this->specGoodsPrice['sku']; //商品条形码
            }

            $cartResult = Db::name('cart_red')->insertGetId($cartAddData);
        }
        if($cartResult !== false){
    ...
    private function addPromGoodsCart(){
        $promGoodsLogic = new RedPromGoodsLogic($this->goods, $this->specGoodsPrice);
        //活动是否存在，是否关闭，是否处于有效期
        ...
        if ($userCartGoods) {
            ...
            $cartResult = Db::name('CartRed')->insert($cartAddData);
        }
        if($cartResult !== false){
            return array('status' => 1, 'msg' => '成功加入购物车', 'result' => '');
        }else{
    ...
    {
        if ($this->user_id) {
            $goods_num = Db::name('cart_red')->where(['user_id' => $this->user_id])->sum('goods_num');
        } else {
            $goods_num = Db::name('cart_red')->where(['session_id' => $this->session_id])->sum('goods_num');
        }
    ...
    {
        if ($this->user_id) {
            $goods_num = Db::name('cart_red')->where(['user_id' => $this->user_id])->count();
        } else {
            $goods_num = Db::name('cart_red')->where(['session_id' => $this->session_id])->count();
        }
        return empty($goods_num) ? 0 : $goods_num;
    }
     ...
     */
    public function getCartList($selected = 0,$store_id=0){
        $cart = new CartRed();
        // 如果用户已经登录则按照用户id查询
        if ($this->user_id) {
        ...
        $midou_use_percent = tpCache('shoppingred.midou_use_percent'); // 购买商品 使用米豆 比率
        $midou_rate        = tpCache('shoppingred.midou_rate');        // 米豆兑换比
        foreach ($cartCheckAfterList as $k => $val) {
            $goodsInfo = M('goods_red')->where('goods_id',$val['goods_id'])->find();
            if($goodsInfo['is_z_change'] != 1) $midou_use_percent = $goodsInfo['midou_use_percent'];
            $midouInfo = getMidou($val['goods_id'],$val['item_id']);
            //分解其中的单元       $midou midou_money midou_index
            if($midouInfo){
                extract($midouInfo);
            }else{
                //未知错误，跳过本次循环
            ...
            $cartCheckAfterList[$k]['midou_use_percent'] = $midou_use_percent;
            $cartCheckAfterList[$k]['midou_rate']        = $midou_rate;
            $cartCheckAfterList[$k]['midou']             = $midou;
            $cartCheckAfterList[$k]['midou_money']       = $midou_money;
            $cartCheckAfterList[$k]['midou_index']       = $midou_index;
        }
        ...
        setcookie('cnred', $cartGoodsTotalNum, null, '/');
    ...
    public function checkCartList($cartList){
        ...
        foreach($cartList as $cartKey=>$cart){
            //商品未通过审核
            if(empty($cart['goods']) || $cart['goods']['is_check'] != 1){
                $cart->delete();
            ...
            if ($goodsPromFactory->checkPromType($cart['prom_type'])) {
                if (!empty($cart['spec_key'])) {
                    $specGoodsPrice = SpecRedGoodsPrice::get(['goods_id' => $cart['goods_id'], 'key' => $cart['spec_key'], 'item_id' => $cart['item_id']], '', true);
    ...
    public function getUserCartOrderCount(){
        $count = Db::name('CartRed')->where(['user_id' => $this->user_id , 'selected' => 1])->count();
        return $count;
    }
    ...
    public function doUserLoginHandle()
    {
        ...
        if (!empty($cart_id_arr)) {
            ...
            M('cart_red')->delete($cart_id_arr); // 删除购物车完全相同的商品
        }
    }
    /**
     * 更改购物车的商品数量
    ...
    public function changeNum($cart_id, $goods_num){
        ...
        $cartGoods = GoodsRed::get($cart['goods_id']);
        #商品限购
        if($cartGoods['purchase'] != 0 && $goods_num > $cartGoods['purchase'] ){
            return ['status' => 0, 'msg' => '商品数量不能大于'.$cartGoods['purchase'], 'result' => ['limit_num'=>$cartGoods['purchase']]];
        }
    ...
    public function delete($cart_ids = array()){
        ...
        $delete = Db::name('cart_red')->where($cartWhere)->where('id','IN',$cart_ids)->delete();
        return $delete;
    }
    /**
     *  更新购物车，并返回计算结果
    ...
    public function AsyncUpdateCart($cart = [])
    {
        ...
        if (empty($cart)) {
            return ['status' => 0, 'msg' => '购物车没商品', 'result' => compact('midou_fee', 'midou_money_fee', 'goods_num', 'cartList')];
        }
        ...
        if ($cartList) {        
            $cartList = collection($cartList)->append(['midou_fee', 'midou_money_fee', 'goods_num'])->toArray();
    ...
    public function getCartPriceInfo($cartList = null){
        $midou_fee = $midou_money_fee = $goods_num = 0;//初始化数据。商品总额/节约金额/商品总共数量
        if($cartList){
            foreach ($cartList as $cartKey => $cartItem) {
                $midou_fee       += $cartItem['goods_num'] * $cartItem['midou'];
                $midou_money_fee += $cartItem['goods_num'] * $cartItem['midou_money'];
                $goods_num       += $cartItem['goods_num'];
            }
        }
        return compact('midou_fee', 'midou_money_fee', 'goods_num');
    }
