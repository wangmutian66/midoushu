...
class CartLogic extends Model
{
    ...
    public function buyNow(){
        ...
        $buyGoods = [
            ...
            'is_allreturn'       => $this->goods['is_allreturn'],
        ];
    ...
    public function addReturnGoodsToCart()
    {
        ...
        if($this->goods['is_allreturn'] == 0){
            return ['status'=>-10,'msg'=>'非活动商品，请到普通区购买','result'=>['url'=>U('Goods/goodsInfo',['id'=>$this->goods['goods_id'],'item_id'=>$this->specGoodsPrice['item_id']],'',true)]];
        }
        $userCartCount = Db::name('cart')->where(['user_id'=>$this->user_id,'session_id'=>$this->session_id])->count();//获取用户购物车的商品有多少种
        if ($userCartCount >= 100) {
            return array('status' => -9, 'msg' => '购物车最多只能放100种商品', 'result' => '');
    ...
    private function addNormalCart(){
        ...
        if ($userCartGoods) {
            ...
            $cartAddData = array(
                ...
                'is_allreturn'       => $this->goods['is_allreturn'],// 是否参与全返
            );
            if($this->specGoodsPrice){
    ...
    public function getUserCartGoodsTypeNum($t)
    {
        if($t == 1){
            if ($this->user_id) {
                $goods_num = Db::name('cart')->where(['user_id' => $this->user_id,'is_allreturn'=>1])->count();
            } else {
                $goods_num = Db::name('cart')->where(['session_id' => $this->session_id,'is_allreturn'=>1])->count();
            }
        } else {
            if ($this->user_id) {
                $goods_num = Db::name('cart')->where(['user_id' => $this->user_id,'is_allreturn'=>0])->count();
            } else {
                $goods_num = Db::name('cart')->where(['session_id' => $this->session_id,'is_allreturn'=>0])->count();
            }
        }
        return empty($goods_num) ? 0 : $goods_num;
    }
   ...
   //  dump($cartList);die;
    ...
    public function getUserCartOrderCount($t){
        if($t == 1)
            $count = Db::name('Cart')->where(['user_id' => $this->user_id , 'is_allreturn' => 1, 'selected' => 1])->count();
        else
            $count = Db::name('Cart')->where(['user_id' => $this->user_id , 'is_allreturn' => 0, 'selected' => 1])->count(); 
        return $count;
    }
