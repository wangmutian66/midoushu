 ...
 */
class YxypGoodsLogic extends Model
{
    /**
     * @param $goods_id_arr
     * @param $filter_param
    ...
    public function get_filter_brand($goods_id_arr, $filter_param, $action)
    {
        ...
        $map['brand_id'] = ['>', 0];
        $brand_id_arr = M('goods_yxyp')->where($map)->column('brand_id');
        $list_brand = M('brand_yxyp')->where('id','in',$brand_id_arr)->limit('30')->select();
    ...
    public function get_filter_spec($goods_id_arr, $filter_param, $action, $mode = 0)
    {
        ...
        $spec_key = DB::query("select group_concat(`key` separator  '_') as `key` from __PREFIX__spec_yxyp_goods_price where goods_id in($goods_id_str)");  //where("goods_id in($goods_id_str)")->select();
        ...
        $spec = M('spec_yxyp')->where(array('search_index'=>1))->getField('id,name');
        $spec_item = M('spec_yxyp_item')->where(array('spec_id'=>array('in',array_keys($spec))))->getField('id,spec_id,item');
    ...
    public function get_filter_attr($goods_id_arr = array(), $filter_param, $action, $mode = 0)
    {
        $goods_id_str = implode(',', $goods_id_arr);
        $goods_id_str = $goods_id_str ? $goods_id_str : '0';
        $goods_attr = M('goods_yxyp_attr')->where(['goods_id'=>['in',$goods_id_str],'attr_value'=>['<>','']])->select();
        // $goods_attr = M('goods_attr')->where("attr_value != ''")->select();
        $goods_attribute = M('goods_yxyp_attribute')->where("attr_index = 1")->getField('attr_id,attr_name,attr_index');
        if (empty($goods_attr)) {
            if ($mode == 1) return array();
            return array('status' => 1, 'msg' => '', 'result' => array());
        }
    ...
    public function commentStatistics($goods_id)
    {
        ...
        $c1 = M('comment_yxyp')->where($commentWhere)->where('ceil((deliver_rank + goods_rank + service_rank) / 3) in (4,5)')->count();
        $c2 = M('comment_yxyp')->where($commentWhere)->where('ceil((deliver_rank + goods_rank + service_rank) / 3) in (3)')->count();
        $c3 = M('comment_yxyp')->where($commentWhere)->where('ceil((deliver_rank + goods_rank + service_rank) / 3) in (0,1,2)')->count();
        $c4 = M('comment_yxyp')->where($commentWhere)->where("img !='' and img NOT LIKE 'N;%'")->count(); // 晒图
        $c0 = $c1 + $c2 + $c3; // 所有评论
        if($c0 <= 0){
            $rate1 = 100;
            $rate2 = 0;
    ...
    public function collect_goods($user_id, $goods_id)
    {
        ...
        $count = M('GoodsYxypCollect')->where("user_id",$user_id)->where("goods_id", $goods_id)->count();
        if ($count > 0) return array('status' => -3, 'msg' => '商品已收藏', 'result' => array());
        M('GoodsYxypCollect')->add(array('goods_id' => $goods_id, 'user_id' => $user_id, 'add_time' => time()));
    ...
    public function get_siblings_cate($cat_id)
    {
        if (empty($cat_id))
            return array();
        $cate_info = M('goods_yxyp_category')->where("id", $cat_id)->find();
        $siblings_cate = M('goods_yxyp_category')->where(['id'=>['<>',$cat_id],'parent_id'=>$cate_info['parent_id']])->select();
    ...
    {
        return M('goods_yxyp')->where(['goods_id'=>['<>',$goods['goods_id']],'cat_id'=>['<>',$goods['cat_id']],'is_on_sale'=>1,'is_check'=>1])->limit(12)->select();
    }
    ...
    function get_filter_price($goods_id_arr, $filter_param, $action, $c = 5)
    {
        ...
        $goods_id_str = implode(',', $goods_id_arr);
        $goods_id_str = $goods_id_str ? $goods_id_str : '0';
        $priceList = M('goods_yxyp')->where("goods_id", "in", $goods_id_str)->getField('shop_price', true);  //where("goods_id in($goods_id_str)")->select();
        rsort($priceList);
        ...
        for ($i = 0; $i < $c; $i++) {
            ...
            $start2 = num_flaot3(($start/$midou_rate));
            $end2   = num_flaot3(($end/$midou_rate));
            $filter_param['price'] = "{$start}-{$end}";

            if ($i == 0)
                $parr[] = array('value' => "{$end2}米豆以下", 'p' => $filter_param['price'], 'href' => urldecode(U("Goods/$action", $filter_param, '')));
            elseif($i == ($c-1) && ($max_price > $end))
                $parr[] = array('value' => "{$end2}米豆以上", 'p' => $filter_param['price'], 'href' => urldecode(U("Goods/$action", $filter_param, '')));
            else
                $parr[] = array('value' => "{$start2}-{$end2}米豆", 'p' => $filter_param['price'], 'href' => urldecode(U("Goods/$action", $filter_param, '')));
        }
        return $parr;
    }
    ...
    function get_filter_menu($filter_param, $action)
    {
        ...
        // 品牌
        if (!empty($filter_param['brand_id'])) {
            $brand_list = M('brand_yxyp')->getField('id,name');
        ...
        if (!empty($filter_param['spec'])) {
            $spec = M('spec_yxyp')->getField('id,name');
            $spec_item = M('spec_yxyp_item')->getField('id,item');
        ...
        if (!empty($filter_param['attr'])) {
            $goods_attribute = M('goods_yxyp_attribute')->getField('attr_id,attr_name');
    ...
    function get_goods_cate(&$goodsCate)
    {
        ...
        $cateAll = get_goods_red_category_tree();
        if ($goodsCate['level'] == 1) {
            ...
            $parent = M('GoodsYxypCategory')->where("id", $goodsCate['parent_id'])->order('`sort_order` desc')->find();//父类
    ...
    function getGoodsIdByBrandPrice($brand_id, $price)
    {
        ...
        if ($brand_id) // 品牌查询
        {
            ...
            $brand_select_goods = M('goods_yxyp')->whereIn('brand_id',$brand_id_arr,'or')->getField('goods_id', true);
        }
        if ($price)// 价格查询
        {
            $price    = explode('-', $price);
            ...
            $price_select_goods = M('goods_yxyp')->where($price_where)->getField('goods_id', true);
        }
    ...
    function getGoodsIdBySpec($spec)
    {
        ...
        $sql = "select * from (select *,concat('_',`key`,'_') as key2 from __PREFIX__spec_yxyp_goods_price as a) b  $where";
        $result = Db::query($sql);
    ...
    function getGoodsDispatching($goods_id,$region_id)
    {
        ...
        $goods = M('goods_yxyp')->where(array('goods_id'=>$goods_id))->find();
        //检查商品是否包邮
        ...
        $parent_region_id = $this->getParentRegionList($region_id); //寻找Region_id的父级id

        $goodsLogic = new YxypGoodsLogic();

        //商品没有配置物流，使用物流配置里的默认物流
        if(empty($goods['shipping_area_ids'])){
            $plugin_goods_shipping = M('plugin')->where(array('type'=>'shipping','status'=>1,'is_default'=>1,'suppliers_id'=>$goods['suppliers_id']))->select();
        ...
        if(count($shipping_area_id_array) == 0){
            $goods_shipping = M('shipping_area')->where(array('shipping_area_id'=>array('in',$goods_shipping_area_id_array),'is_default'=>1,'suppliers_id'=>$goods['suppliers_id']))->select();
        ...
        $goods_shipping = M('area_region')
            ...
            ->where(array('ar.shipping_area_id'=>array('in',$shipping_area_id_array),'sa.suppliers_id'=>$goods['suppliers_id']))
    ...
    function getFilterSelected($sel ,$cat_id = array(1)){
        $where = " 1 = 1 ";
        $Goods = M('goods_yxyp')->where("cat_id" ,"in" ,implode(',', $cat_id));
        //查看全部
        if($sel == 'selall'){
            $where .= '';
        }
    ...
    public function add_visit_log($user_id,$goods){
        $record = M('goods_yxyp_visit')->where(array('user_id'=>$user_id,'goods_id'=>$goods['goods_id']))->find();
        if($record){
            M('goods_yxyp_visit')->where(array('user_id'=>$user_id,'goods_id'=>$goods['goods_id']))->save(array('visittime'=>time()));
        }else{
            $visit = array('user_id'=>$user_id,'goods_id'=>$goods['goods_id'],'visittime'=>time(),'cat_id'=>$goods['cat_id'],'extend_cat_id'=>$goods['extend_cat_id']);
            M('goods_yxyp_visit')->add($visit);
        }
    }
    ...
    {
        $collect = M('goods_yxyp_collect')->where(['user_id' => $user_id, 'goods_id' => $goods_id])->find();
        return $collect ? 1 : 0;
    }
    /**
     * 获取促销商品数据
    ...
    {
        $goods_where = array('g.is_on_sale' => 1,'g.is_check' => 1);
        $promotion_goods = M('goods_yxyp')
            ->alias('g')
            ->field('g.goods_id,g.goods_name,f.price AS shop_price,f.end_time')
            ->join('__FLASH_YXYP_SALE__ f','g.goods_id = f.goods_id')
            ->where($goods_where)
            ->limit(3)
            ->select();
    ...
    {
        $goods_where = array('is_recommend' => 1, 'is_on_sale' => 1, 'is_check' => 1);
//        $goods_where['goods_name'] = array("exp", " NOT REGEXP '华为|荣耀|小米|合约机|三星|魅族' ");//临时屏蔽,苹果APP审核过了之后注释
        $favourite_goods = M('goods_Yxyp')
    ...
    {
        $goods_where = array('is_new' => 1,  'is_on_sale' => 1, 'is_check' => 1);
        $orderBy = array('sort' => 'desc');
        $new_goods = M('goods_yxyp')
    ...
    {
        $goods_where = array('is_hot' => 1,  'is_on_sale' => 1, 'is_check' => 1);
        if ($is_brand) {
    ...
    public function getFlashSaleGoods($count, $page = 1, $start_time=0, $end_time=0)
    {
        ...
        $flash_sale_goods = M('flash_yxyp_sale')->alias('f')
            ...
            ->join('__GOODS_YXYP__ g','g.goods_id = f.goods_id')
            ->where($where)
    ...
    {
        $goods = M('goods_yxyp')->field('cat_id')->where('goods_id', $goods_id)->find();
        if (empty($goods)) {
            return [];
        }

        $where = ['goods_id' => ['<>', $goods_id], 'cat_id' => $goods['cat_id']];
    	$goods_list = M('goods_yxyp')->field("goods_id,goods_name,shop_price,is_virtual")
    ...
    public function integralMall($rank, $user_id, $p = 1)
    {
        ...
        $goods_list_count = M('goods_yxyp')->where($goods_where)->count();   //总数
        $goods_list = M('goods_yxyp')->field('goods_id,goods_name,shop_price,market_price,exchange_integral,is_virtual')
    ...
    function getSortBrands()
    {
        ...
        $brandIdArr =  M("Brand_yxyp")->where("name in (select `name` from `".C('database.prefix')."brand_yxyp` group by name having COUNT(id) > 1)")->getField('id,cat_id');
        $goodsCategoryArr = M('goodsYxypCategory')->where("level = 1")->getField('id,name');
        $nameList = array();
    ...
    public function getGoodsPromSimpleInfo($goods, $goodsPromLogic)
    {
        ...
        $goodsPromFactory = new \app\common\logic\YxypGoodsPromFactory;
    ...
    public function getOrderPromSimpleInfo($goods)
    {
        ...
        $data = [];
        $po = M('prom_yxyp_order')->where(['start_time' => ['<=', $cur_time], 'end_time' => ['>', $cur_time], 'is_close' => 0])->select();
        if (!empty($po)) {
            foreach ($po as $p) {
    ...
    {
        $categoryList =  M("GoodsYxypCategory")->getField('id,name,parent_id,level');
        $nameList = array();
