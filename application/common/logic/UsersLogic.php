...
class UsersLogic extends Model
{
    ...
    public function thirdLogin($data=array()){
        ...
        if(!$user){
            ...
            $map['staff_id']    =   session('recommend_staff.id');
     ...
     * @return array
     */
    public function reg($username,$password,$password2,$paypwd,$paypwd2,$push_id = 0,$invite="",$nickname="",$head_pic=""){
        ...
        if(!$paypwd)
            return array('status'=>-1,'msg'=>'请输入支付密码','result'=>'');

        //验证两次支付密码是否匹配
        if($paypwd2 != $paypwd)
            return array('status'=>-1,'msg'=>'两次输入支付密码不一致','result'=>'');

        $map['password'] = encrypt($password);
        $map['paypwd'] = encrypt($paypwd);
        $map['reg_time'] = time();
        
        if(session('recommend_staff.id')){
            $map['staff_id']    =   session('recommend_staff.id'); 
        }
     ...
     * @return array
     */
    public function regwechat($username,$password,$password2,$push_id = 0,$invite="",$nickname="",$head_pic=""){
        /*dump(session('recommend_staff'));
        die;*/
        $is_validated = 0 ;
        if(check_email($username)){
    ...
    public function get_info($user_id)
    {
        ...
        $user['return_count'] = M('return_goods')->where("user_id=$user_id and status<3 and status>-1")->count();   //退换货数量
     ...
     * 获取订单商品
     */
    public function get_order_goods1($order_id){
        $sql = "SELECT og.*,g.commission,g.is_allreturn,g.original_img,g.status as rstatus FROM __PREFIX__order_goods og LEFT JOIN __PREFIX__goods g ON g.goods_id = og.goods_id WHERE order_id = :order_id";
    ...
    public function get_order_goods($order_id){
        $sql = "SELECT og.*,g.commission,rg.status as rstatus FROM __PREFIX__order_goods og LEFT JOIN __PREFIX__goods g ON g.goods_id = og.goods_id LEFT JOIN __PREFIX__return_goods rg ON rg.goods_id = og.goods_id and rg.order_id = og.order_id WHERE og.order_id = :order_id";
    ...
    public function get_recharge_log($user_id,$pay_status=0){
        ...
        $recharge_log = M('recharge')->where($recharge_log_where)
            ...
            ->limit($Page->firstRow . ',' . $Page->listRows)
            ->select();
        foreach ($recharge_log as $key => $value) {
            $recharge_log[$key]['account']  =   tk_money_format($value['account']);
        }
        $return = [
            'status'    =>1,
            'msg'       =>'',
    ...
    public function wechat_get_goods_collect($user_id){
        ...
        foreach ($result as $keys => $values) {
            // $result[$keys]['goods_image']=URL.goods_thum_images($values['goods_id'],400,400);
            if (strstr(goods_thum_images($values['goods_id'],400,400),'http')) {
                     $result[$keys]['goods_image']=goods_thum_images($values['goods_id'],400,400);
                }else{
                     $result[$keys]['goods_image']=URL.goods_thum_images($values['goods_id'],400,400);
                }
        }
     ...
     */
    public function setMessageForRead($category=0,$msg_id,$status=1)
    {
        $user_info = session('user');
        if (!empty($user_info['user_id'])) {
            $data['status'] = $status;
