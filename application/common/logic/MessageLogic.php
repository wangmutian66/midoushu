...
class MessageLogic extends Model
{
 ...
 /**
     ...
     * @return array
     */
    public function wechatgetUserMessageNotice($user_id)
    {
        $this->checkPublicMessage();
        $user_system_message_no_read_where = array(
            'user_id' => $user_id,
     ...
     * @return array
     */
    public function getSuppliersMessageCount(){
        $suppliers_info = session('suppliers');
        $suppliers_system_message_no_read_where = array(
            'sm.suppliers_id' => $suppliers_info['suppliers_id'],
            'sm.status' => 0,
        );
        $user_system_message_no_read = DB::name('suppliers_message')
            ->alias('sm')
            ->join('__MESSAGE__ m','sm.message_id = m.message_id','LEFT')
            ->where($suppliers_system_message_no_read_where)
            ->count();
        return $suppliers_system_message_no_read;
    }
     ...
     * @return array
     */
    public function getSuppliersSellerMessage()
    {
        $suppliers_info = session('suppliers');
        $suppliers_system_message_no_read_where = array(
            'suppliers_id' => $suppliers_info['suppliers_id'],
            'status' => 0,
            'm.category' => ['<>', 0]
        );
        $suppliers_system_message_no_read = Db::name('suppliers_message')
            ->alias('sm')
     ...
     * @return array
     */
    public function getSuppliersAllMessage()
    {
        $this->checkPublicMessage_sup();
        $suppliers_info = session('suppliers');
        $suppliers_system_message_no_read_where = array(
            'suppliers_id' => $suppliers_info['suppliers_id'],
            'status' => 0,
     ...
     * @return array
     */
    public function getSuppliersMessageNotice()
    {
        $this->checkPublicMessage_sup();
        $suppliers_info = session('suppliers');
        $suppliers_system_message_no_read_where = array(
     ...
     */
    public function setSuppliersMessageForRead($category = 0,$msg_id)
    {
        $suppliers_info = session('suppliers');
        if (!empty($suppliers_info['suppliers_id'])) {
            $data['status'] = 1;
            $set_where['suppliers_id'] = $suppliers_info['suppliers_id'];
            $set_where['category'] = $category;
            if($msg_id){
                $set_where['message_id'] = $msg_id;
            }
            $updat_meg_res = Db::name('suppliers_message')->where($set_where)->update($data);
            if ($updat_meg_res){
    ...
    public function delSuppliersMessage($category = 0,$msg_id='')
    {
        $suppliers_info = session('suppliers');
        if (!empty($suppliers_info['suppliers_id'])) {
            $data['deleted'] = 1;
            $set_where['suppliers_id'] = $suppliers_info['suppliers_id'];
            $set_where['category'] = $category;
            $updat_meg_res = Db::name('suppliers_message')->where($set_where)->whereIn('rec_id',$msg_id)->update($data);
            if ($updat_meg_res){
                return ['status'=>1,'msg'=>'操作成功'];
            }
        }
    ...
    /**
     * 查询系统全体消息，如有将其插入供货商信息表
     * @author dyr
     * @time 2016/09/01
     */
    public function checkPublicMessage_sup()
    {
        
        $suppliers_info = session('suppliers');
        $suppliers_message = Db::name('suppliers_message')->where(array('suppliers_id' => $suppliers_info['suppliers_id'], 'category' => 0))->select();

        $message_where = array(
            'category'  => 0,
            'type'      => 1,
            'send_time' => array('gt', $suppliers_info['add_time']),
            'object'    => 'suppliers'
        );
        if (!empty($suppliers_message)) {
            $suppliers_id_array = get_arr_column($suppliers_message, 'message_id');
            $message_where['message_id'] = array('NOT IN', $suppliers_id_array);
        }
        $suppliers_system_public_no_read = Db::name('message')->field('message_id')->where($message_where)->select();
        foreach ($suppliers_system_public_no_read as $key) {
            DB::name('suppliers_message')->insert(['suppliers_id'=>$suppliers_info['suppliers_id'],'message_id'=>$key['message_id'],'category'=>0,'status'=>0]);
        }
    }
