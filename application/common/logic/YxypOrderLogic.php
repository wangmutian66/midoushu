...
use app\common\model\OrderYxyp;
...
class YxypOrderLogic
{
	...
	public function cancel_order($user_id,$order_id,$action_note='您取消了订单'){
		$order = M('order_yxyp')->where(array('order_id'=>$order_id,'user_id'=>$user_id))->find();
		...
		if($order['coupon_price'] >0){
			...
			M('coupon_yxyp_list')->where(array('order_id'=>$order_id,'uid'=>$user_id))->save($res);
		}

		$row = M('order_yxyp')->where(array('order_id'=>$order_id,'user_id'=>$user_id))->save(array('order_status'=>3,'cancel_time'=>time()));
		...
		M('order_yxyp_action')->add($data);//订单操作记录
		if(!$row)
	...
	public function addReturnGoods($rec_id,$order)
	{
		...
		if(!empty($data['id'])){
			$result = M('return_yxyp_goods')->where(array('id'=>$data['id']))->save($data);
		}else{
			$result = M('return_yxyp_goods')->add($data);
		}
    ...
    public function getReturnGoodsIndex($sale_t, $keywords, $user_id)
    {
    	...
    	$show = $Page->show();
    	$order_list = M('order_yxyp')->where($condition)->order('order_id desc')->limit($Page->firstRow.','.$Page->listRows)->select();
    	
    	foreach ($order_list as $k=>$v) {
            $order_list[$k] = set_btn_order_status($v);  // 添加属性  包括按钮显示属性 和 订单状态显示属性
            $data = M('order_yxyp_goods')->where(['order_id'=>$v['order_id'],'is_send'=>['lt',2]])->select();
            if(!empty($data)){
    ...
    public function getReturnGoodsList($keywords, $addtime, $status, $user_id = 0)
	{
    	...
    	$query = M('return_yxyp_goods')->alias('r')->field('r.*,g.goods_name')
                ->join('__ORDER_YXYP__ o', 'r.order_id = o.order_id AND o.deleted = 0 AND o.user_id='.$user_id)
                ->join('__GOODS_YXYP__ g', 'r.goods_id = g.goods_id', 'LEFT')
                ->where($where);
        $query2 = clone $query;
        $count = $query->count();
    	$page = new \think\Page($count,10);
    	...
    	if(!empty($goods_id_arr)) {
            $goodsList = M('goods_yxyp')->where("goods_id in (".  implode(',',$goods_id_arr).")")->getField('goods_id,goods_name');
        }
    ...
    {
        $validate = validate('order_yxyp');
        if (!$validate->scene('del')->check(['order_id' => $order_id])) {
            return ['status' => 0, 'msg' => $validate->getError()];
        }
        if(empty($this->user_id))return ['status'=>-1,'msg'=>'非法操作'];
        $row = M('order_yxyp')->where(['user_id'=>$this->user_id,'order_id'=>$order_id])->update(['deleted'=>1]);
        if (!$row) {
            M('order_yxyp_goods')->where(['order_id'=>$order_id])->update(['deleted'=>1]);
    ...
    {
    	$order = M('order_yxyp')->where(['order_id' => $order_id, 'user_id' => $user_id])->find();
    	if (!$order) {
    		return ['status' => -1, 'msg' => '订单不存在'];
    	}
    	$order_return_num = M('return_yxyp_goods')->where(['order_id' => $order_id, 'user_id' => $user_id,'status'=>['neq',5]])->count();
    	...
    	$result = M('order_yxyp')->where(['order_id' => $order_id])->update($order_info);
    	if (!$result) {
	...
	function make_virtual_code($order){
		$order_goods = M('order_yxyp_goods')->where(array('order_id'=>$order['order_id']))->find();
		$goods = M('goods_yxyp')->where(array('goods_id'=>$order_goods['goods_id']))->find();
		M('order_yxyp')->where(array('order_id'=>$order['order_id']))->save(array('order_status'=>1,'shipping_time'=>time()));
	...
	public function abolishOrder(){
		...
		$order = Db::name('order_yxyp')->where($order_where)->getField('order_id',true);
	 ...
	 * @return array
	 */
	public function addStoreOrder($user_id,$address_id,$suppliers_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price=[],$store_price=[],$user_note=[],$pay_name='')
	{
		// 0插入订单 order
		...
		foreach ($suppliers_id as $key) {
            ...
            M('order_yxyp_action')->insertGetId($action_info);

            // 1插入order_goods 表
            if($this->action == 'buy_now'){
                $cartList = $this->cartList[$key];
            }else{
                $cartList = M('CartYxyp')->where(['user_id'=>$user_id,'selected'=>1,'suppliers_id'=>$key])->select();
            }
            foreach($cartList as $k => $val)
            {
                ...
                $order_goods_id              = M("OrderYxypGoods")->insertGetId($data2);
            }
            ...
            if(tpCache('shopping.reduce') == 1){
              ...
              //  dump( $r);die;
                ...
                #    M('order')->where("order_sn = '{$order['order_sn']}'")->delete();
            ...
            // 4 删除已提交订单商品
            if($this->action != 'buy_now'){
                M('CartYxyp')->where(['user_id' => $user_id,'selected' => 1,'suppliers_id'=>$key])->delete();
            }       
	...
	public function get_order_sn()
	{
	    ...
	    // 保证不会有重复订单号存在
	    while(true){
	        $order_sn = 'yxyp'.date('YmdHis').rand(1000,9999); // 订单编号	        
	        $order_sn_count = M('order_yxyp')->where("order_sn = '".$order_sn."'")->count();
	        if($order_sn_count == 0)
	            break;
	    }
    ...
    public function alterReturnGoodsInventory($order, $rec_id='')
	{
        if($rec_id){
            ...
            $retunn_info = Db::name('return_yxyp_goods')->where($orderGoodsWhere)->select(); //查找购买数量和购买规格
            $order_goods_prom = Db::name('order_yxyp_goods')->where($orderGoodsWhere)->find(); //购买时参加的活动
            ...
            $order_goods = Db::name('order_yxyp_goods')->where($orderGoodsWhere)->select(); //查找购买数量和购买规格
        }
		foreach($order_goods as $key=>$val){
			if(!empty($val['spec_key'])){ // 先到规格表里面扣除数量
				$SpecGoodsPrice = new SpecYxypGoodsPrice();
				...
				M('goods_yxyp')->where(['goods_id' => $val['goods_id']])->setInc('store_count', $val['goods_num']);//没有规格则增加商品库存
			}
			update_stock_log_red($order['user_id'], $val['goods_num'], $val, $order['order_sn']);//库存日志

			Db::name('GoodsYxyp')->where("goods_id", $val['goods_id'])->setDec('sales_sum', $val['goods_num']); // 减少商品销售量
			//更新活动商品购买量
			if ($val['prom_type'] == 1 || $val['prom_type'] == 2) {
				$GoodsPromFactory = new YxypGoodsPromFactory();
