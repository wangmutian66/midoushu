...
use think\Model;
class YxypGoodsActivityLogic extends Model {
    /**
     * 预售状态读取器
    ...
    {
        if(empty($goods_id)){
            $goods_id = M('goods_yxyp_activity')->where(array('act_id'=>$goods_activity_id))->getField('goods_ids');
        }
        $condition = array(
            'o.order_prom_type' => 4,
        ...
        $info = M('order_yxyp_goods')
            ->alias('g')
            ->field('count(*) as total_order,sum(g.goods_num) as total_goods')
            ->join('__ORDER_YXYP__ o', 'o.order_id = g.order_id')
            ->where($condition)
            ->select();
    ...
    {
        $res = M('goods_yxyp_activity')->where(array('goods_id'=>$goods_id,'act_type'=>$act_type));
        return $res;
    }
}
