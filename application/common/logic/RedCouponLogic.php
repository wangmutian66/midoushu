...
use app\common\model\CouponRed;
use app\common\model\CouponRedList;
use think\Model;
use think\Db;
 ...
 */
class RedCouponLogic extends Model
{
    /**
     * 获取发放有效的优惠券金额
    ...
    {
        $curtime = time();
        $coupon = M('coupon_red')->where('id', $coupon_id)->find();
        $goods_coupon = M('goods_red_coupon')->where('coupon_id', $coupon_id)->where(function ($query) use ($goods_id, $cat_id) {
    ...
    public function getCouponMoney($user_id, $coupon_id)
    {
        ...
        $couponList = M('CouponRedList')->where("uid", $user_id)->where('id', $coupon_id)->find(); // 获取用户的优惠券
        if (empty($couponList)) {
            return 0;
        }
        $coupon = M('CouponRed')->where("id", $couponList['cid'])->find(); // 获取 优惠券类型表
    ...
    {
        $couponList = M('CouponRedList')->where("code", $couponCode)->find(); // 获取用户的优惠券
        if(empty($couponList))
            return array('status'=>-9,'msg'=>'优惠券码不存在','result'=>'');
        if($couponList['order_id'] > 0){
            return array('status'=>-20,'msg'=>'该优惠券已被使用','result'=>'');
        }
        $coupon = M('CouponRed')->where("id", $couponList['cid'])->find(); // 获取优惠券类型表
    ...
    public function getCartCouponList($user_id, $type, $cartList, $p = 1, $size = null)
    {
        ...
        $query = Db::name('coupon_red')->alias('c1')
                ->field('c1.name,c1.money,c1.condition,c1.use_end_time, c2.*')
                ->join('__COUPON_RED_LIST__ c2','c2.cid=c1.id AND c2.status=0', 'LEFT')
                ->where($where);
        if ($size) {
    ...
    {
        $CouponList = new CouponRedList();
        $Coupon = new CouponRed();
        $userCouponArr = [];
        ...
        $couponList = $Coupon->with('GoodsRedCoupon')
    ...
    public function exchangeCoupon($user_id, $coupon_code)
    {
        ...
        $coupon = CouponRed::get($coupon_list['cid']); // 获取优惠券类型表
        if (time() < $coupon['use_start_time']) {
