 ...
 * @package Home\Model
 */
class OrderYxyp extends Model
{

    //获取订单商品
    public function OrderGoods()
    {
        return $this->hasMany('OrderYxypGoods','order_id','order_id');
    }
    /**
     * 获取订单状态对应的中文
     * @param $value
    ...
    {
        return $this->hasOne('TeamYxypActivity', 'team_id', 'order_prom_id');
    }

    public function teamFollow(){
        return $this->hasOne('TeamYxypFollow','order_id','order_id');
    }

    public function teamFound(){
        return $this->hasOne('TeamYxypFound','order_id','order_id');
    }
    /**
     * 获取订单状态的 显示按钮
     * @param $value
    ...
    public function getVirtualOrderButtonAttr($value, $data){
        $vr_order_code = Db::name('vr_yxyp_order_code')->where(['order_id'=>$data['order_id']])->find();
        ...
        if(!empty($vr_order_code)){
            if ($data['pay_status']==1 && $data['order_status']<2 && $vr_order_code['vr_state']!=1 && $vr_order_code['refund_lock']<1)
            {
                ...
                {
                    $Virtual_btn_arr['cancel_btn'] = 2; // 已支付取消按钮
                    M('vr_yxyp_order_code')->where(array('order_id'=>$data['order_id']))->update(['vr_state'=>2]);
                }
