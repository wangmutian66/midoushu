 ...
 */
 ...
 function num_float2($num){ // 保留三位 不进位
    $num = sprintf("%.3f",substr(sprintf("%.4f", $num), 0, -1));
    return $num;
 }

 function num_flaot3($num){ // 保留三位小数 进位
    $num2 = num_float2($num);
    if( ($num - $num2) > 0) $num = $num2+0.001;
    return $num;
 }
...
#张洪凯  2018-11-28
#对已发货但是快递查询表里没有记录进行处理
#进行订阅，同时实时查询目前进度并返回
#$red 为0 是 现金区  为1 是米豆区
function syncDelivery($order_id,$red=0){

    $delivery_doc_talbe = $red == 1 ? "delivery_red_doc" : "delivery_doc";
//    $delivery_log_talbe = $red == 1 ? "delivery_red_log" : "delivery_log";
    #判断是否发货，发货表里是否有记录
    $delivery_doc_model = db($delivery_doc_talbe);
//    $delivery_log_model = db($delivery_log_talbe);

    /*$invoice_key = config('delivery')['key'];
    $getcom_url = config('delivery')['getcom_url'];
    $codes_keys = array_keys(config('delivery')['wuliuarr']);*/

    $data= $delivery_doc_model->field('order_id,shipping_code,invoice_no')->where(['order_id'=>$order_id])->find();
    if($data){
        $res = httpRequest("https://www.midoushu.com/index.php/auto/Kuaidi/doquery?num=".$data['invoice_no']);
        $res = json_decode($res,true);
        $kuaidilog = $res['log'];
        $comname = $res['comname'];
        return array('log'=>$kuaidilog,'comname'=>$res['comname'],'num'=>$res['num']);
    }else{
        return array('log'=>array(),'comname'=>$res['comname'],'num'=>$res['num']);
    }
    ...
    #已发货
    if($data){
        #已发货的订单，判断快递查询表里是否有记录
        $dg =$delivery_log_model->where(['order_id'=>$order_id])->find();
        #快递查询表里有记录，提取物流记录（如果有）
        if($dg){
            if(!empty($dg['data'])){
                if($dg['status'] == 3){
                    $context = unserialize($dg['data']);
                    if(count($context) > 0)
                    {
                        $kuaidilog = array();
                    ...
                    $res = httpRequest("https://www.midoushu.com/index.php/auto/Kuaidi/doquery?num=".$dg['invoice_no']);
                    $res = json_decode($res,true);
                    $kuaidilog = $res['log'];
                }

                return array('log'=>$kuaidilog);
            }
            else{

                $reslist = httpRequest($getcom_url."?num=".$dg['invoice_no']."&key=".$invoice_key);
                ...
                #如果通过单号查询到的快递公司编码为空，则单号填写有误
                if(!$shipping_code) return array('log'=>array());

                $post_data = array();
                $post_data["customer"] = config('delivery')['customer'];
                $key= $invoice_key;
                $post_data["param"] = '{"com":"'.$shipping_code.'","num":"'.$dg['invoice_no'].'"}';
                ...
                if($resdata['result'] !== false){
                    ...
                    $ukdata['status'] = $resdata['state'];
                    $ukdata['data'] = serialize($resdata['data']);
                    $delivery_log_model->where("order_id=".$order_id)->update($ukdata);
                }
            ...
            #快递查询表里没有记录，从发货表里进行同步，同时进行快递推送订阅，同时实时查询快递进度
            $callbackurl = config('delivery')['callbackurl'];
            $post_data = array();
            $post_data["schema"] = 'json';
            $post_data["param"] = '{"company":"'.$data['shipping_code'].'","number":"'.$data['invoice_no'].'","from":"","to":"","key":"'.$invoice_key.'","parameters":{"callbackurl":"'.$callbackurl.'?red='.$red.'&orderid='.$data['order_id'].'","salt":"'.get_rand_str(15,0,1).'","resultv2":"1"}}';
            #订阅请求地址
            ...
            if($kuaidi['returnCode'] == 700 || $kuaidi['returnCode'] == 702){
                ...
                $post_data = array();
                $post_data["schema"] = 'json';
                $post_data["param"] = '{"company":"'.$shipping_code.'","number":"'.$data['invoice_no'].'","from":"","to":"","key":"'.$invoice_key.'","parameters":{"callbackurl":"'.$callbackurl.'?red='.$red.'&orderid='.$data['order_id'].'","salt":"'.get_rand_str(15,0,1).'","resultv2":"1"}}';
                $o="";
                foreach ($post_data as $k=>$v)
                {
                    $o.= "$k=".urlencode($v)."&";		//默认UTF-8编码格式
            ...
            $kdata['addtime'] = time();
            $kdata['orderstatus'] =  $kuaidi['returnCode'];
            $kdata['ordermessage']  =  $kuaidi['message'];
            $kdata['subtime']   =  time();
            $result = $delivery_log_model->save($kdata);

            #如果同步成功进行快递实时查询一次
            $post_data = array();
            $post_data["customer"] = config('delivery')['customer'];
            $key= $invoice_key;
            $post_data["param"] = '{"com":"'.$kdata['shipping_code'].'","num":"'.$kdata['invoice_no'].'"}';
