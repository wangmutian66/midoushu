...
class Cart extends Base {
    ...
    public function _initialize()
    {
        ...
        $this->cartLogic = new YxypCartLogic();
...
#    dump($cartList);die;
    ...
    #    dump($cartList);die;
        $userCartGoodsTypeNum = $cartLogic->getUserCartGoodsTypeNum(0);//获取用户购物车商品总数
...
//                $cartLogic->setSpecGoodsIdModel($item_id); // 商品规格id
        ...
        foreach ($suppliers_id as $key) {
            ...
            $result = calculate_price_yxyp($this->user_id,$order_goods_arr[$key],$shipping_code[$key],0,$address['province'],$address['city'],$address['district'],$pay_points,$user_money,$coupon_id,$red_envelope_id);
            ...
            $store_price[$key] = array(
                ...
                'payables'          => number_format($result['result']['goods_price'], 2, '.', ''), // 应付金额
            ...
            $car_price['payables']          += number_format($result['result']['goods_price'], 2, '.', ''); // 应付金额
    ...
    public function cart4(){
        ...
        if($order_id){
            ...
            $order[0] = M('OrderYxyp')->where($order_where)->find();

            if($order[0]['order_status'] == 3){
                $this->error('该订单已取消',U("Homeyxyp/Order/order_detail",array('id'=>$order_id)));
            }

            // 如果已经支付过的订单直接到订单详情页面. 不再进入支付页面
            if($order[0]['pay_status'] == 1){            
                $order_detail_url = U("Homeyxyp/Order/order_detail",array('id'=>$order[0]['order_id']));
        ...
        if($order_sn){
            ...
            $order = M('OrderYxyp')->where($order_where)->select();
        ...
        //预售和抢购暂不支持货到付款
        $orderGoodsPromType = M('order_yxyp_goods')->where('order_id','in',$oids)->getField('prom_type',true);
        ...
        $bank_img = include APP_PATH.'homeyxyp/bank.php'; // 银行对应图片        
    ...
    public function pre_sell_cart()
    {
        ...
        if(empty($goods_num)){
            $this->error('购买商品数量不能为0', U('Homeyxyp/Activity/pre_sell', array('act_id' => $act_id)));
        }
        if($this->user_id == 0){
            $this->error('请先登录',U('Homeyxyp/User/login'));
        }
        $pre_sell_info = M('goods_activity')->where(array('act_id' => $act_id, 'act_type' => 1))->find();
        if(empty($pre_sell_info)){
            $this->error('商品不存在或已下架',U('Homeyxyp/Activity/pre_sell_list'));
        }
    ...
    public function buyIntegralGoods(){
        ...
        $Integral = new YxypIntegralLogic();
        if(!empty($item_id)){
            $specGoodsPrice = SpecYxypGoodsPrice::get($item_id);
    ...
    public function integral(){
        ...
        if (empty($item_id)) {
            $goods_spec_list = SpecYxypGoodsPrice::all(['goods_id' => $goods_id]);
            ...
            //有规格
            $specGoodsPrice = SpecYxypGoodsPrice::get(['item_id'=>$item_id,'goods_id'=>$goods_id]);
