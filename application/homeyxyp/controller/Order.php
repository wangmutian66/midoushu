...
use app\common\logic\YxypCommentLogic;
...
class Order extends Base {
    ...
    public function _initialize() {      
        parent::_initialize();
        if(session('?user'))  // 是否登录
        {
        	$user = session('user');   //登录信息           
        	$this->user = $user;
    ...
    public function order_list(){
        ...
        if($search_key)
        {
            $where .= " and (order_sn like :search_key1 or order_id in (select order_id from `".C('database.prefix')."order_yxyp_goods` where goods_name like :search_key2) ) ";
            $bind['search_key1'] = "%$search_key%";
            $bind['search_key2'] = "%$search_key%";
        }
        $where.=' and order_prom_type < 5 ';//虚拟拼团订单不列出来
        $count = M('order_yxyp')->where($where)->bind($bind)->count();
        $Page = new Page($count,10);

        $show = $Page->show_cxpc();
        $order_str = "order_id DESC";
        $order_list = M('order_yxyp')->order($order_str)->where($where)->bind($bind)->limit($Page->firstRow.','.$Page->listRows)->select();
    ...
    public function order_detail(){
        ...
        $order_info = M('order_yxyp')->where($map)->find();
        if(!$order_info){
        ...
        //获取订单操作记录
        $order_action = M('order_yxyp_action')->where(array('order_id'=>$id))->select();
        $this->assign('order_status',C('ORDER_STATUS'));
        $this->assign('shipping_status',C('SHIPPING_STATUS'));
        $this->assign('pay_status',C('PAY_STATUS'));
    ...
    public function cancel_order(){
        ...
        $map['order_id'] = $id;
        $map['user_id']  = $this->user_id;
        $order_info = M('order_yxyp')->field('order_status,shipping_status,pay_status')->where($map)->find();
    ...
    public function refund_order()
    {
    	...
    	$order = M('order_yxyp')
                ->field('order_id,consignee,mobile,pay_code,pay_name,user_money,integral_money,coupon_price,order_amount,total_amount,order_status,shipping_status,pay_status')
    ...
    public function delComment()
    {
        ...
        $comment = Db::name('comment_yxyp')->where('comment_id', $comment_id)->find();
        if ($this->user_id != $comment['user_id']) {
            $this->error('不能删除别人的评论');
        }
        Db::name('reply')->where('comment_id', $comment_id)->delete();
        Db::name('comment_yxyp')->where('comment_id', $comment_id)->delete();
        $this->success('删除评论成功');
    }
    ...
    public function ajaxZan()
    {
        ...
        $user_id = $this->user_id;
        $comment_info = M('comment_yxyp')->where(array('comment_id' => $comment_id))->find();
        ...
        if (in_array($user_id, $comment_user_id_array)) {
            ...
            M('comment_yxyp')->where(array('comment_id' => $comment_id))->save($comment_data);
            $result['success'] = 1;
        }
    ...
    public function reply_add()
    {
        ...
        $user_goods_count = Db::name('order_yxyp')
            ->alias('o')
            ->join('__ORDER_YXYP_GOODS__ og','o.order_id = og.order_id', 'LEFT')
            ->where($where)
            ->count();
        if ($user_goods_count > 0) {
            M('reply')->add($reply_data);
            M('comment_yxyp')->where(array('comment_id' => $comment_id))->setInc('reply_num');
            $json['success'] = true;
        } else {
            $json['success'] = false;
    ...
    public function order_confirm()
    {
    	...
    	$data = confirm_order_yxyp($id, $this->user_id);
    ...
    public function return_goods()
    {
        ...
        $order = M('order_yxyp')->where(array('order_id'=>$order_goods['order_id'],'user_id'=>$this->user_id))->find();
    ...
    public function return_goods_list()
    {
        ...
        $page = new Page($count,10);
        $list = M('return_yxyp_goods')->where($where)->order("id desc")->limit("{$page->firstRow},{$page->listRows}")->select();
        $goods_id_arr = get_arr_column($list, 'goods_id');
        if(!empty($goods_id_arr))
            $goodsList = M('goods_yxyp')->where("goods_id","in", implode(',',$goods_id_arr))->getField('goods_id,goods_name,shop_price');
    ...
    {
        $id = I('id/d',0);
        $ReturnGoodsModel = new \app\common\model\ReturnYxypGoods();
    ...
    */
    function buy_post_shipping(){
        ...
        if(db('return_yxyp_goods')->where("user_id = {$this->user_id}")->cache(true)->find($return_id)){
            if($data['shipping_code'] && $data['shipping_post_code']){
                $r = db('return_yxyp_goods')->where("id = {$return_id}")->update($data);
                if($r){
                    $res['status']  =   1;
                    $res['info']    =   '添加运单号成功！';
                }else{
    ...
    public function return_goods_cancel(){
    	...
    	$res=M('return_yxyp_goods')->where(['id'=>$id,'user_id'=>$this->user_id])->save(array('status'=>-2,'canceltime'=>time()));
        if($res){
    ...
    public function receiveConfirm(){
        ...
        if($update) {
            M('order_yxyp_goods')->where(array(
    ...
    public function comment_list()
    {
        ...
        if (empty($order_id) || empty($rec_id)) {
            ...
            $order_info = M('order_yxyp')->field('order_sn,order_id,add_time,order_prom_type,suppliers_id') ->where($order_comment_where)->find();
            //查找评价商品
            $order_comment_where['rec_id'] = $rec_id;
            $order_goods = M('order_yxyp_goods')
                ->field('goods_id,is_comment,goods_name,goods_num,goods_price,spec_key_name,suppliers_id')
    ...
    */
    public function add_comment()
    {
        ...
        $add['suppliers_id'] = I('suppliers_id/d');
