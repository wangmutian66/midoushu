    ...
    */
    ...
    public function return_goods()
    {
        ...
        if(I('post')==1)
        {
            ...
            // $img = M('wechat_img')->where(array('rec_id'=>$rec_id,'user_id'=>$user_id))->getField('img',true);
            // $img =  implode(',', $img);
             $model = new OrderLogic();
            $res = $model->addReturnGoods($rec_id,$order,$img);  //申请售后
            if($res['status']==1){
                // M('wechat_img')->where(array('rec_id'=>$rec_id,'user_id'=>$user_id))->delete();
