...
/**
    ...
    */
    ...
    public function reg()
    {
        ...
        if (I('post.username')) {
            ...
            if($is_bind_account && session("third_oauth")){ //绑定第三方账号
                ...
                $data = $logic->regwechat($username, $password, $password2, 0, $invite ,$nickname , $head_pic);
                //用户注册成功后, 绑定第三方账号
                $userLogic = new UsersLogic();
                $data = $userLogic->oauth_bind_new($data['result']);
            }else{
                $data = $logic->regwechat($username, $password, $password2,0,$invite);
            }
    ...
    {
        $user_id = I('get.user_id/d', 313);
        $address_lists = get_user_address_list($user_id);
        foreach ($address_lists as $key => $value) {
            $address_lists[$key]=$value;
            $address_lists[$key]['province']=wechat_get_region_list($value['province']);
            $address_lists[$key]['city']=wechat_get_region_list($value['city']);
            $address_lists[$key]['district']=wechat_get_region_list($value['district']);
        }
