...
class Cart extends MobileBase {
    ...
    public function index(){
        ...
        $hot_goods = M('Goods')->where('is_hot=1 and is_allreturn=0 and is_on_sale=1 and is_check=1')->limit(20)->cache(true,TPSHOP_CACHE_TIME)->select();
    ...
    public function cart2(){
        ...
        if(empty($address)){
            header("Location: ".U('Mobile/User/add_address',array('source'=>'cart2','goods_id'=>$goods_id,'goods_num'=>$goods_num,'item_id'=>$item_id,'action'=>$action)));
            exit;
        }else{
    ...
    public function cart4(){
        ...
        if($order_id){
            ...
            if($order[0]['order_status'] == 3){
                $this->error('该订单已取消',U("Mobileyxyp/Order/order_detail",array('id'=>$order_id)));
            }

            // 如果已经支付过的订单直接到订单详情页面. 不再进入支付页面
            if($order[0]['pay_status'] == 1){            
                $order_detail_url = U("Mobileyxyp/Order/order_detail",array('id'=>$order[0]['order_id']));
        ...
        if($order_sn){
            ...
            $order_amounts = M('OrderYxyp')->where($order_where)->sum('order_amount');
