...
class Payment extends MobileBase {
    ...
    public function getCode(){     
            ...
            if(!session('user')) $this->error('请先登录',U('Mobile/User/login'));   
            // 修改订单的支付方式
            $payment_arr = M('Plugin')->where("`type` = 'payment'")->getField("code,name"); 

            if($order_num == 1){
                $order[0] = M('OrderYxyp')->where(['order_id' => $order_id])->find();
                if($order[0]['pay_status'] == 1){
                    $this->error('此订单，已完成支付!',U('/mobileyxyp/Order/order_list'));
                }
                M('order_yxyp')->where("order_id", $order_id)->save(array('pay_code'=>$this->pay_code,'pay_name'=>$payment_arr[$this->pay_code]));
            } else {
                $order    = M('OrderYxyp')->where('order_sn ="'.$order_sn.'" OR parent_sn ="'.$order_sn.'"')->select();
                M('order_yxyp')->where('order_sn ="'.$order_sn.'" OR parent_sn ="'.$order_sn.'"')->save(array('pay_code'=>$this->pay_code,'pay_name'=>$payment_arr[$this->pay_code]));
            }

            if(empty($order) || $order[0]['order_status'] > 1){
                    $this->error('非法操作！',U("Mobileyxyp/Index/index"));
            }
            ...
            if($order_num == 1){
                ...
                $order_new['order_amount'] = M('OrderYxyp')->where('order_sn ="'.$order_sn.'" OR parent_sn ="'.$order_sn.'"')->sum('order_amount');
            }
...
#            header("Content-type:text/html;charset=utf-8");            
 ...
 //   public function notifyUrl
    ...
    public function returnUrl(){
        ...
        if(session('order_id')){
            $order = M('order_yxyp')->where("order_sn", $result['order_sn'])->find();
            //2018-9-25 王牧田修改  订单提交支付后直接确认
            M('order_yxyp')->where("order_id", $order['order_id'])->save(['order_status'=>1]);
        ...
        if(session('order_sn')){
            $order = M('order_yxyp')->where('order_sn ="'.$result['order_sn'].'" OR parent_sn ="'.$result['order_sn'].'"')->find();
            //2018-9-25 王牧田修改  订单提交支付后直接确认
            M('order_yxyp')->where("order_id", $order['order_id'])->save(['order_status'=>1]);
            if(empty($order)) // order_sn 找不到 根据 order_id 去找
            {
                $order_sn = session('order_sn'); // 最近支付的一笔订单 id        
                $order = M('order_yxyp')->where('order_sn ="'.$order_sn.'" OR parent_sn ="'.$order_sn.'"')->find();
            }       
            $this->assign('order', $order);
        }
