...
use app\common\logic\YxypCouponLogic;
use app\common\logic\YxypGoodsLogic;
use app\common\logic\YxypTeamFoundLogic;
use app\common\logic\YxypTeamOrderLogic;
use app\common\model\GoodsYxyp;
use app\common\model\OrderYxyp;
use app\common\model\OrderYxypGoods;
use app\common\model\ShippingArea;
use app\common\model\TeamYxypActivity;
use app\common\model\TeamYxypFollow;
use app\common\model\TeamYxypFound;
...
class Team extends MobileBase
{
    ...
    {
        $goods_category = Db::name('goods_yxyp_category')->where(['level' => 1, 'is_show' => 1])->select();
        $this->assign('goods_yxyp_category', $goods_category);
        return $this->fetch();
    }
    ...
    public function category()
    {
        ...
        $goods_category_level_one = Db::name('goods_yxyp_category')->where(['id' => $id])->find();
        $goods_category_level_two = Db::name('goods_yxyp_category')->where(['parent_id' => $goods_category_level_one['id']])->select();//二级分类
    ...
    public function AjaxTeamList(){
        ...
        $goods_where = [];
        if($id && $two_all_ids){
            $category_three_ids = Db::name('goods_yxyp_category')->where(['parent_id' => ['in',$two_all_ids]])->getField('id',true);//三级分类id
            $goods_where['cat_id'] = ['in',$category_three_ids];
        }
        if($tid){
            $category_three_ids = Db::name('goods_yxyp_category')->where(['parent_id' => $tid])->getField('id',true);//三级分类id
            $goods_where['cat_id'] = ['in',$category_three_ids];
        }
        $team_where = ['t.status' => 1, 't.is_lottery' => 0, 'g.is_on_sale' => 1];
        if(count($goods_where) > 0){
            $goods_ids = Db::name('goods_yxyp')->where($goods_where)->getField('goods_id', true);
            if(!empty($goods_ids)){
        ...
        $TeamActivity = new TeamYxypActivity();
        $list = $TeamActivity->alias('t')->join('__GOODS_YXYP__ g', 'g.goods_id = t.goods_id')
    ...
    public function info(){
        ...
        if(empty($goods_id)){
            $this->error('参数错误', U('Mobileyxyp/Team/index'));
        }
        $TeamActivity = new TeamYxypActivity();
        $Goods = new GoodsYxyp();
        ...
        if (empty($teamList)) {
            $this->error('该商品拼团活动不存在或者已被删除', U('Mobileyxyp/Team/index'));
        }
        if(empty($goods)){
            $this->error('此商品不存在或者已下架', U('Mobileyxyp/Team/index'));
        }
        ...
        if($user_id){
            $collect = Db::name('goods_yxyp_collect')->where(array("goods_id"=>$goods_id ,"user_id"=>$user_id))->count();
            $this->assign('collect',$collect);
        }
        $spec_goods_price = Db::name('spec_yxyp_goods_price')->where("goods_id",$goods_id)->getField("key,price,store_count,item_id,prom_id"); // 规格 对应 价格 库存表
        if($spec_goods_price){
        ...
        $this->assign('spec_yxyp_goods_price', json_encode($spec_goods_price,true));
        $goods_images_list = Db::name('goods_yxyp_images')->where("goods_id" , $goods_id)->select(); // 商品图册
    ...
    public function addOrder()
    {
        ...
        $team = TeamYxypActivity::get($team_id);
        if($found_id){
            $teamFound = TeamYxypFound::get(['found_id' => $found_id, 'status' => 1]);
            if(empty($teamFound)) {
            ...
            $teamFoundLogic = new YxypTeamFoundLogic();
        ...
        $teamOrderLogic = new YxypTeamOrderLogic();
        if (!empty($teamFound)) {
    ...
    public function order()
    {
        ...
        $Order = new OrderYxyp();
        $OrderGoods = new OrderYxypGoods();
        $order = $Order->where(['order_id'=>$order_id,'user_id'=>$this->user_id])->find();
        if(empty($order)){
            $this->error('订单不存在或者已取消', U("Mobileyxyp/Order/order_list"));
        }
        if ($address_id) {
        ...
        if(empty($address)){
            header("Location: ".U('Mobileyxyp/User/add_address',array('source'=>'team','order_id'=>$order_id)));
            exit;
        }else{
        ...
        if($order['pay_status'] == 1){
            $order_detail_url = U("Mobileyxyp/Order/order_detail",array('id'=>$order_id));
            header("Location: $order_detail_url");
        }
        if($order['order_status'] == 3 ){   //订单已经取消
            $this->error('订单已取消',U("Mobileyxyp/Order/order_list"));
        }
        //微信浏览器
        ...
        if($order['coupon_price'] <= 0){
            $couponLogic = new YxypCouponLogic();
            $TeamOrderLogic = new YxypTeamOrderLogic();
    ...
    public function getOrderInfo(){
        ...
        if(empty($order)){
            $this->ajaxReturn(['status'=>0,'msg'=>'该订单已关闭或者不存在','result'=>['url'=>U("Mobileyxyp/Order/order_list")]]);
        }
        $orderInfo = $order->toArray();
        if(empty($order['province'])){
            if(empty($address_id)){
                $this->ajaxReturn(['status' => 0, 'msg' => '请选择地址', 'result' => ['url' => U('Mobileyxyp/User/add_address', array('source' => 'team', 'order_id' => $order_id))]]);
            }
            //获取用户地址，检查用户地址
        ...
        //获取拼单信息，并检查拼单,是否能拼
        $TeamFoundLogic = new YxypTeamFoundLogic();
        $teamFollow = TeamRedFollow::get(['order_id' => $order_id, 'follow_user_id' => $this->user_id]);
        if ($teamFollow) {
    ...
    public function found()
    {
        ...
        $teamFound = TeamRedFound::get($found_id);
        ...
        if(time() - $teamFound['found_time'] > $team['time_limit']){
            ...
            if($teamFound['join'] < $teamFound['need']){
                ...
                $teamFound->save();
                //更新团员成团失败
                Db::name('team_yxyp_follow')->where(['found_id'=>$found_id,'status'=>1])->update(['status'=>3]);
            }
        }
    ...
    public function lottery(){
        ...
        $team_lottery = Db::name('team_yxyp_lottery')->where('team_id',$team_id)->select();
        $TeamActivity = new TeamYxypActivity();
