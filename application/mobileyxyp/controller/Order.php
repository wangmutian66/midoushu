...
class Order extends MobileBase
{
    ...
    public function order_list()
    {
        ...
        $count = M('order_yxyp')->where($where)->count();
        $Page = new Page($count, 10);
        $show = $Page->show();
        $order_str = "order_id DESC";
        $order_list = M('order_yxyp')->order($order_str)->where($where)->limit($Page->firstRow . ',' . $Page->listRows)->select();
    ...
    public function team_detail(){
        ...
        $Order = new \app\common\model\OrderYxyp();
    ...
    public function order_detail()
    {
        ...
        #2018-11-28  张洪凯  优化
        $res = syncDelivery($id);
        $this->assign('comname',$res['comname']);
    ...
    */
    ...
    public function order_confirm()
    {
        ...
        if ($data['status'] != 1) {
            $this->error($data['msg'],U('Mobileyxyp/Order/order_list'));
        } else {
            $model = new YxypUsersLogic();
    ...
    public function record_refund_order()
    {
        ...
        $logic = new \app\common\logic\YxypOrderLogic;
    ...
    {
        //退换货商品信息
        $count = M('return_yxyp_goods')->where("user_id", $this->user_id)->count();
        $pagesize = C('PAGESIZE');
        $page = new Page($count, $pagesize);
        $list = M('return_yxyp_goods')->where("user_id", $this->user_id)->order("id desc")->limit("{$page->firstRow},{$page->listRows}")->select();
    ...
    {
        $id = I('id/d', 0);
        $return_goods = M('return_yxyp_goods')->where("id = $id")->find();
    ...
    public function return_goods_cancel(){
        ...
        $return_goods = M('return_yxyp_goods')->where(array('id'=>$id,'user_id'=>$this->user_id))->find();
        if(empty($return_goods)) $this->error('参数错误');
        M('return_yxyp_goods')->where(array('id'=>$id))->save(array('status'=>-2,'canceltime'=>time()));
    ...
    public function receiveConfirm(){
        ...
        $return_info=M('return_yxyp_goods')->field('order_id,order_sn,goods_id,spec_key')->where('id',$return_id)->find(); //查找退换货商品信息
        $update = M('return_yxyp_goods')->where('id',$return_id)->save(['status'=>3]);  //要更新状态为已完成
        if($update) {
    ...
    public function comment()
    {
        ...
        $status = I('get.status');
        $logic = new \app\common\logic\YxypCommentLogic;
    ...
    public function add_comment()
    {
        if (IS_POST) {
            ...
            if ($row['status'] == 1) {
                $this->success('评论成功', U('/Mobileyxyp/Order/comment', array('status'=>1)));
                exit();
            } else {
                $this->error($row['msg']);
            }
        ...
        $rec_id = I('rec_id/d');
        $order_goods = M('order_yxyp_goods')->where("rec_id", $rec_id)->find();
        $this->assign('order_goods', $order_goods);
