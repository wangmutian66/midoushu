...
use app\common\logic\YxypVirtualLogic;
use think\Page;
use think\Db;
class Virtual extends MobileBase
{
    ...
    public function _initialize()
    {
        ...
        if (session('?user')) {
            ...
            if (!in_array(ACTION_NAME, $nologin)) {
                header("location:" . U('Mobileyxyp/User/login'));
                exit;
            }
            if (ACTION_NAME == 'password') $_SERVER['HTTP_REFERER'] = U("Mobileyxyp/User/index");
        }
    ...
    public function buy_step2(){
    	...
    	$order = M('OrderYxyp')->where("order_id = $order_id")->find();
    ...
    public function add_order(){
    	...
    	$isbuy = M('order_yxyp_goods')->alias('og')
            ->join(C('DB_PREFIX').'order_yxyp o','og.order_id = o.order_id','LEFT')
    	    ->where($isbuyWhere)
            ->sum('og.goods_num');
    	...
    	$order_id = M('order_yxyp')->add($orderArr);
    ...
    public function check_virtual_goods(){
        ...
        if($goods['is_virtual'] == 1 && $goods['virtual_indate']>time() && $goods['store_count']>0){
            ...
            if(!empty($goods_spec) && $goods_spec !='undefined'){
                $specGoodsPriceList = M('SpecYxypGoodsPrice')->where(array('goods_id'=>$goods_id))->cache(true,TPSHOP_CACHE_TIME)->getField("key,key_name,price,store_count,sku"); // 获取商品对应的规格价钱 库存 条码
...
//                        $goods['spec_key_name'] .= $specGoodsPriceList[$val]['key_name'];
            ...
            if(!empty($goods_spec_key)){
                $specGoods = M('SpecYxypGoodsPrice')->where(array('goods_id'=>$goods_id,'key'=>$goods_spec_key))->find();
    ...
    public function virtual_list()
    {
        ...
        $virtualLogic = new \app\common\logic\YxypVirtualLogic;
    ...
    public function virtual_order(){
        ...
        $VirtualLogic = new YxypVirtualLogic();
        ...
        //获取订单操作记录
        $order_action = Db::name('order_yxyp_action')->where(array('order_id'=>$order_id))->select();
    ...
    public function virtual_confirm(){
        ...
        if($data['status']==1){
            Db::name('order_yxyp_goods')->where(['order_id'=>$order_id])->save(['is_send'=>1]);  //订单商品状态需要更改一下，不然评价列表找不到
        }
