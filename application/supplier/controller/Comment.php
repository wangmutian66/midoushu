...
class Comment extends Base {
    ...
    public function ajaxindex(){
        ...
        $count = $model->alias('comment')
                    ...
                    ->join('__GOODS__ g','comment.goods_id = g.goods_id','LEFT')
                    ->where($where)
                    ->count();
    ...
    public function detail(){
        ...
        if(IS_POST){
            ...
            $add['add_time']  = time();
            $add['username']  = get_suppliers_name($this->suppliers_id);
            $add['is_show']   = 1;
    ...
    public function commentHandle(){
        ...
        if($row !== false){
            $this->ajaxReturn(['status' => 1,'msg' => '操作完成','url'=>U('Supplier/Comment/index')]);
        }else{
            $this->ajaxReturn(['status' => -1,'msg' => '操作失败','url'=>U('Supplier/Comment/index')]);
        }
    }
    ...
    public function ajax_ask_list(){
        ...
        $count = $model->alias('consult')->where($where)->count();        
    ...
    public function ask_handle()
    {
        ...
        if($row !== false){
            $this->ajaxReturn(['status' => 1,'msg' => '操作完成','url'=>U('Supplier/Comment/ask_list')]);
        }else{
            $this->ajaxReturn(['status' => -1,'msg' => '操作失败','url'=>U('Supplier/Comment/ask_list')]);
        }
    }
}