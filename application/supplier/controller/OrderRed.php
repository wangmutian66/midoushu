...
class OrderRed extends Base {
 ...
 //   var $is_red;
    ...
    public function ajaxindex(){
        ...
        $condition['suppliers_id'] = $this->suppliers_id;  // 供货商ID
...
//        $condition['is_store'] = "0";
    ...
    */
            ...
            if($data['refund_type']== 0){
                ...
                if($order['pay_code'] == 'weixin' || $order['pay_code'] == 'alipay' || $order['pay_code'] == 'alipayMobile'){
                    ...
                    if($order['pay_code'] == 'weixin'){
                        ...
                        $payment_obj->payment_red_refund($data);
                    }
                }else{
    ...
    public function detail($order_id){
        ...
        // 获取操作记录
        $action_log = M('order_red_action')->where(array('order_id'=>$order_id,'action_user'=>$this->suppliers_id))->order('log_time desc')->select();
        $has_user   = false;
        $adminIds   = [];
        ...
        if($SupplierIds && count($SupplierIds) > 0){
            $Suppliers = M("Suppliers")->where("suppliers_id in (".implode(",",$SupplierIds).")")->getField("suppliers_id , suppliers_name", true);
        }
        if($has_user){
    ...
    public function edit_order(){
        ...
        if(IS_POST)
        {
            ...
            //$order['shipping_name'] = M('plugin')->where(array('status'=>1,'type'=>'shipping','suppliers_id'=>$suppliers_id,'code'=>I('shipping')))->getField('name');
            ...
            if($o && $l){
                $this->success('修改成功',U('Admin/OrderRed/detail',array('order_id'=>$order_id)));
            }else{
        ...
        //获取支付方式
        $payment = M('plugin')->where(array('status'=>1,'type'=>'payment','code'=>$order['pay_code']))->find();
        //获取配送方式
        $shipping = M('plugin')->where(array('status'=>1,'type'=>'shipping','suppliers_id'=>$suppliers_id,'code'=>$order['shipping_code']))->find();
    ...
    public function deliveryHandle(){
        ...
        if($res){
            $this->success('操作成功',U('/Supplier/OrderRed/delivery_info',array('order_id'=>$data['order_id'])));
        }else{
            $this->success('操作失败',U('/Supplier/OrderRed/delivery_info',array('order_id'=>$data['order_id'])));
        }
    }
    ...
    public function delivery_info(){
        ...
        $delivery_record = M('delivery_red_doc')->alias('d')->join('__ADMIN__ a','a.admin_id = d.admin_id')->where('d.order_id='.$order_id)->select();
        if($delivery_record){
    ...
    public function return_info()
    {
        ...
        $suppliers_id = $this->suppliers_id;
        $return_id    = I('id');
        $return_goods = M('return_red_goods')->where("id = $return_id AND suppliers_id = $suppliers_id")->find();
    #    dump($return_goods);die;
        ...
        if(IS_POST)
        {
          ...
          /*  echo M('return_red_goods')->getlastsql();
            die;*/
        //    dump($data);die;
    ...
    public function add_return_goods()
    {
            ...
            $return_goods = M('return_red_goods')->where("order_id = $order_id and goods_id = $goods_id and suppliers_id = $suppliers_id")->find();
    ...
    public function order_log(){
        ...
        if($Supplier_id >0 ){
            $condition['action_user'] = $Supplier_id;
        }
        //modify
     //   $condition['is_red'] = ['eq',$this->is_red];
        ...
        $Supplier = M('suppliers')->getField('suppliers_id,suppliers_name');
    ...
    public function export_order()
    {
        ...
        $where['suppliers_id'] = $this->suppliers_id;
        ...
        if(is_array($orderList)){
            ...
            foreach($orderList as $k=>$val){
                ...
                $orderGoods = D('order_red_goods')->where('order_id='.$val['order_id'].' AND suppliers_id='.$this->suppliers_id)->select();
                $strGoods="";
                $goods_num = 0;
    ...
    public function ajax_return_list(){
        ...
        $suppliers_id = $this->suppliers_id;
        $where = " 1 = 1 ";
        $where .= " AND suppliers_id = ".$suppliers_id; // 供货商ID;
    ...
    public function ajaxOrderNotice(){
        $suppliers_id = $this->suppliers_id;
        $order_amount = M('order_red')->where("order_status=0 and (pay_status=1 or pay_code='cod') and suppliers_id=".$suppliers_id)->count();
        echo $order_amount;
    }
    ...
    public function batchDelivery(){
        ...
        if(IS_POST){
            ...
            $condition['order_status']    = array('in','1,2,4');
            $condition['suppliers_id'] = $this->suppliers_id;
            $count = M('order_red')->where($condition)->count();
