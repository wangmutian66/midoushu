...
class Plugin extends Base {
    ...
    public function index(){
        $map['suppliers_id'] = $this->suppliers_id;
        $map['type'] = 'shipping';
        $plugin_list = M('plugin')->where($map)->select();
       
        $this->assign('shipping',$plugin_list);
    ...
    public function install(){
        ...
        if($update['status']==0){
            ...
            $config = include $file;
            $update['suppliers_id'] = $config['suppliers_id'];
        ...
        if($row){
            ...
            if($condition['type'] == 'shipping'){
                ...
                $config2['first_weight']    = '1000'; // 首重
                $config2['second_weight']   = '2000'; // 续重
                $config2['money']           = '12';
                $config2['add_money']       = '2';
                $add['shipping_area_name'] = '全国其他地区';
                $add['shipping_code']      = $condition['code'];
                $add['config']             = serialize($config2);
                $add['is_default']         = 1;
                $add['suppliers_id']       = $config['suppliers_id'];
                if($update['status']){
    ...
    private function insertPlugin($plugin_list){
        ...
        $d_list =  M('plugin')->field('code,type')->where(array('type'=>'shipping','suppliers_id'=>$suppliers_id))->select(); // 数据库
        $new_arr = array(); // 本地
        //插件类型
        foreach($plugin_list as $pt=>$pv){
            //  本地对比数据库
            foreach($pv as $t=>$v){
                if($v['suppliers_id'] == $suppliers_id){ // 判断是否是供货商安装插件
                    $tmp['code'] = $v['code'];
                    $tmp['type'] = $pt;
                    $new_arr[]   = $tmp;
                    // 对比数据库 本地有 数据库没有
                    $is_exit = M('plugin')->where(array('type'=>$pt,'code'=>$v['code'],'suppliers_id'=>$suppliers_id))->find();
                    if(empty($is_exit)){
                        $add['code']      = $v['code'];
                        $add['name']      = $v['name'];
                        ...
                        $add['type']      = $pt;
                        if($pt == 'shipping'){
                            $add['suppliers_id'] = $v['suppliers_id'];
                            $add['config']       = empty($v) ? '' : serialize($v); 
                        }else{
    ...
    public function shipping_list(){
        ...
        //2016-01-11 获取插件信息
        $suppliers_id = $this->suppliers_id;
        $shipping_info = M('plugin')->where(array('code'=>$row['code'],'type'=>'shipping','suppliers_id'=>$suppliers_id))->find();
        $result = DB::query($sql);
        //获取配送名称
        $this->assign('plugin',$row);
    ...
    public function shipping_desc(){
        ...
        $desc = I('post.desc');
        $code = I('post.code');
        $row = M('plugin')->where(array('code'=>$code,'type'=>'shipping','suppliers_id'=>$suppliers_id))->save(array('desc'=>$desc));
        if(!$row)
    ...
    // 设置默认物流
    public function shipping_default(){
        $suppliers_id = $this->suppliers_id;
        if(IS_GET){
            $type = I('type');
            $code = I('code');
            $data['is_default'] = 0; 
            $r = M('plugin')->where(array('type'=>'shipping','suppliers_id'=>$suppliers_id))->save($data);
            $data2['is_default'] = 1;
            $r2 = M('plugin')->where(array('code'=>$code,'type'=>'shipping','suppliers_id'=>$suppliers_id))->save($data2);
            if($r2 !== false){
                $this->success('设置成功',U('Plugin/index')); exit;
            }else{
                $this->success('设置失败',U('Plugin/index'));
            }           
        }
    }
    ...
    public function shipping_print(){
    	...
    	if(IS_POST){
    		...
    		$code = I('code');
            $r = M('plugin')->where(array('code'=>$code,'type'=>'shipping','suppliers_id'=>$suppliers_id))->save($data);
    		if($r !== false){
        ...
        if(IS_POST){
            ...
            if(I('get.edit') == 1){
                ...
                if($row){
                    ...
                    if(I('post.default') == 1){
                        //默认全国其他地区
                        exit($this->success("添加成功",U('Supplier/Plugin/shipping_list',array('type'=>'shipping','code'=>$add['shipping_code']))));
                    }
                    M('area_region')->insertAll($add2)&&exit($this->success("添加成功",U('Supplier/Plugin/shipping_list',array('type'=>'shipping','code'=>$add['shipping_code']))));
                }
    ...
    public function add_shipping()
    {
        ...
        $name = I('name');     // 物流名字
        $desc = I('desc', ''); // 插件描述
        $suppliers_id = $this->suppliers_id; // 供货商ID

        $code =  $code.'_'.$suppliers_id;
        $dir  = "./plugins/shipping/$code";

        if(strstr($dir, '../') || strstr($code, '(') || strstr($name, '(') || strstr($desc, '('))
            $this->error("非法输入, 不得带有 ( ) 符号");
        
        if (!preg_match("/[a-zA-Z]{2,20}/", $code)) $this->error("物流编码必须 2-20位小写字母组成");
    ...
    public function del_shipping($code){
        ...
        delFile($dir); // 删除 物流配置
        rmdir($dir); // 删除 物流配置
        M('plugin')->where("code = '$code' and type = 'shipping' and suppliers_id=".$suppliers_id)->delete();
