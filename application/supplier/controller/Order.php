...
use app\supplier\logic\OrderLogic;
use think\AjaxPage;
use think\Page;
use think\Db;
use think\Request;
...
class Order extends Base {
 ...
 //   var $is_red;
    ...
    public function index(){
        //SELECT * FROM `tp_communication` WHERE  (  (  `fromid` = 1  AND `toid` = 5 ) OR (  `fromid` = 1  AND `toid` = 5 ) )  AND `chat` = '1'
        $a = db('communication')->where(function($sql){
            $sql->where(function($sqla){
                $sqla->where(["fromid"=>1,"toid"=>"5"]);
            })->whereOr(function($sql1){
                $sql1->where(["fromid"=>1,"toid"=>"5"]);
            });
        })->where(["chat"=>"1"])->fetchSql(true)->select();
    ...
    public function ajaxindex(){
        ...
        I('order_status2')        != '' ? $condition['order_status']    = array('in','2,4')    : false;
    ...
    public function virtual_cancel(){
        ...
        if(IS_POST){
            ...
            if($order){
                $r = M('order')->where(array('order_id'=>$order_id))->save(array('order_status'=>3,'Supplier_note'=>$Supplier_note));
                if($r){
                    $orderLogic = new OrderLogic();
                    $orderLogic->orderActionLog($order_id,$Supplier_note, '取消订单');
    ...
    */
    ...
    public function detail($order_id){
        ...
        // 获取操作记录
        $action_log  = M('order_action')->where(array('order_id'=>$order_id,'action_user'=>$this->suppliers_id))->order('log_time desc')->select();
        $has_user    = false;
        $SupplierIds = [];
    ...
    public function edit_order(){
       	...
       	if(IS_POST)
        {
            ...
            if($o && $l){
            	$this->success('修改成功',U('Supplier/Order/detail',array('order_id'=>$order_id)));
            }else{
            	$this->success('修改失败',U('Supplier/Order/detail',array('order_id'=>$order_id)));
            }
            exit;
        }
        // 获取省份
    ...
    public function add_order()
    {
        ...
        $area     = M('region')->where(array('parent_id'=>$order['city'],'level'=>3))->select();
        //  获取配送方式
        $shipping_list = M('plugin')->where(array('status'=>1,'type'=>'shipping','suppliers_id'=>$suppliers_id))->select();
        //  获取支付方式
        ...
        if(IS_POST)
        {
            ...
            foreach ($order_goods_arr as $key => $value) {
                ...
                if($order_id)
                {
                    ...
                    M('order_action')->add([
                        'order_id'      => $order_id,
                        'action_user'   => session('suppliers_id'),
                        'order_status'  => 0,  //待支付
                        'shipping_status' => 0, //待确认
                        'action_note'   => $order['Supplier_note'],
                        'status_desc'   => '提交订单',
                        'log_time'      => time()
                    ]);
                }
            ...
            $this->success('添加商品成功',U("Supplier/Order/detail",array('order_id'=>$order_insert_id)));
            exit(); 
        }  
    ...
    public function search_goods()
    {
        ...
        $suppliers_id = $this->suppliers_id;                               // 供货商ID liyi 2018.04.18
    ...
    public function split_order(){
    	...
    	if(IS_POST){
    		...
    		//################################新单处理结束
    		$this->success('操作成功',U('Supplier/Order/detail',array('order_id'=>$order_id)));
            exit;
    	}
    ...
    public function editprice($order_id){
        ...
        if(IS_POST){
            ...
            if(!$row){
                $this->success('没有更新数据',U('Supplier/Order/editprice',array('order_id'=>$order_id)));
            }else{
                $this->success('操作成功',U('Supplier/Order/detail',array('order_id'=>$order_id)));
            }
            exit;
    ...
    public function shipping_print(){
        ...
        $content_var = array($shop['store_name'],$shop['contact'],$shop['phone'],$shop['province'],$shop['city'],
        	...
        	date('d'),date('Y-m-d'),$order['order_sn'],$order['Supplier_note'],$order['shipping_price'],
        );
    ...
    public function deliveryHandle(){
		...
		if($res){
			...
			$this->success('操作失败',U('Supplier/Order/delivery_info',array('order_id'=>$data['order_id'])));
		}
    }
    ...
    public function delivery_info(){
		...
		$delivery_record = M('delivery_doc')->alias('d')->join('__SUPPLIERS__ a','a.suppliers_id = d.suppliers_id')->where('d.order_id='.$order_id)->select();
		if($delivery_record){
    ...
    public function return_info()
    {
        ...
        $suppliers_id = $this->suppliers_id;
        $return_id    = I('id'); //
        $return_goods = M('return_goods')->where("id = $return_id AND suppliers_id = $suppliers_id")->find();
        if(!$return_goods)
        {
            $this->error('非法操作!');
            exit;
    ...
    public function refund_balance(){
		...
		$this->ajaxReturn(['status'=>1,'msg'=>"操作成功",'url'=>U("Supplier/Order/return_list")]);
    ...
    public function add_return_goods()
    {
        ...
        $return_goods = M('return_goods')->where("order_id = $order_id and goods_id = $goods_id and suppliers_id = $suppliers_id")->find();
        if(!empty($return_goods))
        {
            $this->error('已经提交过退货申请!',U('Supplier/Order/return_list'));
            exit;
        }
        $order = M('order')->where("order_id = $order_id")->find();
        ...
        $data['remark']   = '供货商申请退换货'; // 问题描述
        M('return_goods')->add($data);
        $this->success('申请成功,现在去处理退货',U('Supplier/Order/return_list'));
        exit;
    }
    ...
    public function order_action(){    	
        ...
        if($action && $order_id){
            ...
            if($action !=='pay'){  // 不是付款
                ...
                if($action == 'refuse_qx'){
                    $res2 = $orderLogic->orderRefuseLog($order_id,0,$convert_action,0,I('note'));
                }
            }

        	$a = $orderLogic->orderProcessHandle($order_id,$this->suppliers_id,$action,array('note'=>I('note'),'suppliers_id'=>$this->suppliers_id)); // 操作订单

        	if($res !== false && $a !== false){
                if ($action == 'remove') {
                    exit(json_encode(array('status' => 1, 'msg' => '操作成功', 'data' => array('url' => U('Supplier/order/index')))));
                }
        	 	exit(json_encode(array('status' => 1,'msg' => '操作成功')));
        	}else{
                if ($action == 'remove') {
                    exit(json_encode(array('status' => 0, 'msg' => '操作失败', 'data' => array('url' => U('Supplier/order/index')))));
                }
        	 	exit(json_encode(array('status' => 0,'msg' => '操作失败')));
        	}
        }else{
        	$this->error('参数错误',U('Supplier/Order/detail',array('order_id'=>$order_id)));
        }
    }
    ...
    public function export_order()
    {
    	...
    	$strTable .= '<td style="text-align:center;font-size:12px;" width="*">订单成本价</td>';
        ...
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">是否全返</td>';
	    ...
	    if(is_array($orderList)){
	    	...
	    	foreach($orderList as $k=>$val){
                $n++;
                if($val['is_allreturn'] == 1) $allreturn_str = '全返订单';
	    		...
	    		$orderGoods = D('order_goods')->where('order_id='.$val['order_id'].' AND suppliers_id='.$this->suppliers_id)->select();
	    		$strGoods="";
                $goods_num = 0;
...
#        echo M('return_goods')->getlastsql();die;
    ...
    public function ajaxOrderNotice(){
        $suppliers_id = $this->suppliers_id;
        $order_amount = M('order')->where("order_status=0 and (pay_status=1 or pay_code='cod') and suppliers_id=".$suppliers_id)->count();
        echo $order_amount;
    }
