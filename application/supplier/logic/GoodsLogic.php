...
class GoodsLogic extends Model
{
    ...
    var $suppliers_id;
    public function __construct($suppliers_id=0)
    {
        if($suppliers_id != 0)
     ...
     * @return  mix
     */
    public function goods_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0,$is_red= 0)
    {
                global $goods_category, $goods_category2;                
                $sql = "SELECT * FROM  __PREFIX__goods_category where is_red = {$is_red} ORDER BY parent_id , sort_order ASC";
    ...
    {     
        ...
        foreach ($spec_arr_sort as $key =>$val)
        {
         ...
         $keySpecGoodsPrice = M('SpecGoodsPrice')->where('goods_id = '.$goods_id)->getField('key,key_name,price,cost_price,store_count,bar_code,sku');//规格项
