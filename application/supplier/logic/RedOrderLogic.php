...
class RedOrderLogic extends Model
{
    ...
    public function getOrderGoods($order_id,$is_send =''){
        ...
        $sql = "SELECT g.*,o.*,(o.goods_num * o.cost_price) AS goods_total FROM __PREFIX__order_red_goods o ".
...
//        /* 选择一个随机的方案 */send_http_status('310');
    ...
    function order_pay_cancel($order_id)
    {
    	...
    	foreach($orderGoodsArr as $key => $val)
    	{
    		...
    		{   // 先到规格表里面增加数量 再重新刷新一个 这件商品的总数量
				$SpecGoodsPrice = new \app\common\model\RedSpecGoodsPrice();
    ...
    public function deliveryHandle($data){
		...
		$data['suppliers_id']   = session('suppliers.suppliers_id');
    ...
    public function updateOrderShipping($data=[],$order=[]){
        ...
        $delivery_res = M('delivery_red_doc')->where(['order_id'=>$data['order_id']])->save($updata);  //改变售后的信息
