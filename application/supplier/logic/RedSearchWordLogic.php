...
use app\supplier\model\FlashSale;
use think\Model;
use think\db;
 ...
 * 秒杀逻辑定义
 * Class CatsLogic
 * @package supplier\Logic
 */
class RedSearchWordLogic extends Model
{
    public function initGoodsSearchWord($sid){
        header("Content-Type:text/html;charset=utf-8");
        Db::query('TRUNCATE table '.C('database.prefix').'search_word');
        $where['keywords']  = array('neq' , '');
        $where['suppliers_id'] = $sid; 
        $goodsKeyWords = Db::name('goods_red')->field('keywords')->where($where)->distinct('keywords')->select();
        ...
        foreach($goodsKeyWords as $key=>$val){
            ...
            $goods_num = Db::name('goods_red')->where('goods_name|keywords','like',$goods_where_like)->count();
            $search_add_data['goods_num'] = $goods_num;
            $success = Db::name('search_red_word')->insert($search_add_data);
            if($success !== false){
                $successNum++;
            }
