...
//        $this->nowPage    = empty($_REQUEST[$this->p]) ? 1 : intval($_REQUEST[$this->p]);        
    ...
    public function show_cx() {
        ...
        //上一页
        $up_row  = $this->nowPage - 1;
        $up_page = $up_row > 0 ? '<a class="bl" href="' . $this->url($up_row) . '">' . $this->config['prev'] . '</a>' : '';

        //下一页
        $down_row  = $this->nowPage + 1;
        $down_page = ($down_row <= $this->totalPages) ? '<a class="bl" href="' . $this->url($down_row) . '">' . $this->config['next'] . '</a>' : '';

        //第一页
        $the_first = '';
        if($this->totalPages > $this->rollPage && ($this->nowPage - $now_cool_page) >= 1){
            $the_first = '<a class="bl" href="' . $this->url(1) . '">' . $this->config['first'] . '</a>';
        }

        //最后一页
        $the_end = '';
        if($this->totalPages > $this->rollPage && ($this->nowPage + $now_cool_page) < $this->totalPages){
            $the_end = '<a class="bl" href="' . $this->url($this->totalPages) . '">' . $this->config['last'] . '</a>';
        }
        ...
        for($i = 1; $i <= $this->rollPage; $i++){
            ...
            if($page > 0 && $page != $this->nowPage){
                ...
                if($page <= $this->totalPages){
                    $link_page .= '<a class="bl" href="' . $this->url($page) . '">' . $page . '</a>';
                }else{
                    break;
                }
            }else{
                if($page > 0 && $this->totalPages != 1){
                    //  $link_page .= '<span class="current">' . $page . '</span>';
                    $link_page .= '<a class="bl ys" tabindex="0" data-dt-idx="1" aria-controls="example1" href="#">' . $page . '</a>';
        ...
        $page_str = str_replace(
            ...
            array($this->config['header'], $this->nowPage, $up_page, $down_page, $the_first, $link_page, $the_end, $this->totalRows, $this->totalPages),
            $this->config['theme']);
        return "<div class='fy'>{$page_str}</div>";
    }


    public function show_cxpc() {
        if(0 == $this->totalRows) return '';
        ...
        for($i = 1; $i <= $this->rollPage; $i++){
            ...
            if($page > 0 && $page != $this->nowPage){
                ...
                if($page > 0 && $this->totalPages != 1){
                    //  $link_page .= '<span class="current">' . $page . '</span>';
                    $link_page .= '<a class="on" tabindex="0" data-dt-idx="1" aria-controls="example1" href="#">' . $page . '</a>';
        ...
        $page_str = str_replace(
            ...
            array($this->config['header'], $this->nowPage, $up_page, $down_page, $the_first, $link_page, $the_end, $this->totalRows, $this->totalPages),
            $this->config['theme']);
        return "<div class='y-sxfy fri'>{$page_str}</div>";
    }
    ...
    public function show_sup() {
        ...
        //上一页
        $up_row  = $this->nowPage - 1;
        $up_page = $up_row > 0 ? '<a class="ssy" href="' . $this->url($up_row) . '">' . $this->config['prev'] . '</a>' : '';

        //下一页
        $down_row  = $this->nowPage + 1;
        $down_page = ($down_row <= $this->totalPages) ? '<a class="ssy" href="' . $this->url($down_row) . '">' . $this->config['next'] . '</a>' : '';

        //第一页
        $the_first = '';
        if($this->totalPages > $this->rollPage && ($this->nowPage - $now_cool_page) >= 1){
            $the_first = '<a class="ssy" href="' . $this->url(1) . '">' . $this->config['first'] . '</a>';
        }

        //最后一页
        $the_end = '';
        if($this->totalPages > $this->rollPage && ($this->nowPage + $now_cool_page) < $this->totalPages){
            $the_end = '<a class="ssy" href="' . $this->url($this->totalPages) . '">' . $this->config['last'] . '</a>';
        }
        ...
        for($i = 1; $i <= $this->rollPage; $i++){
            ...
            if($page > 0 && $page != $this->nowPage){
                ...
                if($page <= $this->totalPages){
                    $link_page .= '<a class="fy" href="' . $this->url($page) . '">' . $page . '</a>';
                }else{
                    break;
                }
            }else{
                if($page > 0 && $this->totalPages != 1){
                    //  $link_page .= '<span class="current">' . $page . '</span>';
                    $link_page .= '<a class="fy ys" tabindex="0" data-dt-idx="1" aria-controls="example1" href="#">' . $page . '</a>';
                }
            }
        }
