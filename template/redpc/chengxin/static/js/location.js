...
function ajaxGetRegion()
{
	...
	$.ajax({
		type : "get",
		dataType:'json',
		url:"/index.php?m=Homered&c=Goods&a=getRegion",
...
function ajaxDispatching(region_id) {
	...
	if(typeof(goods_id) != 'undefined' && region_id!= ''){
		$.ajax({
			...
			url: "/index.php?m=Homered&c=Goods&a=dispatching",
			success: function (data) {
				if (data.status == 1) {
...
//                        addrHtml = oldAddrHtml + "," + tProvince + tCity + tArea + tTown;
			...
			function chooseCity(cityId, cityName) {
				...
				$.getJSONP("/index.php?m=Homered&c=Goods&callback=getAreaListcallback&a=region&fid="+cityId);
			}
			function chooseArea(areaId, areaName) {
				...
				$.getJSONP("/index.php?m=Homered&c=Goods&callback=getAreaListcallback&a=region&fid="+areaId);
			}
