...
function get_cart_num() {
	...
	if (cart_cn == '') {
		$.ajax({
			type: "GET",
			url: "/index.php?m=Homered&c=Cart&a=header_cart_list",//+tab,
...
function header_cart_del(ids) {
	...
	$.ajax({
		type: "POST",
		url: "/index.php?m=Homered&c=Cart&a=delete",
...
function ajax_side_cart_list() {
	$.ajax({
		type: "GET",
		url: "/index.php?m=Homered&c=Cart&a=header_cart_list&template=ajax_side_cart_list",//+tab,
		success: function (data) {
			cart_cn = getCookie('cn');
			$('#cart_quantity').html(cart_cn);
			$('#tab_cart_num').html(cart_cn);
