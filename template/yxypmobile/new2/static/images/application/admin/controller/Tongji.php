...
class Tongji extends Base {
    ...
    function initialize_start(){
        ...
        $where['is_allreturn']  =['eq',1];
        $previous_order_list = M('order o')->field('order_id,user_id,order_amount,shipping_price,already_rebate,add_time,order_sn')
                                ->where($where)
                                ->cache('order_initialize',600)
                                ->select();
        unset($where['is_allreturn']);
        if($previous_order_list){
            foreach ($previous_order_list as $key => $value) {
                $previous_order_list[$key]['is_red']    =   0;
            }
        }
        $where['already_rebate'] = ['exp','< (midou_money - shipping_price) '];
        $previous_order_list_red = M('order_red o')
                                ->field('order_id,user_id,order_amount,shipping_price,already_rebate,midou_money,add_time,order_sn')
                                ->where($where)
                                ->cache('order_red_initialize',600)
                                ->select();
        ...
        $previous_order_list = array_merge($previous_order_list,$previous_order_list_red);
        $previous_order_list = $this->bubble_sort($previous_order_list); 
        $i = 1;
        foreach ($previous_order_list as $key => $value) {
