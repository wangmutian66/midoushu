...
class Goods extends Base {
    ...
    public function ajaxGoodsList(){            
        ...
        (I('is_allreturn') !== '') && $where = "$where and g.is_allreturn = ".I('is_allreturn');           
        $cat_id = I('cat_id');
        // 关键词搜索               
        $key_word = I('key_word') ? trim(I('key_word')) : '';
        if($key_word)
        {
            $where = "$where and (g.goods_name like '%$key_word%' or g.goods_sn like '%$key_word%' or s.suppliers_name like '%$key_word%')" ;
        }
