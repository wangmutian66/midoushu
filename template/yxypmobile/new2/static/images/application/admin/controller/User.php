    ...
    */
    ...
    public function account_edit(){
        ...
        if(IS_POST){
            ...
            if(accountLog($user_id,$user_money,$pay_midou,$pay_points,$desc,0))
            {
    ...
    public function transfer(){
    	...
    	foreach($withdrawals as $val){
    		...
    		if($user['frozen_money'] < $val['money'])
    		{
    			...
    			if($atype == 'online'){
                    ...
                    $up_data['frozen_money'] = -1*$val['money']+$user['frozen_money'];
    ...
    function tocash(){
        ...
        $Page  = new Page($count,20);
        $list = Db::name('tocash')->alias('cash')->field('cash.*,u.nickname,mobile')->join('__USERS__ u', 'u.user_id = cash.user_id', 'left')->where($where)->order("cash.id desc")->limit($Page->firstRow.','.$Page->listRows)->select();
        $show  = $Page->show();
        $this->assign('show',$show);
        $this->assign('list',$list);
        $this->assign('pager',$Page);
    ...
    function doCash(){
        ...
        if($id){
            ...
            if($status == -1){
                ...
                if($r['dj_rebate'] < $r['money']){
                    $res['status']  =   0;
                    $res['info']    =   '冻结余额不足，审核失败！';
                    $this->ajaxReturn($res);
                }else{
                    $user_data['dj_rebate'] =   ['exp',"dj_rebate - {$r['money']}"];
                    $user_data['rebate_money']  =   ['exp',"rebate_money + {$r['money']}"];
            ...
            #    $rebate_money = db('users')->where("user_id = {$user_id}")->getField('rebate_money');
                ...
                if($r['dj_rebate'] < $r['money']){
                    ...
                    #现金与红包比例
                    $temp_ar = explode('|',tpCache('proportion.red_envelope'));     
                    $back_money = $r['money'] * $temp_ar[0];
                    $back_midou = $r['money'] * $temp_ar[1];
                    $user_data['dj_rebate'] =   ['exp',"dj_rebate - {$r['money']}"];
                    ...
                    if($s){
                        $red_envelope_data  =   ['red_name'=>'返利提现','create_time'=>NOW_TIME,'source'=>'提现反米豆','money'=>$back_midou,'user_id'=>$r['user_id']];
