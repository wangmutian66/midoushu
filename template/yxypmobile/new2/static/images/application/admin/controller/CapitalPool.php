  ...
  //  public  $mod;
   ...
   #     $total_ratio = explode('|',tpCache('proportion.total_ratio'));
    ...
    #    $back_money = bcmul($money,$total_ratio[1],9);
        ...
        $previous_order_list = M('order')->field('order_id,user_id,order_amount,shipping_price,already_rebate,add_time')->where("add_time < ".NOW_TIME." and order_status in(2,4) and already_rebate < (order_amount - shipping_price) ")->select();
        if($previous_order_list){
            foreach ($previous_order_list as $key => $value) {
                $previous_order_list[$key]['is_red']    =   0;
            }
        }
    #    $previous_order_list[1]['add_time'] =   1527237038;        测试相同时间的数据
        $previous_order_list_red = M('order_red')->field('order_id,user_id,order_amount,shipping_price,already_rebate,midou_money,add_time')->where("add_time < ".NOW_TIME." and order_status in(2,4) and already_rebate < (midou_money - shipping_price) ")->select();
        if($previous_order_list_red){
            foreach ($previous_order_list_red as $key => $value) {
                $previous_order_list_red[$key]['is_red']    =   1;
            }
        }
        $previous_order_list = array_merge($previous_order_list,$previous_order_list_red);      #合并数组
      /*  dump($previous_order_list);
        ...
        $previous_order_list = $this->bubble_sort($previous_order_list);                #数组按照下单时间排序
...
/*        dump($back_proportion);
            ...
            foreach ($back_proportion as $k => $v) {
                ...
                //    echo $k;
                    ...
                    if($value['is_red'] == 0){
                        ...
                        $limit_money = bcsub(bcsub($value['midou_money'],$value['shipping_price'],9),$value['already_rebate'],9);
                    }
                 ...
                 //     echo $money;die;
                    ...
                    if($value['is_red'] == 1){
                        $order_update_rebate_red[]  =   ['order_id'=>$value['order_id'],'already_rebate'=>['exp',"already_rebate + {$money}"]];    
                    }else{
      ...
      /*  dump($order_update_rebate);
        ...
        if($order_update_rebate_red){
            model("order_red")->saveAll($order_update_rebate_red);
        }
