		...
		success: function (data) {
			...
			if (form.length > 0) {
				...
				layer.open({
					...
					yes: function () {
						ajax_header_cart();
						// location.href = "/index.php?m=Mobile&c=Index&a=index";
...
function AjaxAddCartReturn(goods_id, num, to_cart) {
	...
	$.ajax({
		type: "POST",
		url: "/index.php?m=Mobile&c=ReturnCart&a=ajaxAddCart",
		data: data,
		dataType: 'json',
		success: function (data) {
			// 加入购物车后再跳转到 购物车页面
			if (form.length > 0) {
				...
				if (to_cart == 1) {
					//直接购买
					location.href = "/index.php?m=Mobile&c=ReturnCart&a=index";
				}
				var cart_num = parseInt(cart_quantity.html()) + parseInt($('#number').val());
				cart_quantity.html(cart_num);
				layer.open({
				...
				if (data.status == -1) {
					location.href = "/index.php?m=Mobile&c=ReturnGoods&a=goodsInfo&id=" + goods_id;
				}
				if (data.status <= 0) {
					if(!$.isEmptyObject(data.result)){
						if(!$.isEmptyObject(data.result.url)){
