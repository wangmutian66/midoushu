 ...
 function AjaxAddCartReturn(goods_id, num, to_cart) {
	 // 如果有商品规格 说明是商品详情页提交
	 var form = $("#buy_goods_form");
	 var cart_quantity = $("#cart_quantity");
	 var data;//post数据
	 ...
	 $.ajax({
		 type: "POST",
		 url: "/index.php?m=Home&c=ReturnCart&a=ajaxAddCart",
		 data: data,
		 dataType: 'json',
		 success: function (data) {
			 var cart_num;
			 if (form.length > 0) {
				 // 加入购物车后再跳转到 购物车页面
				 if (to_cart == 1) {
					 location.href = "/index.php?m=Home&c=ReturnCart&a=index";//直接购买
				 }
				 cart_num = parseInt(cart_quantity.html()) + parseInt($('input[name="goods_num"]').val());
			 } else {
				 if (data.status == -1) {
					 location.href = "/index.php?m=Home&c=ReturnGoods&a=goodsInfo&id=" + goods_id;
					 return false;
				 }
				 cart_num = parseInt(cart_quantity.html()) + parseInt(num);
			 }
			 ...
			 layer.open({
				 ...
				 skin: 'layui-layer-rim', //加上边框
				 area: ['490px', '386px'], //宽高
				 content: "/index.php?m=Home&c=ReturnGoods&a=open_add_cart"
			 });
		 }
	 });
 }
