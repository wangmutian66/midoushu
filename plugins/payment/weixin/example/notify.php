...
class PayNotifyCallBack extends WxPayNotify
{
	...
	public function NotifyProcess($data, &$msg)
	{
        ...
        $out_trade_no = $order_sn = $data['out_trade_no']; //商户系统的订单号，与请求一致。
        ...
        # db('test')->add(['name'=>1,'email'=>$order_sn]);
        ...
        #    db('test')->add(['name'=>2,'email'=>$order_sn]);
            $order_amount = M('recharge')->where(['order_sn' => $order_sn, 'pay_status' => 0])->value('account');
       #     db('test')->add(['name'=>3,'email'=>$order_sn,'number'=>$order_amount]);
        }elseif(stripos($out_trade_no,'midou')!==false){
            //米豆商城
            $order_sn = substr($out_trade_no, 0, -6);
            $order_amount = M('order_red')->where(['order_sn' => "$order_sn"])->value('order_amount');
        }elseif(stripos($out_trade_no,'staff_paid')!==false){
            //员工代付
            $order_sn = substr($out_trade_no, 0, -4);
     ...
     #   db('test')->add(['name'=>4,'email'=>$order_sn,'number'=>$order_amount]);
