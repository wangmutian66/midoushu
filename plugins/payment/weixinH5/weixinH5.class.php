...
class weixinH5
{
   ...
   #     dump($config);die;
    ...
    function get_code($order, $config)
    {
        ...
        $notify_url = SITE_URL . '/index.php/mobile/Payment/notifyUrl/pay_code/weixinH5'; 
  ...
  /*      dump($return);
        die;*/
      #  $redirect_url = urlencode("redirect_url=https://www.midoushu.com/home/test");
     #   $return['mweb_url'] = $return['mweb_url'] . '&redirect_url=https%3A%2F%2Fwww.midoushu.com/home/test';
        //echo $return['mweb_url'];die;
     ...
     */
    function get_paid_code($paid_order, $config)
    {     
        ...
        if(is_array($r)){
            if($r['is_weixin'] == 1){
                WxPayConfig::$appid = $r['w_appid']; // * APPID：绑定支付的APPID（必须配置，开户邮件中可查看）
                WxPayConfig::$mchid = $r['w_mchid']; // * MCHID：商户号（必须配置，开户邮件中可查看）
                WxPayConfig::$key = $r['w_miyao']; // KEY：商户支付密钥，参考开户邮件设置（必须配置，登录商户平台自行设置）  
            }
        }
        ...
        $company_name = db('staff staff')
                                 ->where("staff.id = {$paid_order['staff_id']}")
                                 ->join('company store','store.cid = staff.store_id')
                                 ->getField('store.cname');
        if(stripos($paid_order['paid_sn'],'staff_paid')!==false){
            //员工代付
            $title = $company_name . ' - 员工代付';
        }elseif(stripos($paid_order['paid_sn'],'mypays')!==false){
            //员工代付
            $title = $company_name . ' - 扫码';
        }
  ...
  /*      echo "米豆薯 - {$title}订单支付";
        ...
        $input = new WxPayUnifiedOrder();
        $input->SetBody("米豆薯 - {$title}订单支付"); // 商品描述
        $input->SetAttach("weixinH5"); // 附加数据，在查询API和支付通知中原样返回，该字段主要用于商户携带订单的自定义数据
        $input->SetOut_trade_no($paid_order['paid_sn'].rand(1111,9999)); // 商户系统内部的订单号,32个字符内、可包含字母, 其他说明见商户订单号

        $input->SetTotal_fee($paid_order['money']*100); // 订单总金额，单位为分，详见支付金额
        $input->SetTrade_type("MWEB"); // 交易类型   取值如下：JSAPI，NATIVE，APP，MWEB 详细说明见参数规定    MWEB--H5wap支付
        $input->SetNotify_url($notify_url); // 接收微信支付异步通知回调地址，通知url必须为直接可访问的url，不能携带参数。
        $input->SetSceneInfo($sceneInfo);
